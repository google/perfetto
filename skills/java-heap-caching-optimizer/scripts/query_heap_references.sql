...
SELECT
  ...
  FROM heap_graph_object o_owner
  JOIN heap_graph_class c_owner ON o_owner.type_id = c_owner.id
  JOIN heap_graph_reference r ON r.owner_id = o_owner.id
  JOIN heap_graph_object o_owned ON r.owned_id = o_owned.id
  JOIN heap_graph_class c_owned ON o_owned.type_id = c_owned.id
WHERE c_owner.name = '<owner_classname>'
