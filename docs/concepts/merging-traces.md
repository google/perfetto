...
3. **Drop.** Two different real clock domains (say `BOOTTIME` here and
   ...
   related to trace time are dropped and counted in the
   `clock_sync_unrelatable_clock_domains` and `clock_sync_failure_no_path`
   stats. The fix is to record clock snapshots, or to assert the relation in
   a manifest.
...
declare. For Perfetto proto traces the default is `BOOTTIME`.
...
represented and are dropped, counted in the
`trace_sorter_negative_timestamp_dropped` stat. The most common cause in
merged traces is a manifest `offset_ns` that moves a file too far.
