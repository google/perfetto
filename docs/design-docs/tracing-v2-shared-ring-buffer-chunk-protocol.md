...
This document specifies the chunk layout, its 32-bit state word, every legal
...
The state word is 32 bits. Perfetto still supports 32-bit targets where a
64-bit atomic is not guaranteed to be lock-free.
...
`writer_b` has published behind the reader. Making the reader the only actor
that writes `Free` was necessary, but it did not identify *which traversal* the
free word belongs to.
...
statistics. Its exact layout is outside this document.
...
A writer may not do that after a failed claim. Its reservation authorizes one
exact expected word. A different word belongs to another owner or traversal.
...
flowchart TD
    ...
    Hole["release-store read_pos + 1;<br/>return Skipped"]
    Emit["release-store read_pos + 1;<br/>return Emitted"]
    Retry{"retry budget left?"}
    Later["return RetryLater;<br/>read_pos unchanged"]
    ...
    State -- "Complete" --> All
    All --> Reclaim
    State -- "RewriteRequested" --> Hole
    State -- "Acknowledged" --> Ack
    Advance -- "CAS succeeds" --> Hole
    Mark -- "CAS succeeds, valid bytes" --> Emit
    Mark -- "CAS succeeds, no usable bytes" --> Hole
    Reclaim -- "CAS succeeds, valid bytes" --> Emit
    Reclaim -- "CAS succeeds, no usable bytes" --> Hole
    Ack -- "CAS succeeds" --> Hole
    Advance -- "CAS fails" --> Retry
    Mark -- "CAS fails" --> Retry
    Reclaim -- "CAS fails" --> Retry
    Retry -- yes --> State
    Retry -- no --> Later
```

***A mismatched free tag, a reserved state value, or a failed `Acknowledged`
reclaim is a protocol error. The reader reports it and leaves both the chunk
state and `read_pos` unchanged.*** 
...
[rfc14]: published_rfcs/rfc-0014-tracing-protocol-redesign.md
