...
NOTE: The TraceProcessor can be initialized in a combination of ways including:
      ...
      (e.g. `TraceProcessor(file_path='trace.perfetto-trace', addr='localhost:9001')`)
      <br> - A path to a `trace_processor` binary and the trace to be loaded in
      (e.g. `TraceProcessor(file_path='trace.perfetto-trace', config=TraceProcessorConfig(bin_path='./trace_processor'))`)
