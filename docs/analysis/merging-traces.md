...
archive (ZIP or TAR) containing the files to merge:

```bash
tar cf merged.tar trace_a.pftrace trace_b.pftrace
...
```bash
tar cf merged.tar perfetto_manifest.json device_a.pftrace device_b.pftrace
trace_processor merged.tar
```
...
[the perfetto_manifest reference](/docs/reference/perfetto-manifest.md).
