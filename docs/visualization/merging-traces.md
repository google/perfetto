...
**+ Add machine...** to create a named machine (for example "Phone A") so
...
the machine name as a suffix, for example `CPU 0 Scheduling (Phone A)` or
`system_server 902 (Phone A)`; tracks from the default machine are
unsuffixed. The timeline spans the union of all traces' recording windows,
so two traces recorded minutes apart legitimately produce a long timeline
with activity clustered at each end.

![A merged trace with per-machine track suffixes](/docs/images/merging-traces-merged-timeline.png)
