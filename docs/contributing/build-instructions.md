...
A presubmit check (`tools/run_presubmit`, also run by CI on every pull request)
checks that the Android.bp is consistent with GN files.
...
- Android's NDK r26c (using NDK's libcxx)
