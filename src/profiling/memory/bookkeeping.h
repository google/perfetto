...
class HeapTracker {
 public:
  ...
  void RecordMalloc(const std::vector<UnwindFrame>& callstack,
                    uint64_t address,
  ...
  uint64_t GetSizeForTesting(const std::vector<UnwindFrame>& stack);
  uint64_t GetMaxForTesting(const std::vector<UnwindFrame>& stack);
  uint64_t GetMaxCountForTesting(const std::vector<UnwindFrame>& stack);
