...
// Native methods are exported under the JNI name-mangling convention
// (Java_<package>_<class>_<method>) so they resolve lazily on first
// call using the caller's class loader. This avoids an eager
// FindClass in JNI_OnLoad, which would use the boot class loader and
// fail for classes loaded by the system_server class loader.
#ifdef PERFETTO_JNI_JARJAR_PREFIX
#define JNI_FN(name) \
  Java_com_android_internal_dev_perfetto_sdk_PerfettoDataSource_##name
#else
#define JNI_FN(name) Java_dev_perfetto_sdk_PerfettoDataSource_##name
#endif
...
// Returns a JNIEnv for the calling thread. Callbacks run on Perfetto's
// internal service thread, which is not attached to the JVM by default —
// attach it as a daemon so it stays attached for the lifetime of the
// process without requiring an explicit detach.
JNIEnv* GetJNIEnv(JavaVM* jvm) {
  ...
  if (status == JNI_EDETACHED) {
    if (jvm->AttachCurrentThreadAsDaemon(&env, /* args= */ nullptr) != JNI_OK) {
      return nullptr;
    }
    return env;
  }
...
// If a Java callback threw, log and clear the exception. Otherwise the next
// JNI call from the same thread will fail and the user-visible error will be
...
// ============================================================================
// Data source callbacks (invoked by the C SDK on its internal thread)
// ============================================================================
...
using perfetto::jni::DsState;
using perfetto::jni::IncrState;

// ============================================================================
// JNI-exposed methods (lazy-resolved by JNI name mangling)
// ============================================================================

extern "C" JNIEXPORT jlong JNICALL JNI_FN(nativeRegister)(JNIEnv* env,
                                                          jclass,
                                                          jobject java_ds,
                                                          jstring name) {
  ...
  PerfettoDsSetOnSetupCallback(ds_impl, perfetto::jni::OnSetup);
  PerfettoDsSetOnStartCallback(ds_impl, perfetto::jni::OnStart);
  PerfettoDsSetOnStopCallback(ds_impl, perfetto::jni::OnStop);
  PerfettoDsSetOnFlushCallback(ds_impl, perfetto::jni::OnFlush);
  PerfettoDsSetOnCreateIncr(ds_impl, perfetto::jni::OnCreateIncr);
  PerfettoDsSetOnDeleteIncr(ds_impl, perfetto::jni::OnDeleteIncr);
...
JNI_FN(nativeCheckAnyIncrementalStateCleared)(jlong ds_ptr) {
  auto* ds_impl =
...
JNI_FN(nativeWritePacketToAllInstances)(JNIEnv* env,
                                        jclass,
                                        jlong ds_ptr,
                                        jbyteArray buf,
                                        jint len) {
...
extern "C" JNIEXPORT void JNICALL JNI_FN(nativeFlush)(jlong ds_ptr) {
  auto* ds_impl =
      reinterpret_cast<struct PerfettoDsImpl*>(static_cast<uintptr_t>(ds_ptr));
