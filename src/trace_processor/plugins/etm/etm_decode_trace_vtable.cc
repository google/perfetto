...
#include "src/trace_processor/plugins/etm/opencsd.h"
#include "src/trace_processor/plugins/etm/sql_values.h"
#include "src/trace_processor/plugins/etm/util.h"
...
base::StatusOr<tables::EtmV4ChunkTable::Id> GetEtmV4ChunkId(
  ...
  if (in_id.AsLong() < 0 ||
      in_id.AsLong() >= plugin->chunk_table().row_count()) {
    return base::ErrStatus("Invalid chunk_id value: %" PRIu32,
                           plugin->chunk_table().row_count());
  }
...
class EtmDecodeChunkVtable::Cursor
 ...
 public:
  explicit Cursor(Vtab* vtab)
      : plugin_(vtab->plugin), cursor_(nullptr, vtab->plugin) {}
...
base::Status EtmDecodeChunkVtable::Cursor::Filter(int,
  ...
  for (; *idxStr != 0; ++idxStr, ++argv) {
    switch (*idxStr) {
      case kChunkIdEqArg: {
        ASSIGN_OR_RETURN(id, GetEtmV4ChunkId(plugin_, *argv));
        break;
      }
