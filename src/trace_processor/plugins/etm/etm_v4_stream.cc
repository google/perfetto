...
#include "src/trace_processor/plugins/etm/etm_v4_stream.h"
...
#include "src/trace_processor/plugins/etm/frame_decoder.h"
...
void EtmV4Stream::StartSession(std::optional<int64_t> start_ts) {
  ...
  auto session_id = EtmTpPlugin::Get(context_)->mutable_session_table()
...
void EtmV4Stream::EndSession() {
  ...
  for (auto& chunk : session_->chunks_) {
    ...
    auto id = EtmTpPlugin::Get(context_)->mutable_chunk_table()
