...
class IntructionCursor : public sqlite::Module<EtmIterateRangeVtable>::Cursor {
 public:
  explicit IntructionCursor(EtmTpPlugin* plugin) : plugin_(plugin) {}
 ...
 private:
  ...
  void Reset(const InstructionRangeSqlValue* range) {
    ...
    const auto& config =
        StorageHandle(plugin_).GetEtmV4Config(range->config_id);
...
int EtmIterateRangeVtable::Open(sqlite3_vtab* vtab,
  ...
  *cursor = new IntructionCursor(GetVtab(vtab)->plugin);
  return SQLITE_OK;
}
