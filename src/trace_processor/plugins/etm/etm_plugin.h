...
#ifndef SRC_TRACE_PROCESSOR_PLUGINS_ETM_ETM_PLUGIN_H_
#define SRC_TRACE_PROCESSOR_PLUGINS_ETM_ETM_PLUGIN_H_
...
// Plugin for Embedded Trace Macrocell (ETM) support.
// Owns the ETM-specific tables and data, registers ETM operators.
class EtmPlugin : public Plugin<EtmPlugin> {
 public:
  EtmPlugin();
  ~EtmPlugin() override;

  // Retrieves this module from TraceProcessorContext.
  static EtmPlugin* Get(TraceProcessorContext* context);

  // --- PluginBase overrides ---
  void RegisterImporters(TraceProcessorContext* context) override;
  void RegisterDataframes(TraceProcessorContext* context,
                          std::vector<PluginDataframe>& tables) override;
  void RegisterSqliteModules(
      TraceProcessorContext* context,
      std::vector<SqliteModuleRegistration>& modules) override;
...
#endif  // SRC_TRACE_PROCESSOR_PLUGINS_ETM_ETM_PLUGIN_H_
