...
PERFETTO_TP_REGISTER_PLUGIN(EtmTpPlugin);

EtmTpPlugin::EtmTpPlugin() = default;
EtmTpPlugin::~EtmTpPlugin() = default;

EtmTpPlugin* EtmTpPlugin::Get(TraceProcessorContext* context) {
  return static_cast<EtmTpPlugin*>(context->etm_tp_plugin);
}

void EtmTpPlugin::RegisterImporters(TraceProcessorContext* context) {
...
void EtmTpPlugin::OnEventsFullyExtracted(TraceProcessorContext*) {
  if (etm_tracker_) {
    etm_tracker_->Finalize();
  }
}

void EtmTpPlugin::RegisterStaticTables(
    TraceStorage*,
...
void EtmTpPlugin::RegisterFunctionsAndOperators(TraceStorage*,
