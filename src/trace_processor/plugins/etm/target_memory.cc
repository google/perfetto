...
#include "src/trace_processor/plugins/etm/virtual_address_space.h"
...
void TargetMemory::InitStorage(TraceProcessorContext* context) {
  auto* plugin = EtmTpPlugin::Get(context);
  PERFETTO_CHECK(plugin->target_memory() == nullptr);
  plugin->set_target_memory(
      std::unique_ptr<Destructible>(new TargetMemory(context)));
}

// static
const TargetMemory* TargetMemory::Get(TraceProcessorContext* context) {
  auto* plugin = EtmTpPlugin::Get(context);
  PERFETTO_DCHECK(plugin->target_memory() != nullptr);
  return static_cast<const TargetMemory*>(plugin->target_memory());
}
