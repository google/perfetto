...
#include "src/trace_processor/plugins/etm/element_cursor.h"
...
#include "src/trace_processor/plugins/etm/etm_v4_decoder.h"
#include "src/trace_processor/plugins/etm/mapping_version.h"
#include "src/trace_processor/plugins/etm/sql_values.h"
#include "src/trace_processor/plugins/etm/storage_handle.h"
#include "src/trace_processor/plugins/etm/target_memory.h"
#include "src/trace_processor/plugins/etm/target_memory_reader.h"
#include "src/trace_processor/plugins/etm/types.h"
#include "src/trace_processor/plugins/etm/etm_tp_plugin.h"
#include "src/trace_processor/storage/trace_storage.h"

namespace perfetto::trace_processor::etm {

ElementCursor::ElementCursor(TraceStorage* storage, EtmTpPlugin* plugin)
    : storage_(storage),
      plugin_(plugin),
      reader_(std::make_unique<TargetMemoryReader>(
          static_cast<const TargetMemory*>(plugin->target_memory()))) {}
...
base::Status ElementCursor::Filter(
  ...
  auto session = *plugin_->session_table().FindById(
      plugin_->chunk_table().FindById(*chunk_id)->session_id());
  ...
  const auto& data = StorageHandle(plugin_).GetChunk(*chunk_id);
...
base::Status ElementCursor::ResetDecoder(
  ...
  const EtmV4Config& config =
      StorageHandle(plugin_).GetEtmV4Config(config_id).etm_v4_config();
