...
void EtmTracker::AddSessionData(tables::EtmV4SessionTable::Id session_id,
  ...
  uint32_t chunk_set_id = EtmPlugin::Get(context_)->chunk_table().row_count();

  for (auto& chunk : chunks) {
    auto chunk_id = EtmPlugin::Get(context_)->mutable_chunk_table()
...
EtmTracker::InsertEtmV4Config(PerCpuConfiguration per_cpu_configs) {
  ...
  uint32_t set_id = EtmPlugin::Get(context_)->configuration_table().row_count();
  for (auto it = per_cpu_configs.GetIterator(); it; ++it) {
    ...
    auto id =
        EtmPlugin::Get(context_)->mutable_configuration_table()->Insert(row).id;
    res.insert(id);
