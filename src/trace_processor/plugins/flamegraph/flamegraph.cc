...
class OneOrSummaryState {
 public:
  void Add(StringPool::Id value, StringPool&) {
    if (!has_value_) {
      representative_ = value;
      has_value_ = true;
      return;
    }
    if (!distinct_values_ && value == representative_) {
      return;
    }
    if (!distinct_values_) {
      distinct_values_ =
          std::make_unique<base::FlatHashMapV2<StringPool::Id, bool>>();
      distinct_values_->Insert(representative_, true);
    }
    distinct_values_->Insert(value, true);
  }

  void Merge(const OneOrSummaryState& other, StringPool& pool) {
    if (!other.has_value_) {
      return;
    }
    Add(other.representative_, pool);
    if (!other.distinct_values_) {
      return;
    }
    for (auto it = other.distinct_values_->GetIterator(); it; ++it) {
      if (it.key() != other.representative_) {
        Add(it.key(), pool);
      }
    }
  }
  ...
  StringPool::Id Output(StringPool& pool) const {
    PERFETTO_DCHECK(has_value_);
    if (!distinct_values_) {
      return representative_;
    }
    std::string summary = pool.Get(representative_).ToStdString();
    summary += " and ";
    summary += std::to_string(distinct_values_->size());
    summary += " others";
    return pool.InternString(base::StringView(summary));
  }

 private:
  StringPool::Id representative_;
  bool has_value_ = false;
  std::unique_ptr<base::FlatHashMapV2<StringPool::Id, bool>> distinct_values_;
};

class ConcatWithCommaState {
 public:
  void Add(StringPool::Id value, StringPool& pool) {
    if (has_value_) {
      value_.push_back(',');
    }
    const auto str = pool.Get(value);
    value_.append(str.data(), str.size());
    has_value_ = true;
  }

  void Merge(const ConcatWithCommaState& other, StringPool&) {
    if (!other.has_value_) {
      return;
    }
    if (has_value_) {
  ...
  StringPool::Id Output(StringPool& pool) const {
    PERFETTO_DCHECK(has_value_);
    return pool.InternString(base::StringView(value_));
  }
...
class FlamegraphBuilder {
 ...
 private:
  ...
  core::Tree::Column StringAggregateStatesToColumn(
      const std::vector<State>& states);

  template <typename State>
  void StringAggregateColumn(const core::Tree::Column& input,
                             AggregateColumns* output);
...
bool IsValidConfig(const Config& config) {
  ...
  for (const Config::AggregateColumn& aggregate : config.aggregate_columns) {
    ...
    if (aggregate.aggregate == Config::Aggregate::kSum) {
      if (!IsNumericColumn(*aggregate.input)) {
        return false;
      }
    } else if (!aggregate.input->type.Is<core::String>()) {
      return false;
    }
  }
...
template <typename State>
core::Tree::Column FlamegraphBuilder::StringAggregateStatesToColumn(
    const std::vector<State>& states) {
  core::Tree::Column column = core::Tree::Column::Create<StringPool::Id>(
      static_cast<uint32_t>(states.size()));
  column.null_bv = core::BitVector::CreateWithSize(states.size(), false);
  core::Span<StringPool::Id> output_values =
      column.unchecked_span<StringPool::Id>();
  for (uint32_t row = 0; row < states.size(); ++row) {
    if (states[row].has_value()) {
      output_values[row] = states[row].Output(config_.pool);
      column.null_bv.set(row);
    }
  }
  return column;
}
...
template <typename State>
void FlamegraphBuilder::StringAggregateColumn(const core::Tree::Column& input,
                                              AggregateColumns* output) {
  std::vector<State> retained(input_.row_count);
  core::Span<const StringPool::Id> values =
      input.unchecked_span<StringPool::Id>();
  for (uint32_t row = 0; row < input_.row_count; ++row) {
    if (input.null_bv.size() > 0 && !input.null_bv.is_set(row)) {
      continue;
    }
    const PathState& state = path_state_[row];
    const uint32_t retained_frame = state.retained_frame;
    if (retained_frame == core::Tree::kNullParent || state.StackIsHidden()) {
      continue;
    }
    retained[retained_frame].Add(values[row], config_.pool);
  }

  std::vector<State> downward(downward_tree_.size());
  for (uint32_t frame = 0; frame < input_.row_count; ++frame) {
    const PathState& state = path_state_[frame];
    if (!state.IsRetained(frame) || !retained[frame].has_value() ||
        !state.ContributesToDownwardCumulative(required_show_stack_bits_)) {
      continue;
    }
    downward[state.downward_node].Merge(retained[frame], config_.pool);
  }

  std::vector<State> upward(upward_tree_.size());
  for (const TreeConstituent& constituent : upward_aggregate_constituents_) {
    if (retained[constituent.frame].has_value()) {
      upward[constituent.node].Merge(retained[constituent.frame], config_.pool);
    }
  }

  output->downward = StringAggregateStatesToColumn(downward);
  output->upward = StringAggregateStatesToColumn(upward);
}
  ...
  for (uint32_t i = 0; i < config_.aggregate_columns.size(); ++i) {
    ...
    switch (aggregate.aggregate) {
      ...
      case Config::Aggregate::kOneOrSummary:
        StringAggregateColumn<OneOrSummaryState>(input, &aggregate_columns_[i]);
        break;
      case Config::Aggregate::kConcatWithComma:
        StringAggregateColumn<ConcatWithCommaState>(input,
                                                    &aggregate_columns_[i]);
        break;
    }
  }
  return base::OkStatus();
