...
struct FlamechartAgg : public sqlite::AggregateFunction<FlamechartAgg> {
  ...
  static void Step(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    ...
    const auto* tree =
        sqlite::value::Pointer<core::Tree>(argv[0], kTreePointerType);
    if (!tree) {
      agg.failed = true;
      return sqlite::result::Error(
          ctx, "flamechart: first argument must be a TREE pointer");
    }
    if (agg.tree && agg.tree != tree) {
      agg.failed = true;
      return sqlite::result::Error(
          ctx, "flamechart: tree must be the same for every row");
    }
    agg.tree = tree;
  ...
  static void Final(sqlite3_context* ctx) {
    ...
    // that error and release the context without dereferencing an invalid tree.
