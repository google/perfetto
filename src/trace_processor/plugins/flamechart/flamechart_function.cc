...
//   0: TREE pointer - the stack tree, from __intrinsic_tree_from_table. Taken
//      on the first row and ignored afterwards.
...
struct FlamechartAgg : public sqlite::AggregateFunction<FlamechartAgg> {
  ...
  static void Step(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    ...
    if (!agg.has_tree) {
      auto tree = sqlite::utils::TakeMovePointerValue<core::Tree>(
          argv[0], kTreePointerType, "flamechart");
      if (!tree.ok()) {
        agg.failed = true;
        return sqlite::utils::SetError(ctx, tree.status());
      }
      agg.tree = std::move(*tree);
      agg.has_tree = true;
    }
  ...
  static void Final(sqlite3_context* ctx) {
    ...
    // SQLite finalizes aggregates even after Step reports an error. Preserve
    // that error.
    if (raw_agg && raw_agg.get()->failed) {
      return;
    }
