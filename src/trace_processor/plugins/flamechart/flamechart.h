...
// does not contribute to sample counts.
//
// The returned dataframe has five columns, in this order:
//   0: ts           - segment start timestamp
//   1: dur          - segment duration, or -1 if still open at the last point
...
// after the last point have duration -1 (incomplete). Zero-duration segments
// from stack transitions at equal timestamps are omitted.
//
// The sweep is a single pass over the points. Each point walks its stack and
// updates counts at every depth, so time is O(sum of sample stack depths).
// Memory is O(maximum stack depth) plus the emitted output.
