...
TEST_F(FlamechartRunsTest, SingleRunMergesAllPointsPerDepth) {
  ...
  EXPECT_EQ(rows[0], (Row{10, -1, 0, 0, 3}));
  EXPECT_EQ(rows[1], (Row{10, -1, 1, 1, 3}));
  EXPECT_EQ(rows[2], (Row{10, -1, 2, 2, 3}));
}
...
TEST_F(FlamechartRunsTest, SharedPrefixMergesAcrossRuns) {
  ...
  // per run: C for [10, 30), D remains incomplete from 30 onward.
  EXPECT_EQ(rows[0], (Row{10, -1, 0, 0, 4}));
  EXPECT_EQ(rows[1], (Row{10, -1, 1, 1, 4}));
  EXPECT_EQ(rows[2], (Row{10, 20, 2, 2, 2}));
  EXPECT_EQ(rows[3], (Row{30, -1, 2, 3, 2}));
}
...
TEST_F(FlamechartRunsTest, DisjointStacksOpenFreshSegments) {
  ...
  EXPECT_EQ(rows[1], (Row{20, -1, 0, 3, 1}));
  EXPECT_EQ(rows[2], (Row{10, 10, 1, 1, 1}));
  EXPECT_EQ(rows[3], (Row{20, -1, 1, 4, 1}));
...
TEST_F(FlamechartRunsTest, DepthChangeClosesAndOpensTail) {
  ...
  const auto rows = ReadRuns(*result);
  ASSERT_EQ(rows.size(), 4u);
  EXPECT_EQ(rows[0], (Row{10, -1, 0, 0, 2}));
  EXPECT_EQ(rows[1], (Row{10, 10, 1, 1, 1}));
  EXPECT_EQ(rows[2], (Row{20, -1, 1, 2, 1}));
  EXPECT_EQ(rows[3], (Row{20, -1, 2, 3, 1}));
}
...
TEST_F(FlamechartRunsTest, LooksUpOriginalIds) {
  ...
  const auto rows = ReadRuns(*result);
  ASSERT_EQ(rows.size(), 3u);
  EXPECT_EQ(rows[0], (Row{10, -1, 0, 100, 2}));
  EXPECT_EQ(rows[1], (Row{10, -1, 1, 101, 2}));
  EXPECT_EQ(rows[2], (Row{10, -1, 2, 102, 2}));
}
...
TEST_F(FlamechartRunsTest, SkipsUnresolvableLeaves) {
  ...
  ASSERT_EQ(rows.size(), 3u);
  EXPECT_EQ(rows[0], (Row{10, -1, 0, 0, 2}));
  EXPECT_EQ(rows[1], (Row{10, -1, 1, 1, 2}));
  EXPECT_EQ(rows[2], (Row{10, -1, 2, 2, 2}));
}

TEST_F(FlamechartRunsTest, SingleSampleIsIncompleteAtEveryDepth) {
  core::Tree tree = MakeTree({{std::nullopt, {}}, {{0}, {}}});
  auto result = Build(tree, {10}, {1});
  ASSERT_TRUE(result.ok()) << result.status().message();
  EXPECT_EQ(ReadRuns(*result),
            (std::vector<Row>{{10, -1, 0, 0, 1}, {10, -1, 1, 1, 1}}));
}

TEST_F(FlamechartRunsTest, EqualTimestampsOmitZeroDurationFrames) {
  core::Tree tree = MakeTree({{std::nullopt, {}}, {{0}, {}}, {{0}, {}}});
  auto result = Build(tree, {10, 10}, {1, 2});
  ASSERT_TRUE(result.ok()) << result.status().message();
  EXPECT_EQ(ReadRuns(*result),
            (std::vector<Row>{{10, -1, 0, 0, 2}, {10, -1, 1, 2, 1}}));
}

TEST_F(FlamechartRunsTest, EqualTimestampsOmitTransientStackAtEveryDepth) {
  core::Tree tree = MakeTree({{std::nullopt, {}},
                              {{0}, {}},
                              {std::nullopt, {}},
                              {{2}, {}},
                              {std::nullopt, {}},
                              {{4}, {}}});
  auto result = Build(tree, {10, 20, 20, 30}, {1, 3, 5, 1});
  ASSERT_TRUE(result.ok()) << result.status().message();
  EXPECT_EQ(ReadRuns(*result), (std::vector<Row>{{10, 10, 0, 0, 1},
                                                 {20, 10, 0, 4, 1},
                                                 {30, -1, 0, 0, 1},
                                                 {10, 10, 1, 1, 1},
                                                 {20, 10, 1, 5, 1},
...
TEST_F(FlamechartRunsTest, TrailingUnresolvableSampleLeavesFramesIncomplete) {
  core::Tree tree = MakeTree({{std::nullopt, {}}, {{0}, {}}});
  auto result = Build(tree, {10, 20}, {1, 42});
  ASSERT_TRUE(result.ok()) << result.status().message();
  EXPECT_EQ(ReadRuns(*result),
            (std::vector<Row>{{10, -1, 0, 0, 1}, {10, -1, 1, 1, 1}}));
}
