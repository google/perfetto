...
base::StatusOr<dataframe::Dataframe> Build(const core::Tree& tree,
  ...
  const auto emit = [&](const OpenSegment& seg, size_t depth, int64_t dur) {
    // Equal-timestamp stack transitions can close a frame immediately.
    if (dur == 0) {
      return;
    }
    builder.PushNonNull(0, seg.start);
    builder.PushNonNull(1, dur);
  ...
  for (size_t i = 0; i < ts.size(); ++i) {
    ...
    // segments are kept as-is, without increasing their sample counts.
    ...
    for (size_t d = old_depth; d > common; --d) {
      emit(open[d - 1], d - 1, ts[i] - open[d - 1].start);
    }
    open.resize(common);
  ...
  // No later observation closes these frames: emit them as incomplete.
