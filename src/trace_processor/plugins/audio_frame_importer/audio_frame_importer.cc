...
namespace perfetto::trace_processor::audio_frame_importer {
namespace {

// Returns the raw encoded bytes (au_data, or codec_config) for one
// __intrinsic_audio_frames row. The bytes are a zero-copy view into the
// original trace blob, kept alive by the owning vector for the session, so
// SQLite is given a static pointer.
struct AudioFrameAuData : public sqlite::Function<AudioFrameAuData> {
  static constexpr char kName[] = "__intrinsic_audio_frame_au_data";
...
// The plugin owns the __intrinsic_audio_frames table and the parallel au_data
// payload vector (indexed by row id): both are populated by AudioFrameModule
// during parsing and live for the whole session.
class AudioFrameImporter : public Plugin<AudioFrameImporter> {
 public:
  ~AudioFrameImporter() override;

  void RegisterDataframes(std::vector<PluginDataframe>& out) override {
    EnsureTable();
    out.push_back(
        {&table_->dataframe(), tables::AndroidAudioFramesTable::Name(), {}});
  }
  ...
  void RegisterProtoImporterModules(
    ...
    EnsureTable();
    module_context->modules.emplace_back(new AudioFrameModule(
  ...
  void RegisterFunctions(PerfettoSqlConnection*,
    ...
    out.push_back(MakeFunctionRegistration<AudioFrameAuData>(&au_data_));
  }
...
AudioFrameImporter::~AudioFrameImporter() = default;
...
void RegisterPlugin() {
  static PluginRegistration reg(
      []() -> std::unique_ptr<PluginBase> {
        return std::make_unique<AudioFrameImporter>();
      },
      AudioFrameImporter::kPluginId, AudioFrameImporter::kDepIds.data(),
      AudioFrameImporter::kDepIds.size());
