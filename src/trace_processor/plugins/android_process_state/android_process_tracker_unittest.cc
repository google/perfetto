...
class AndroidProcessTrackerTest : public ::testing::Test {
 public:
  AndroidProcessTrackerTest() {
    ...
    android_process_tracker = std::make_unique<AndroidProcessTracker>(&context);
  }
...
TEST_F(AndroidProcessTrackerTest, SameSeqIdReturnsSameProcess) {
  ...
  EXPECT_EQ(android_process_tracker->GetStartSeqId(upid), 10);

  UniquePid again = android_process_tracker->GetOrStartProcess(
      1000, 1234, 10, name, ThreadNamePriority::kTrackDescriptor);
...
TEST_F(AndroidProcessTrackerTest, FindProcessLocatesRecycledIncarnation) {
  ...
  EXPECT_EQ(android_process_tracker->FindProcess(1234, 1), upid_a);
  EXPECT_EQ(android_process_tracker->FindProcess(1234, 2), upid_b);
  EXPECT_EQ(android_process_tracker->FindProcess(1234, 3), std::nullopt);
}
...
TEST_F(AndroidProcessTrackerTest, AdoptsLiveProcessWithNoSeqId) {
  ...
  EXPECT_EQ(android_process_tracker->GetStartSeqId(upid), 7);
}
...
TEST_F(AndroidProcessTrackerTest, MissingSeqIdKeepsLiveProcess) {
  ...
  UniquePid same = android_process_tracker->GetOrStartProcess(
