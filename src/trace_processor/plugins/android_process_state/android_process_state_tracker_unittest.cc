...
class AndroidProcessStateTrackerTest : public ::testing::Test {
 public:
  AndroidProcessStateTrackerTest() {
    ...
    android_process_tracker_ =
        std::make_unique<AndroidProcessTracker>(&context_);
    ...
    tracker_ = std::make_unique<AndroidProcessStateTracker>(
        &context_, android_process_tracker_.get(), process_state_table_.get(),
...
TEST_F(AndroidProcessStateTrackerTest, CoexistenceDefaultsToKernelAuthority) {
  ...
  EXPECT_FALSE(android_process_tracker_->FrameworkIsProcessAuthority());
}
...
TEST_F(AndroidProcessStateTrackerTest, UserspaceAuthorityMode) {
  ...
  tracker_->OnConfigDetected(/*ftrace_configured=*/false,
                             /*dump_process_metadata=*/true);
  EXPECT_TRUE(android_process_tracker_->FrameworkIsProcessAuthority());
}
...
TEST_F(AndroidProcessStateTrackerTest, SystemServerZeroSuppressionHandling) {
  ...
  tracker_->ParseProcessStateDump(protozero::ConstBytes{
  ...
  EXPECT_FALSE(android_process_tracker_->GetStartSeqId(*opt_upid).has_value());
...
TEST_F(AndroidProcessStateTrackerTest, ReconcileUserspaceAuthority) {
  ...
  EXPECT_EQ(android_process_tracker_->GetStartSeqId(*opt_upid), 100);
...
TEST_F(AndroidProcessStateTrackerTest, ReconcileUserspacePidRecycling) {
  ...
  UniquePid old_upid = android_process_tracker_->GetOrStartProcess(
  ...
  EXPECT_EQ(context_.storage->GetString(*new_process.name()),
            "com.test.newapp");
  EXPECT_EQ(android_process_tracker_->GetStartSeqId(*opt_upid), 20);
...
TEST_F(AndroidProcessStateTrackerTest, RecyclingObservedMidTraceThenSnapshot) {
  ...
  UniquePid upid_a = android_process_tracker_->GetOrStartProcess(
  ...
  UniquePid upid_b = android_process_tracker_->GetOrStartProcess(
...
TEST_F(AndroidProcessStateTrackerTest, InvalidStartTimeMsOverflow) {
  ...
  // The record is dropped rather than producing a nonsensical start_ts.
  auto opt_upid = context_.process_tracker->GetProcessOrNull(2001);
  ASSERT_TRUE(opt_upid.has_value());
  EXPECT_FALSE(
      context_.storage->process_table()[*opt_upid].start_ts().has_value());
}
...
TEST_F(AndroidProcessStateTrackerTest, ReconcileKernelAuthority) {
  ...
  context_.process_tracker->GetOrCreateProcess(3001);
...
TEST_F(AndroidProcessStateTrackerTest,
       FreezerDumpBeforeProcessStateDumpResolvesUpid) {
  tracker_->OnConfigDetected(/*ftrace_configured=*/false,
                             /*dump_process_metadata=*/true);

  // Freezer dump arrives BEFORE the process state dump for a process (PID 4001)
  // that was never seen mid-trace.
  protozero::HeapBuffered<fb::AndroidFreezerStateSnapshot> freezer_snapshot;
  auto* freezer_rec = freezer_snapshot->add_record();
  freezer_rec->set_pid(4001);
  freezer_rec->set_uid(10080);
  std::string freezer_bytes = freezer_snapshot.SerializeAsString();
  tracker_->ParseFreezerDump(protozero::ConstBytes{
      reinterpret_cast<const uint8_t*>(freezer_bytes.data()),
      freezer_bytes.size()});

  protozero::HeapBuffered<fb::AndroidProcessStateSnapshot> proc_snapshot;
  auto* proc_rec = proc_snapshot->add_record();
  proc_rec->set_pid(4001);
  proc_rec->set_uid(10080);
  proc_rec->set_process_name("com.test.freezer_first");
  proc_rec->set_start_seq_id(55);
  std::string proc_bytes = proc_snapshot.SerializeAsString();
  tracker_->ParseProcessStateDump(protozero::ConstBytes{
      reinterpret_cast<const uint8_t*>(proc_bytes.data()), proc_bytes.size()});

  tracker_->Finalize();

  auto opt_upid = context_.process_tracker->GetProcessOrNull(4001);
  ASSERT_TRUE(opt_upid.has_value());
  EXPECT_EQ(freezer_state_table_->row_count(), 1u);
  EXPECT_EQ((*freezer_state_table_)[0].upid(), *opt_upid);
  EXPECT_EQ((*freezer_state_table_)[0].is_initial(), 1u);
}
