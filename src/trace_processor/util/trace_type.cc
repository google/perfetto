...
TraceType GuessTraceType(const uint8_t* data, size_t size) {
  ...
  // A perfetto_manifest sidecar file: a JSON object whose only top-level key
  // is "perfetto_manifest". It overrides clock/machine handling for the
  // trace files that follow it.
  if (base::StartsWith(start_minus_white_space, "{\"perfetto_manifest\""))
    return kPerfettoManifestTraceType;
