...
TraceType GuessTraceType(const uint8_t* data, size_t size) {
  ...
  // A perfetto_metadata sidecar file: a JSON object whose only top-level key
  // is "perfetto_metadata". It overrides clock/machine handling for the other
  // files in the archive containing it.
  if (base::StartsWith(start_minus_white_space, "{\"perfetto_metadata\""))
    return kPerfettoMetadataTraceType;
