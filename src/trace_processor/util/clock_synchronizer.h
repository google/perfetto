...
class ClockSynchronizerBase {
 public:
  ...
  struct ClockId {
    ...
    ClockId() = default;
    ClockId(int64_t cid) : clock_id(static_cast<uint32_t>(cid)) {}  // NOLINT
    ClockId(uint32_t cid, uint32_t sid, uint32_t tfi)
 ...
 public:
  ...
  base::StatusOr<uint32_t> AddSnapshot(
    ...
    for (const auto& clock_ts : clock_timestamps) {
      ...
      if (domain.snapshots.empty()) {
        if (clock_ts.clock.is_incremental &&
          ...
          return base::ErrStatus(
              "Clock sync error: the global clock with id=%" PRIu32
        ...
        clock_event_listener_->OnInvalidClockSnapshot();
        return base::ErrStatus(
            "Clock sync error: the clock domain with id=%" PRIu32
            " (unit=%" PRId64
            ", incremental=%d), was previously registered with "
            "different properties (unit=%" PRId64 ", incremental=%d).",
            clock_id.clock_id, clock_ts.clock.unit_multiplier_ns,
      ...
      if (PERFETTO_UNLIKELY(clock_id == trace_time_clock_id_ &&
        ...
        clock_event_listener_->OnInvalidClockSnapshot();
        return base::ErrStatus(
            "Clock sync error: the trace clock (id=%" PRIu32
      ...
      if (!vect.snapshot_ids.empty() &&
        ...
        clock_event_listener_->OnInvalidClockSnapshot();
        return base::ErrStatus(
            "Clock sync error: duplicate clock domain with id=%" PRIu32
            " at snapshot %" PRIu32 ".",
            clock_id.clock_id, snapshot_id);
      }
      ...
      if (!vect.timestamps_ns.empty() &&
        ...
        if (clock_id == trace_time_clock_id_) {
          ...
          return base::ErrStatus(
              ...
              " not >= %" PRId64 ".",
              clock_id.clock_id, snapshot_id, timestamp_ns,
              vect.timestamps_ns.back());
        }

        PERFETTO_DLOG("Detected non-monotonic clock with ID %" PRIu32,
                      clock_id.clock_id);
        ...
        auto begin = graph_.lower_bound(ClockGraphEdge{clock_id, {}, 0});
        ClockId upper = {clock_id.clock_id, clock_id.seq_id,
                         clock_id.trace_file_id + 1};
        auto end = graph_.lower_bound(ClockGraphEdge{upper, {}, 0});
  ...
  base::Status SetTraceTimeClock(ClockId clock_id) {
    ...
    if (trace_time_clock_id_used_for_conversion_ &&
      ...
      return base::ErrStatus(
          "Not updating trace time clock from %" PRIu32 " to %" PRIu32
          " because the old clock was already used for timestamp "
          "conversion - ClockSnapshot too late in trace?",
          trace_time_clock_id_.clock_id, clock_id.clock_id);
    }
    trace_time_clock_id_ = clock_id;
