...
HEAP_GRAPH_OBJECT_DATA_TABLE = Table(
    ...
    tabledoc=TableDoc(
        doc='''
          ...
          Contains decoded field values, string content, and primitive
          array blob references. Only populated for HPROF (ART) heap
          dumps, not for proto heap graphs.
        ''',
        group='ART Heap Graphs',
        columns={
            'object_id':
            ...
            'field_set_id':
                '''join key with heap_graph_object_field containing
                primitive field values for this object.''',
            'value_string':
                '''decoded string value for java.lang.String instances,
                or text representation for primitive arrays.''',
            'array_element_type':
...
HEAP_GRAPH_OBJECT_FIELD_TABLE = Table(
    python_module=__file__,
    class_name='HeapGraphObjectFieldTable',
    sql_name='__intrinsic_heap_graph_object_field',
    columns=[
        C(
            'field_set_id',
            CppUint32(),
    ...
    tabledoc=TableDoc(
        doc='''
          ...
          primitive field values (for instances) or array elements
        ...
        columns={
            ...
            'field_name':
                '''The field name. E.g. Foo.count, or [0] for array
                elements.''',
            'field_type':
