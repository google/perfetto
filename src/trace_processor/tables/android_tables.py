...
ANDROID_VIDEO_FRAMES_TABLE = Table(
    ...
    tabledoc=TableDoc(
        doc='''
          Video frames captured from the device display. The encoded payload
          (an H.264/HEVC access unit, or the decoder codec_config) is a
          zero-copy TraceBlobView held in
          TraceStorage::video_frame_au_data(); fetch the bytes with
          __INTRINSIC_VIDEO_FRAME_AU_DATA(id) which returns a BLOB. See the
          dev.perfetto.VideoFrames plugin.
        ''',
        group='Android',
        columns={
