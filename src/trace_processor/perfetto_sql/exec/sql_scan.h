...
class SqlScan : public core::exec::Source {
 public:
  // Without a catalog no column can be traced back, so all of them are
  // variants.
  static base::StatusOr<std::unique_ptr<SqlScan>> Create(
      SqliteConnection*,
      SqlSource,
      StringPool*,
      const lineage::Catalog* = nullptr);
  ~SqlScan() override;
 ...
 private:
  // One column's values. Only the buffer matching the column's type is ever
  // filled.
  struct Column {
    std::vector<core::exec::Variant> variants;
