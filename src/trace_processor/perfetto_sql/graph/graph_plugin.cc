...
struct GraphBuild : public sqlite::Function<GraphBuild> {
  ...
  PERFETTO_TEMPLATED_USED static void Step(sqlite3_context* ctx,
    ...
    // Copy node data.
    graph->data->nodes.node_ids = nodes->node_ids;
    graph->data->nodes.source_indices = nodes->source_indices;
    graph->data->nodes.passthrough_columns = nodes->passthrough_columns;
    graph->data->nodes.id_to_index = nodes->id_to_index.Copy();
...
// =============================================================================
// __intrinsic_graph_to_tree_agg(graph, root_id, mode) -> TREE
// =============================================================================

struct GraphToTreeAggContext {
  Graph* graph = nullptr;
  std::vector<int64_t> root_ids;
  GraphTraversalMode mode = GraphTraversalMode::kBfs;
};

struct GraphToTreeAgg : public sqlite::AggregateFunction<GraphToTreeAgg> {
  static constexpr char kName[] = "__intrinsic_graph_to_tree_agg";
  ...
  PERFETTO_TEMPLATED_USED static void Step(sqlite3_context* ctx,
    ...
    auto& agg_ctx = sqlite::AggregateContext<
        GraphToTreeAggContext>::GetOrCreateContextForStep(ctx);

    // First row: get graph and mode.
    if (!agg_ctx.graph) {
      SQLITE_ASSIGN_OR_RETURN(ctx, agg_ctx.graph,
                              ExpectPointer<Graph>(argv[0], kName));
      SQLITE_RETURN_IF_ERROR(
          ctx, sqlite::utils::ExpectArgType(argv[2], sqlite::Type::kText, kName,
                                            "mode"));
      std::string mode_str = sqlite::value::Text(argv[2]);
      if (mode_str == "BFS") {
        agg_ctx.mode = GraphTraversalMode::kBfs;
      } else if (mode_str == "DFS") {
        agg_ctx.mode = GraphTraversalMode::kDfs;
      } else {
        return sqlite::utils::SetError(
            ctx, "__intrinsic_graph_to_tree_agg: mode must be BFS or DFS");
      }
    }

    // Collect root IDs.
    if (sqlite::value::Type(argv[1]) == sqlite::Type::kInteger) {
      agg_ctx.root_ids.push_back(sqlite::value::Int64(argv[1]));
    }
  }

  PERFETTO_TEMPLATED_USED static void Final(sqlite3_context* ctx) {
    auto agg_ctx = sqlite::AggregateContext<
        GraphToTreeAggContext>::GetContextOrNullForFinal(ctx);

    if (!agg_ctx || !agg_ctx.get()->graph) {
      return sqlite::result::Null(ctx);
    }

    auto* agg = agg_ctx.get();

    Graph* graph = agg->graph;
    ...
    for (int64_t root_id : agg->root_ids) {
    ...
    SQLITE_ASSIGN_OR_RETURN(
        ctx, auto result,
        GraphToTree(data.nodes, working_edges, root_indices, agg->mode));
...
base::Status GraphPlugin::Register(PluginContext& ctx) {
  ...
  // Graph to tree conversion aggregate.
  RETURN_IF_ERROR(ctx.RegisterAggregateFunction<GraphToTreeAgg>(nullptr));
