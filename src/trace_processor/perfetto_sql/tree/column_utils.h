...
// Returns error on type mismatch or unsupported blob type.
inline base::Status PushSqliteValueToColumn(PassthroughColumn& col,
                                            sqlite3_value* value,
                                            StringPool* pool) {
  switch (sqlite::value::Type(value)) {
    ...
    case sqlite::Type::kInteger: {
      ...
      if (!col.IsInt64()) {
        return base::ErrStatus("Column '%s' type mismatch: expected int64",
                               col.name.c_str());
      }
    ...
    case sqlite::Type::kFloat: {
      ...
      if (!col.IsDouble()) {
        return base::ErrStatus("Column '%s' type mismatch: expected double",
                               col.name.c_str());
      }
    ...
    case sqlite::Type::kText: {
      ...
      if (!col.IsString()) {
        return base::ErrStatus("Column '%s' type mismatch: expected string",
                               col.name.c_str());
      }
    ...
    case sqlite::Type::kBlob:
      return base::ErrStatus("Column '%s': blob values not supported",
                             col.name.c_str());
  }
