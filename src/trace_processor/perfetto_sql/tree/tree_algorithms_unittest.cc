...
// Helper to call MergeSiblings with a single int64_t key column.
base::StatusOr<MergeSiblingsResult> MergeSiblingsInt64(
    const std::vector<uint32_t>& parent_indices,
    const std::vector<int64_t>& keys,
    const std::vector<int64_t>& order,
    TreeMergeMode mode) {
  PassthroughColumn key_col("key", keys);
  std::vector<const PassthroughColumn*> key_cols = {&key_col};
  return MergeSiblings(parent_indices, key_cols, order, mode);
}

// Helper to call MergeSiblings with a single StringPool::Id key column.
base::StatusOr<MergeSiblingsResult> MergeSiblingsString(
    const std::vector<uint32_t>& parent_indices,
    const std::vector<StringPool::Id>& keys,
    const std::vector<int64_t>& order,
    TreeMergeMode mode) {
  PassthroughColumn key_col("key", keys);
  std::vector<const PassthroughColumn*> key_cols = {&key_col};
...
TEST(TreeAlgorithmsTest, MergeSiblings_NoMerge) {
  ...
  auto result =
      MergeSiblingsInt64(parent_indices, keys, order, TreeMergeMode::kConsecutive);
  ASSERT_TRUE(result.ok()) << result.status().c_message();
...
TEST(TreeAlgorithmsTest, MergeSiblings_GlobalMergeNonAdjacent) {
  ...
  auto result =
      MergeSiblingsInt64(parent_indices, keys, order, TreeMergeMode::kGlobal);
  ASSERT_TRUE(result.ok()) << result.status().c_message();
...
TEST(TreeAlgorithmsTest, MergeSiblings_StringPoolIdKeys) {
  ...
  auto result =
      MergeSiblingsString(parent_indices, keys, order, TreeMergeMode::kConsecutive);
  ASSERT_TRUE(result.ok()) << result.status().c_message();
...
TEST(TreeAlgorithmsTest, Collapse_Empty) {
  std::vector<uint32_t> parent_indices;
  std::vector<int64_t> keys;

  auto result = Collapse(parent_indices, keys);
  ASSERT_TRUE(result.ok()) << result.status().c_message();
  EXPECT_THAT(result->collapsed_sources, IsEmpty());
  EXPECT_THAT(result->new_parent_indices, IsEmpty());
  EXPECT_THAT(result->old_to_new, IsEmpty());
}

TEST(TreeAlgorithmsTest, Collapse_SingleNode) {
  std::vector<uint32_t> parent_indices = {kNullUint32};
  std::vector<int64_t> keys = {42};
  ...
  // Single root node stays as is
  EXPECT_EQ(result->collapsed_sources.size(), 1u);
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0));
  EXPECT_EQ(result->new_parent_indices[0], kNullUint32);
  EXPECT_EQ(result->old_to_new[0], 0u);
}

TEST(TreeAlgorithmsTest, Collapse_NoCollapse_DifferentKeys) {
  // Tree: 0 -> 1 -> 2, all different keys
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 1};
  std::vector<int64_t> keys = {1, 2, 3};
  ...
  // No collapse - all 3 nodes survive
  EXPECT_EQ(result->collapsed_sources.size(), 3u);
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0));
  EXPECT_THAT(result->collapsed_sources[1], ElementsAre(1));
  EXPECT_THAT(result->collapsed_sources[2], ElementsAre(2));
  EXPECT_THAT(result->old_to_new, ElementsAre(0, 1, 2));
  EXPECT_THAT(result->new_parent_indices,
              ElementsAre(kNullUint32, 0u, 1u));
}

TEST(TreeAlgorithmsTest, Collapse_CollapseChildIntoParent) {
  // Tree: 0 -> 1, both with same key
  // After collapse: only node 0 survives (node 1 collapsed into it)
  std::vector<uint32_t> parent_indices = {kNullUint32, 0};
  std::vector<int64_t> keys = {42, 42};  // Same key!

  auto result = Collapse(parent_indices, keys);
  ASSERT_TRUE(result.ok()) << result.status().c_message();

  // Node 1 collapsed into node 0
  EXPECT_EQ(result->collapsed_sources.size(), 1u);
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0, 1));
  EXPECT_EQ(result->new_parent_indices[0], kNullUint32);
  EXPECT_EQ(result->old_to_new[0], 0u);
  EXPECT_EQ(result->old_to_new[1], 0u);  // Both map to same output
}

TEST(TreeAlgorithmsTest, Collapse_CollapseChain) {
  // Tree: 0 -> 1 -> 2 -> 3, all same key
  // After collapse: only node 0 survives
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 1, 2};
  std::vector<int64_t> keys = {1, 1, 1, 1};  // All same

  auto result = Collapse(parent_indices, keys);
  ASSERT_TRUE(result.ok()) << result.status().c_message();

  // All nodes collapse into node 0
  EXPECT_EQ(result->collapsed_sources.size(), 1u);
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0, 1, 2, 3));
  EXPECT_THAT(result->old_to_new, ElementsAre(0, 0, 0, 0));
}

TEST(TreeAlgorithmsTest, Collapse_PartialChain) {
  // Tree: 0 -> 1 -> 2 -> 3
  // Keys:  A    A    B    B
  // After collapse: 0 (contains 0,1) -> 2 (contains 2,3)
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 1, 2};
  std::vector<int64_t> keys = {1, 1, 2, 2};

  auto result = Collapse(parent_indices, keys);
  ASSERT_TRUE(result.ok()) << result.status().c_message();

  // Two output nodes
  EXPECT_EQ(result->collapsed_sources.size(), 2u);
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0, 1));
  EXPECT_THAT(result->collapsed_sources[1], ElementsAre(2, 3));
  EXPECT_EQ(result->new_parent_indices[0], kNullUint32);
  EXPECT_EQ(result->new_parent_indices[1], 0u);  // Child of collapsed 0
  EXPECT_THAT(result->old_to_new, ElementsAre(0, 0, 1, 1));
}

TEST(TreeAlgorithmsTest, Collapse_BranchingTree_NoCollapse) {
  // Tree:
  //   0 (key=A)
  //   ├── 1 (key=B)
  //   └── 2 (key=C)
  // No collapse since all keys differ from parent
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 0};
  std::vector<int64_t> keys = {1, 2, 3};
...
TEST(TreeAlgorithmsTest, Collapse_BranchingTree_CollapseOneChild) {
  // Tree:
  //   0 (key=A)
  //   ├── 1 (key=A) <- Collapses into 0
  //   └── 2 (key=B)
  // After: 0 (contains 0,1), 2
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 0};
  std::vector<int64_t> keys = {1, 1, 2};
  ...
  EXPECT_EQ(result->collapsed_sources.size(), 2u);
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0, 1));
  EXPECT_THAT(result->collapsed_sources[1], ElementsAre(2));
  EXPECT_EQ(result->old_to_new[0], 0u);
  EXPECT_EQ(result->old_to_new[1], 0u);
  EXPECT_EQ(result->old_to_new[2], 1u);
}

TEST(TreeAlgorithmsTest, Collapse_ReparentGrandchildren) {
  // Tree:
  //   0 (key=A)
  //   └── 1 (key=A) <- Collapses into 0
  //       └── 2 (key=B) <- Should become child of 0
  // After: 0 (contains 0,1) -> 2
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 1};
  std::vector<int64_t> keys = {1, 1, 2};
  ...
  EXPECT_THAT(result->collapsed_sources[1], ElementsAre(2));
  EXPECT_EQ(result->new_parent_indices[0], kNullUint32);
  EXPECT_EQ(result->new_parent_indices[1], 0u);  // 2's parent is now 0
}

TEST(TreeAlgorithmsTest, Collapse_MultipleRoots) {
  // Two separate trees:
  // Tree 1: 0 -> 1 (same key)
  // Tree 2: 2 -> 3 (different keys)
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, kNullUint32, 2};
  std::vector<int64_t> keys = {1, 1, 2, 3};

  auto result = Collapse(parent_indices, keys);
  ASSERT_TRUE(result.ok()) << result.status().c_message();

  // Tree 1: collapses to 1 node
  // Tree 2: stays as 2 nodes
  EXPECT_EQ(result->collapsed_sources.size(), 3u);
}

TEST(TreeAlgorithmsTest, Collapse_StringPoolIdKeys) {
  StringPool pool;
  StringPool::Id a = pool.InternString(base::StringView("a"));
  StringPool::Id b = pool.InternString(base::StringView("b"));
  ...
  // Keys:  a    a    b
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 1};
  std::vector<StringPool::Id> keys = {a, a, b};

  auto result = Collapse(parent_indices, keys);
  ASSERT_TRUE(result.ok()) << result.status().c_message();

  // Nodes 0 and 1 collapse
  EXPECT_EQ(result->collapsed_sources.size(), 2u);
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0, 1));
  EXPECT_THAT(result->collapsed_sources[1], ElementsAre(2));
}

TEST(TreeAlgorithmsTest, Collapse_ComplexTree) {
  // Tree:
  //       0 (key=A)
  //      / \
  //     1   2 (key=A, key=B)
  ...
  //     3   4 (key=A, key=B)
  // Collapse:
  // - Node 1 (key=A) collapses into 0
  // - Node 3 (key=A) collapses into 0 (via 1)
  // - Node 2 (key=B) stays
  // - Node 4 (key=B) collapses into 2
  // Result: 0 (sources: 0,1,3), 2 (sources: 2,4)
  std::vector<uint32_t> parent_indices = {kNullUint32, 0, 0, 1, 2};
  std::vector<int64_t> keys = {1, 1, 2, 1, 2};

  auto result = Collapse(parent_indices, keys);
  ASSERT_TRUE(result.ok()) << result.status().c_message();

  EXPECT_EQ(result->collapsed_sources.size(), 2u);
  // Node 0,1,3 all have key=1 and form a chain
  EXPECT_THAT(result->collapsed_sources[0], ElementsAre(0, 1, 3));
  // Node 2,4 both have key=2
  EXPECT_THAT(result->collapsed_sources[1], ElementsAre(2, 4));

  // Check parent relationship
  EXPECT_EQ(result->new_parent_indices[0], kNullUint32);  // Root
  EXPECT_EQ(result->new_parent_indices[1], 0u);  // Child of root

  // Check old_to_new mapping
  EXPECT_EQ(result->old_to_new[0], 0u);
  EXPECT_EQ(result->old_to_new[1], 0u);
  EXPECT_EQ(result->old_to_new[2], 1u);
  EXPECT_EQ(result->old_to_new[3], 0u);
  EXPECT_EQ(result->old_to_new[4], 1u);
}
