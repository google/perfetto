...
// Merges sibling nodes that share the same composite key value.
//
// Args:
//   parent_indices: For each row, the parent's row index (kNullUint32 for
//   roots)
//   key_columns: Pointers to passthrough columns used for grouping siblings.
//                Siblings are merged if ALL key columns match.
...
// Result of collapsing parent-child chains with same key.
struct CollapseResult {
  // For each output row, the source row indices that were collapsed into it.
  CsrVector<uint32_t> collapsed_sources;
  // New parent indices after collapsing (kNullUint32 for roots).
  std::vector<uint32_t> new_parent_indices;
  // Mapping from old row index to new row index (kNullUint32 if collapsed).
  std::vector<uint32_t> old_to_new;
};

// Collapses parent-child chains where both nodes have the same key.
//
// When a node has the same key as its parent, it is merged into the parent:
// - The node's values are aggregated into the parent
// - The node's children are reparented to the parent
// - The node is removed from the tree
//
// This operation is applied recursively: if A->B->C all have the same key,
// B and C are both collapsed into A.
//
// Args:
//   parent_indices: For each row, the parent's row index (kNullUint32 for roots)
//   key_values: The key column values for comparison
//
// Returns mapping information for applying aggregations.
...
// Explicit instantiation declarations
extern template base::StatusOr<CollapseResult> Collapse<int64_t>(
    const std::vector<uint32_t>& parent_indices,
    const std::vector<int64_t>& key_values);

extern template base::StatusOr<CollapseResult> Collapse<StringPool::Id>(
    const std::vector<uint32_t>& parent_indices,
    const std::vector<StringPool::Id>& key_values);
