...
// Finalizes tree data after an aggregation operation (merge/invert).
// This handles the common pattern of:
// 1. Updating parent_indices with new structure
// 2. Aggregating passthrough columns from merged sources
// 3. Nulling out original ID columns (meaningless after merge/invert)
// 4. Resetting source_indices to identity (since columns are now materialized)
void FinalizeAggregation(TreeData& data,
                         std::vector<uint32_t> new_parent_indices,
                         const std::vector<PassthroughColumn>& materialized,
                         const CsrVector<uint32_t>& merged_sources,
                         const std::vector<TreeAggSpec>& agg_specs) {
  auto new_count = merged_sources.size();
  data.parent_indices = std::move(new_parent_indices);
  data.passthrough_columns =
      AggregatePassthroughColumns(materialized, merged_sources, agg_specs);

  for (auto& col : data.passthrough_columns) {
    if (col.name == Tree::kOriginalIdCol ||
        col.name == Tree::kOriginalParentIdCol) {
      col.data = std::vector<int64_t>(new_count, kNullInt64);
    }
  }

  data.source_indices.resize(new_count);
  std::iota(data.source_indices.begin(), data.source_indices.end(), 0u);
}

// =============================================================================
// __intrinsic_tree_keys(column_names STRING...) -> TREE_KEYS pointer
// Variadic function that accepts multiple column names for composite keys.
// =============================================================================
struct TreeKeys : public sqlite::Function<TreeKeys> {
  static constexpr char kName[] = "__intrinsic_tree_keys";
  ...
  PERFETTO_TEMPLATED_USED static void Step(sqlite3_context* ctx,
    ...
    SQLITE_RETURN_IF_ERROR(ctx,
                           sqlite::utils::CheckArgCountAtLeast(kName, argc, 1));
    std::vector<std::string> column_names;
    column_names.reserve(static_cast<size_t>(argc));
    for (int i = 0; i < argc; ++i) {
      SQLITE_RETURN_IF_ERROR(
          ctx, sqlite::utils::ExpectArgType(argv[i], sqlite::Type::kText, kName,
                                            "column_name"));
      column_names.push_back(sqlite::value::Text(argv[i]));
    }
    return MakeUniquePtrResult<TreeKeysSpec>(ctx, std::move(column_names));
  }
};
...
struct TreeInvert : public sqlite::Function<TreeInvert> {
  ...
  PERFETTO_TEMPLATED_USED static void Step(sqlite3_context* ctx,
    ...
    SQLITE_ASSIGN_OR_RETURN(ctx, auto* tree,
                            ExpectPointer<Tree>(argv[0], kName));
    SQLITE_ASSIGN_OR_RETURN(ctx, auto* keys,
                            ExpectPointer<TreeKeysSpec>(argv[1], kName));
    if (keys->column_names.size() != 1) {
      return sqlite::utils::SetError(
          ctx, "tree_invert: expected exactly one key column");
    }
...
struct TreeMergeSiblings : public sqlite::Function<TreeMergeSiblings> {
  ...
  PERFETTO_TEMPLATED_USED static void Step(sqlite3_context* ctx,
    ...
    SQLITE_ASSIGN_OR_RETURN(ctx, auto* keys,
                            ExpectPointer<TreeKeysSpec>(argv[2], kName));
    SQLITE_ASSIGN_OR_RETURN(ctx, auto* order,
...
// =============================================================================
// tree_collapse - Collapses parent-child chains with same key (lazy)
// =============================================================================
struct TreeCollapse : public sqlite::Function<TreeCollapse> {
  static constexpr char kName[] = "tree_collapse";
  PERFETTO_TEMPLATED_USED static constexpr int kArgCount = -1;
  ...
  PERFETTO_TEMPLATED_USED static void Step(sqlite3_context* ctx,
                                           ...
                                           sqlite3_value** argv) {
    SQLITE_RETURN_IF_ERROR(ctx,
                           sqlite::utils::CheckArgCountAtLeast(kName, argc, 2));
    SQLITE_ASSIGN_OR_RETURN(ctx, auto* tree,
                            ExpectPointer<Tree>(argv[0], kName));
    SQLITE_ASSIGN_OR_RETURN(ctx, auto* keys,
                            ExpectPointer<TreeKeysSpec>(argv[1], kName));
    if (keys->column_names.size() != 1) {
      return sqlite::utils::SetError(
          ctx, "tree_collapse: expected exactly one key column");
    }
...
struct TreeEmit : public sqlite::Function<TreeEmit> {
  ...
  static base::Status ExecuteMerge(TreeData& data,
    ...
    // Find and validate order column
    ...
    // Find and validate all key columns
    std::vector<const PassthroughColumn*> key_cols;
    key_cols.reserve(op.key_columns.size());
    for (const auto& key_name : op.key_columns) {
      ASSIGN_OR_RETURN(const auto* key_col,
                       FindColumnOrError(materialized, key_name, "Merge"));
      if (!key_col->IsString() && !key_col->IsInt64()) {
        return base::ErrStatus("Key column '%s' must be string or integer type",
                               key_name.c_str());
      }
      key_cols.push_back(key_col);
    }

    // Use materialized values directly - they're already gathered via
    // source_indices. Don't double-gather!
    const auto& order_values = order_col->AsInt64();

    ASSIGN_OR_RETURN(auto merge_result,
                     MergeSiblings(data.parent_indices, key_cols, order_values,
                                   op.mode));

    FinalizeAggregation(data, std::move(merge_result.new_parent_indices),
                        materialized, merge_result.merged_sources,
                        op.aggregations);
    return base::OkStatus();
  }
  ...
  // Collapses parent-child chains where both have the same key.
  // Unlike merge/invert, collapse doesn't aggregate - it keeps the chain root's
  // values by updating source_indices (similar to delete).
  static base::Status ExecuteCollapse(TreeData& data,
                                      const TreeCollapseOp& op,
                                      StringPool* /*pool*/) {
    if (data.passthrough_columns.empty()) {
      return base::ErrStatus("tree_collapse requires passthrough columns");
    }

    // Find and validate key column
    ASSIGN_OR_RETURN(const auto* key_col,
                     FindColumnOrError(data.passthrough_columns, op.key_column,
                                       "Collapse"));

    CollapseResult collapse_result;
    if (key_col->IsString()) {
      auto key_values = GatherValues(key_col->AsString(), data.source_indices);
      ASSIGN_OR_RETURN(collapse_result,
                       Collapse<StringPool::Id>(data.parent_indices, key_values));
    } else if (key_col->IsInt64()) {
      auto key_values = GatherValues(key_col->AsInt64(), data.source_indices);
      ASSIGN_OR_RETURN(collapse_result,
                       Collapse<int64_t>(data.parent_indices, key_values));
    } else {
      return base::ErrStatus("Key column must be string or integer type");
    }

    // Update structure and source_indices (no materialization needed).
    // Each output row points to the first source in collapsed_sources (the
    // chain root).
    auto new_count = collapse_result.collapsed_sources.size();
    data.parent_indices = std::move(collapse_result.new_parent_indices);

    std::vector<uint32_t> new_source_indices(new_count);
    for (uint32_t i = 0; i < new_count; ++i) {
      // First source is the chain root (processed first in topological order)
      new_source_indices[i] =
          data.source_indices[collapse_result.collapsed_sources[i][0]];
    }
    data.source_indices = std::move(new_source_indices);
  ...
  static base::Status ExecuteInvert(TreeData& data,
    ...
    FinalizeAggregation(data, std::move(invert_result.new_parent_indices),
                        materialized, invert_result.merged_sources,
                        op.aggregations);
    return base::OkStatus();
  }
...
base::Status TreePlugin::Register(PluginContext& ctx) {
  // Helper functions
  RETURN_IF_ERROR(ctx.RegisterFunction<TreeKeys>(nullptr));
  ...
  RETURN_IF_ERROR(ctx.RegisterFunction<TreeCollapse>(nullptr));
