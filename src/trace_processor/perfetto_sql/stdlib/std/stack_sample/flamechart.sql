...
-- Whether a sampling timebase supports reconstructing callstack runs over time.
-- Clock, cycle, and instruction sampling qualify. Other event counters and
-- unknown timebases do not. This describes eligibility, not duration accuracy:
-- boundaries are inferred from samples rather than recorded function exits.
CREATE PERFETTO FUNCTION _stack_sample_flamechart_supported(
  -- The structured stack_sample_session.timebase_unit, not a counter name.
  timebase_unit STRING
)
-- True for supported sampling timebases; false for unknown or other units.
RETURNS BOOL
AS
SELECT coalesce($timebase_unit IN ('ns', 'cycles', 'instructions'), false);

-- sqlformat file off

-- Computes the flame-chart rectangle set ("runs") for a stack tree and a
-- series of sample points. Zero-duration runs from stack transitions at equal
...
-- still open after the last point have duration -1 (incomplete). Consumers
-- can extend these runs to the end of the trace.
--
-- Example usage:
-- ```
...
-- );
-- ```
CREATE PERFETTO MACRO _stack_sample_flamechart_runs(
