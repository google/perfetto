...
SELECT state, rank, replace(state, 'PROCESS_STATE_', '') AS state_name FROM r;
...
WITH
  ...
  raw_changes AS (
    SELECT
      ...
      s.is_initial,
      s.upid,
      s.proc_state AS cur_state,
      lag(s.proc_state) OVER (
        PARTITION BY
          s.upid
...
SELECT
  ...
  c.state,
  c.prev_state,
  coalesce(r.state_name, c.state) AS state_name,
  coalesce(pr.state_name, c.prev_state) AS prev_state_name,
...
LEFT JOIN _android_process_state_rank AS r
  ON r.state = c.state
LEFT JOIN _android_process_state_rank AS pr
  ON pr.state = c.prev_state
WHERE
  c.state != 'PROCESS_STATE_NONEXISTENT';
...
SELECT
  ...
  coalesce(r.state_name, c.group_name) AS state_name,
  coalesce(r.rank, 1000) AS state_rank,
