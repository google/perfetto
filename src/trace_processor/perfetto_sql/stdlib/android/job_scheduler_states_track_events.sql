...
-- Extracts the trace tag from a JobScheduler job name.
--
-- In Android JobScheduler, the job name string follows the general structure:
--   [#<trace_tag>#][@<namespace>@]<target_component_or_tag>
-- where the trace tag is an optional prefix enclosed between '#' delimiters.
--
-- Synthetic Examples:
--   - '#sync_worker#@BACKUP_NAMESPACE@com.example.app/com.example.app.BackupService' -> 'sync_worker'
--   - '#download_task#DownloadManager:com.example.downloader' -> 'download_task'
--   - '#cleanup#com.example.app/com.example.app.CleanupJobService' -> 'cleanup'
--   - 'com.example.app/com.example.app.SyncJobService' -> ''
--
-- Returns the extracted trace tag, or empty string if not present.
CREATE PERFETTO FUNCTION _android_js_extract_trace_tag(job_name STRING)
RETURNS STRING
AS
SELECT coalesce(regexp_extract($job_name, '#([^#]+)#'), '');

-- Extracts the package name from a JobScheduler job name.
--
-- In Android JobScheduler, the target package is embedded in one of two formats:
-- 1. Tagged / Delegated job format (SourceTag:PackageName):
--    [#<trace_tag>#][@<namespace>@]<source_tag>:<package_name>[#<job_id>]
--    where the package name is located after the last colon (':').
-- 2. Standard component format (PackageName/ClassName):
--    [#<trace_tag>#][@<namespace>@]<package_name>/<class_name>[#<job_id>]
--    where the package name is located before the slash ('/').
--
-- Synthetic Examples:
--   - '#sync_worker#@SYNC_NAMESPACE@com.example.sync/tag:com.example.app' -> 'com.example.app'
--   - '#download_task#DownloadManager:com.example.downloader' -> 'com.example.downloader'
--   - '#backup#@BACKUP_NAMESPACE@com.example.app/com.example.app.BackupService' -> 'com.example.app'
--   - 'com.example.app/com.example.app.SyncJobService' -> 'com.example.app'
--
...
SELECT
  coalesce(
    regexp_extract($job_name, ':([a-zA-Z0-9_.-]+)(?:#[0-9]+)?$'),
    regexp_extract($job_name, '([a-zA-Z0-9_.-]+)/')
  );

-- Extracts the job namespace from a JobScheduler job name.
--
-- In Android JobScheduler, the namespace is an optional segment enclosed
-- between '@' delimiters following any trace tag prefix:
--   [#<trace_tag>#][@<namespace>@]<target_component_or_tag>
--
-- Synthetic Examples:
--   - '#sync_worker#@SYNC_NAMESPACE@com.example.app/com.example.app.SyncJobService' -> 'SYNC_NAMESPACE'
--   - '@BACKUP_NAMESPACE@#restore_task#com.example.app/com.example.app.RestoreService' -> 'BACKUP_NAMESPACE'
--   - '#download_task#DownloadManager:com.example.downloader' -> ''
--   - 'com.example.app/com.example.app.SyncJobService' -> ''
--
-- Returns the extracted namespace, or empty string if not present.
...
SELECT
  ...
  _android_js_extract_trace_tag(
    CASE
      WHEN js.job_name IS NULL
      OR js.job_name = '' THEN s.name
      ELSE js.job_name
    END
  ) AS filtered_trace_tag,
...
-- 2. JOB_STATE_SCHEDULED -> JOB_STATE_CANCELLED: App calls cancel() or updates job while waiting in queue (!wasRunning).
-- 3. JOB_STATE_STARTED -> JOB_STATE_FINISHED: The only valid transition for a running job. Emitted on successful completion,
--    timeout, constraint drop, preemption, process death/ANR, or cancellation of a running job.
-- 4. JOB_STATE_FINISHED -> JOB_STATE_STARTED: Rescheduled periodic/retry job executes (internal rescheduling bypasses JOB_STATE_SCHEDULED).
-- 5. JOB_STATE_FINISHED -> JOB_STATE_CANCELLED: A rescheduled job waiting in queue for next window is cancelled before running.
-- 6. JOB_STATE_FINISHED -> JOB_STATE_SCHEDULED: App explicitly invokes JobScheduler.schedule() again for the job ID.
-- 7. JOB_STATE_CANCELLED -> JOB_STATE_SCHEDULED: App re-schedules or replaces the cancelled job with the same job ID.
--
-- Note on Packet Loss & Dropped Events:
-- JOB_STATE_STARTED -> JOB_STATE_SCHEDULED is invalid in the OS runtime and only appears in traces due to packet loss
-- (e.g. traced_chunks_discarded) where a JOB_STATE_FINISHED event was dropped, causing LEAD() to bridge
-- across the idle gap to the next JOB_STATE_SCHEDULED event when an app recycles the integer job_id.
--
-- Resilience to packet losses & dropped trace events:
-- 1. If JOB_STATE_FINISHED is dropped/missing:
--    The next event in the window partition will be JOB_STATE_SCHEDULED, JOB_STATE_CANCELLED, or NULL.
--    Because lead_state != 'JOB_STATE_FINISHED', the unclosed JOB_STATE_STARTED slice is safely discarded,
--    preventing false multi-hour durations from bridging across recycled IDs.
-- 2. If JOB_STATE_STARTED is dropped/missing:
--    The preceding event will be JOB_STATE_SCHEDULED, JOB_STATE_CANCELLED, or JOB_STATE_FINISHED. Because state != 'JOB_STATE_STARTED',
--    no execution interval is emitted.
