  ...
  job_states_with_lead AS (
    SELECT
      *,
      ...
      lead(public_stop_reason, 1, 'STOP_REASON_UNDEFINED') OVER job_asc AS lead_public_stop_reason,
...
SELECT
  ...
  ts_lead - ts AS dur,
  iif(lead_state = 'JOB_STATE_SCHEDULED', TRUE, FALSE) AS is_rescheduled,
  coalesce(
...
WHERE
  ...
  AND lead_state IN ('JOB_STATE_FINISHED', 'JOB_STATE_SCHEDULED');
