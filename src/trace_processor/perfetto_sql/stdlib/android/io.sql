...
CREATE PERFETTO PIPELINE _android_io_f2fs_counter_stats(
...
CREATE PERFETTO PIPELINE _android_io_f2fs_write_stats(
  -- Utid of the thread.
...
SUBPIPELINE f2fs_write_end AS (
  FROM ftrace_event
  |> WHERE name GLOB 'f2fs_write_end*'
  |> EXTEND extract_arg(arg_set_id, 'len') AS len
  |> EXTEND extract_arg(arg_set_id, 'dev') AS dev
  |> EXTEND extract_arg(arg_set_id, 'ino') AS ino
  |> EXTEND extract_arg(arg_set_id, 'copied') AS copied
)
...
|> AGGREGATE
   thread.utid,
   ANY_VALUE(thread.tid) AS tid,
   ANY_VALUE(thread.name) AS thread_name,
   ANY_VALUE(process.upid) AS upid,
...
-- and bytes written
CREATE PERFETTO PIPELINE _android_io_f2fs_aggregate_write_stats(
