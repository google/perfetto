...
CREATE OR REPLACE PERFETTO FUNCTION _find_android_jank_cuj_sf_main_thread_slice(
...
SELECT * FROM _FIND_ANDROID_JANK_CUJ_SF_MAIN_THREAD_SLICE('onMessageInvalidate *');
...
DROP TABLE IF EXISTS _android_jank_cuj_sf_main_thread_frame_boundary;
...
WITH combined_commit_composite_slice AS (
  ...
  FROM _android_jank_cuj_sf_commit_slice commit_slice
  JOIN _android_jank_cuj_sf_composite_slice composite_slice USING(cuj_id, vsync)
),
...
main_thread_slice AS (
  ...
  UNION ALL
  SELECT utid, cuj_id, vsync, ts, dur, ts_end FROM _android_jank_cuj_sf_on_message_invalidate_slice
)
SELECT
  cuj_id,
  utid,
