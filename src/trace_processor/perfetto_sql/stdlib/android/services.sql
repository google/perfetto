...
-- NOTE (psqlnext): the single-consumer `_bind_dispatch` and `_bind_receive`
-- tables are folded into `android_service_bindings` as inline SUBPIPELINEs. The
-- `graph_next_sibling!` adjacency macro is retained as a §4 source (next-sibling
-- by sort key is a single-hop lookup, not a transitive graph reduction).
...
CREATE PERFETTO PIPELINE android_service_bindings(
...
SUBPIPELINE dispatch AS (
  ...
  |> JOIN thread_slice AS slice ON slice.id = next_sibling.node_id
  |> JOIN slice AS next_slice ON next_slice.id = next_sibling.next_node_id
  |> WHERE next_slice.name = 'binder transaction async'
       AND slice.name GLOB 'requestServiceBinding*'
  |> SELECT
       next_slice.id,
       next_slice.ts,
       next_slice.dur,
       slice.utid,
       cast_int!(STR_SPLIT(STR_SPLIT(slice.name, 'bindSeq=', 1), ' ', 0)) AS bind_seq
)
...
FROM dispatch
|> JOIN receive ON dispatch.bind_seq = receive.bind_seq
|> LEFT JOIN android_binder_txns AS server_binder
     ON server_binder.binder_txn_id = dispatch.id
|> LEFT JOIN ancestor_slice(dispatch.id) AS anc ON anc.depth = 0
