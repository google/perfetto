...
-- Raw ftrace events
CREATE PERFETTO PIPELINE _raw_dmabuf_events MATERIALIZED AS
...
CREATE PERFETTO PIPELINE _gralloc_binders MATERIALIZED AS
SUBPIPELINE gralloc_threads AS (
  FROM process
...
CREATE PERFETTO PIPELINE _attributed_dmabufs MATERIALIZED AS
...
CREATE PERFETTO PIPELINE android_dmabuf_allocs(
...
CREATE PERFETTO PIPELINE android_memory_cumulative_dmabuf(
  -- upid of process responsible for the allocation (matches utid)
  upid JOINID(process.id),
  -- process name
  process_name STRING,
