...
CREATE PERFETTO PIPELINE _jank_cujs_slices MATERIALIZED AS
FROM slice
...
CREATE PERFETTO PIPELINE _cuj_state_markers MATERIALIZED AS
...
|> LEFT JOIN track AS marker_track ON marker_track.id = cuj_state_marker.track_id
...
|> WHERE
     ...
     OR (marker_track.name = cuj.cuj_slice_name AND cuj_state_marker.name GLOB 'FT#*')
...
CREATE PERFETTO PIPELINE _cuj_instant_events MATERIALIZED AS
FROM _jank_cujs_slices AS cuj
...
CREATE PERFETTO PIPELINE _all_frames_in_cuj AS
...
|> JOIN _jank_cujs_slices AS cuj ON cie.cuj_id = cuj.cuj_id
...
|> WHERE
     frame.frame_id >= cie.begin_vsync
     AND frame.frame_id <= cie.end_vsync
     AND (
...
CREATE PERFETTO PIPELINE _android_distinct_frames_in_cuj MATERIALIZED AS
...
|> AGGREGATE
     ...
     ANY_VALUE(cuj_name) AS cuj_name,
     ANY_VALUE(upid) AS upid,
     ANY_VALUE(cuj_layer_id) AS cuj_layer_id,
     ANY_VALUE(layer_id) AS layer_id,
     ANY_VALUE(layer_name) AS layer_name,
     ...
     ANY_VALUE(ui_thread_utid) AS ui_thread_utid,
     ANY_VALUE(render_thread_utid) AS render_thread_utid,
   ...
   GROUP BY frame_id, cuj_id
|> EXTEND row_number() OVER (PARTITION BY cuj_id ORDER BY frame_ts) AS frame_idx
|> EXTEND count(*) OVER (PARTITION BY cuj_id) AS frame_cnt;

-- Track all distinct frames with layer_id consideration that overlap with the CUJ slice.
CREATE PERFETTO PIPELINE _android_distinct_frames_layers_cuj AS
-- Captures all frames in the CUJ boundary. In cases where there are multiple actual frames, there
-- can be multiple rows with the same frame_id.
FROM _all_frames_in_cuj
|> AGGREGATE
     ...
     ANY_VALUE(ui_thread_utid) AS ui_thread_utid,
     ANY_VALUE(render_thread_utid) AS render_thread_utid,
     ANY_VALUE(frame_ts) AS frame_ts,
     ANY_VALUE(ts_end) AS ts_end,
     ANY_VALUE(dur) AS dur
   GROUP BY frame_id, cuj_id, layer_id;

-- Table captures all Choreographer#doFrame within a CUJ boundary.
CREATE PERFETTO PIPELINE _android_jank_cuj_do_frames MATERIALIZED AS
SUBPIPELINE do_frame_slice_with_end_ts AS (
  FROM android_frames_choreographer_do_frame AS do_frame
  |> JOIN slice USING (id)
  |> SELECT
       slice.id,
       do_frame.frame_id AS vsync,
       do_frame.ts,
       do_frame.upid,
...
|> WHERE
     ...
     AND do_frame.vsync > 0
     AND (do_frame.vsync >= cie.begin_vsync OR cie.begin_vsync IS NULL)
     AND (do_frame.vsync <= cie.end_vsync OR cie.end_vsync IS NULL)
|> SELECT cuj.cuj_id, cie.ui_thread, thread.utid, do_frame.*;

-- Table tracking all jank CUJs information.
CREATE PERFETTO PIPELINE android_jank_cuj(
