...
CREATE PERFETTO TABLE _android_jank_cuj_sf_vsync_boundary(
  -- CUJ id.
  cuj_id LONG,
  -- Minimum vsync ID within the CUJ for SF.
  vsync_min LONG,
  -- Maximum vsync ID within the CUJ for SF.
  vsync_max LONG
)
AS
...
CREATE PERFETTO TABLE _android_jank_cuj_render_thread_frame_boundary(
  ...
  -- Vsync ID of this frame.
  vsync LONG,
  -- Expected start timestamp (from postAndWait).
  ts_expected TIMESTAMP,
  ...
  -- Corrected start timestamp for the frame boundary.
  ts TIMESTAMP,
  -- End timestamp of the frame boundary.
  ts_end TIMESTAMP,
  -- Duration of the frame boundary.
  dur DURATION
)
...
CREATE PERFETTO TABLE _android_jank_cuj_sf_render_engine_frame_boundary(
  -- CUJ id.
  cuj_id LONG,
  -- Thread id of the RE thread.
  utid JOINID(thread.id),
  -- Vsync ID of this frame.
  vsync LONG,
  -- Start timestamp of the RE frame boundary.
  ts TIMESTAMP,
  -- Start timestamp of the drawLayers slice.
  ts_draw_layers_start TIMESTAMP,
  -- End timestamp of the RE frame boundary.
  ts_end TIMESTAMP,
  -- Duration of the RE frame boundary.
  dur DURATION
)
AS
SELECT
...
-- Compute the overall CUJ boundary on SF.
CREATE PERFETTO TABLE _android_jank_cuj_sf_boundary(
  -- CUJ id.
  cuj_id LONG,
  -- Start timestamp of the CUJ on SF.
  ts TIMESTAMP,
  -- End timestamp of the CUJ on SF.
  ts_end TIMESTAMP,
  -- Duration of the CUJ on SF.
  dur DURATION
)
AS
