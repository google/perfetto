...
CREATE PERFETTO PIPELINE android_sysui_jank_cujs(
...
SUBPIPELINE cuj_frame_boundary AS (
  FROM _android_distinct_frames_in_cuj
  |> WHERE frame_idx = 1 OR frame_idx = frame_cnt
  ...
  |> EXTEND (
       ...
       WHERE id = expected_frame_timeline_id
     ) AS start_frame_ts
  |> EXTEND (
       SELECT (ts + dur) AS ts_end FROM slice WHERE id = do_frame_id
     ) AS end_frame_ts_end
)
...
|> AGGREGATE
     cuj.cuj_id AS cuj_id,
     ANY_VALUE(cuj.upid) AS upid,
     ANY_VALUE(cuj.process_name) AS process_name,
     ANY_VALUE(cuj.cuj_slice_name) AS cuj_slice_name,
     -- Extracts "CUJ_NAME" from "J<CUJ_NAME>"
     ANY_VALUE(_extract_cuj_name_from_slice(cuj.cuj_slice_name)) AS cuj_name,
     ANY_VALUE(cuj.slice_id) AS slice_id,
     ...
     ANY_VALUE(CASE
       ...
       ELSE NULL
     END) AS state,
     ANY_VALUE(cuj_events.ui_thread) AS ui_thread,
     ANY_VALUE(cuj_events.layer_id) AS layer_id,
     ANY_VALUE(cuj_events.begin_vsync) AS begin_vsync,
     ANY_VALUE(cuj_events.end_vsync) AS end_vsync
   GROUP BY cuj_id
|> WHERE
   state != 'canceled'
...
CREATE PERFETTO PIPELINE android_sysui_latency_cujs(
  -- Unique incremental ID for each CUJ.
  cuj_id LONG,
  -- process id.
  upid JOINID(process.id),
...
SUBPIPELINE _latency_cuj_tracks AS (
  FROM track
  |> WHERE name GLOB 'L<*>'
  |> SELECT id, name
)
-- Markers (cancel, timeout) related to Latency CUJ slices.
SUBPIPELINE _latency_cuj_markers AS (
...
CREATE PERFETTO PIPELINE android_jank_latency_cujs(
  -- Unique incremental ID for each CUJ.
  cuj_id LONG,
...
|> EXTEND "jank" AS cuj_type, cuj_id AS original_cuj_id
|> UNION ALL (
