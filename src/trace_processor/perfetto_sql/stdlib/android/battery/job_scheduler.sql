...
CREATE PERFETTO FUNCTION _has_job_state_flag(flags LONG, flag_name STRING)
RETURNS BOOL
AS
SELECT
  CASE $flag_name
    WHEN 'HAS_CHARGING_CONSTRAINT' THEN ($flags & (1 << 0)) != 0
    WHEN 'HAS_BATTERY_NOT_LOW_CONSTRAINT' THEN ($flags & (1 << 1)) != 0
    WHEN 'HAS_STORAGE_NOT_LOW_CONSTRAINT' THEN ($flags & (1 << 2)) != 0
    WHEN 'HAS_TIMING_DELAY_CONSTRAINT' THEN ($flags & (1 << 3)) != 0
    WHEN 'HAS_DEADLINE_CONSTRAINT' THEN ($flags & (1 << 4)) != 0
    WHEN 'HAS_IDLE_CONSTRAINT' THEN ($flags & (1 << 5)) != 0
    WHEN 'HAS_CONNECTIVITY_CONSTRAINT' THEN ($flags & (1 << 6)) != 0
    WHEN 'HAS_CONTENT_TRIGGER_CONSTRAINT' THEN ($flags & (1 << 7)) != 0
    WHEN 'IS_REQUESTED_EXPEDITED_JOB' THEN ($flags & (1 << 8)) != 0
    WHEN 'IS_RUNNING_AS_EXPEDITED_JOB' THEN ($flags & (1 << 9)) != 0
    WHEN 'IS_PREFETCH' THEN ($flags & (1 << 10)) != 0
    WHEN 'IS_REQUESTED_AS_USER_INITIATED_JOB' THEN ($flags & (1 << 19)) != 0
    WHEN 'IS_RUNNING_AS_USER_INITIATED_JOB' THEN ($flags & (1 << 20)) != 0
    WHEN 'IS_PERIODIC' THEN ($flags & (1 << 21)) != 0
    WHEN 'HAS_FLEXIBILITY_CONSTRAINT' THEN ($flags & (1 << 22)) != 0
    WHEN 'CAN_APPLY_TRANSPORT_AFFINITIES' THEN ($flags & (1 << 24)) != 0
    ELSE FALSE
  END;

-- Converts job state integer to string name.
CREATE PERFETTO FUNCTION _get_job_state_name(state LONG)
RETURNS STRING
AS
SELECT
  CASE $state
...
CREATE PERFETTO FUNCTION _get_standby_bucket_name(bucket LONG)
RETURNS STRING
AS
SELECT
  CASE $bucket
    WHEN 0 THEN 'ACTIVE'
    WHEN 1 THEN 'WORKING_SET'
    WHEN 2 THEN 'FREQUENT'
    WHEN 3 THEN 'RARE'
    WHEN 4 THEN 'NEVER'
    WHEN 5 THEN 'RESTRICTED'
    WHEN 6 THEN 'EXEMPTED'
    ELSE 'UNKNOWN'
  END;

-- Converts internal stop reason integer to string name.
CREATE PERFETTO FUNCTION _get_internal_stop_reason_name(reason LONG)
RETURNS STRING
AS
SELECT
  CASE $reason
    WHEN -1 THEN 'INTERNAL_STOP_REASON_UNKNOWN'
    WHEN 0 THEN 'INTERNAL_STOP_REASON_CANCELED'
    WHEN 1 THEN 'INTERNAL_STOP_REASON_CONSTRAINTS_NOT_SATISFIED'
    WHEN 2 THEN 'INTERNAL_STOP_REASON_PREEMPT'
    WHEN 3 THEN 'INTERNAL_STOP_REASON_TIMEOUT'
    WHEN 4 THEN 'INTERNAL_STOP_REASON_DEVICE_IDLE'
    WHEN 5 THEN 'INTERNAL_STOP_REASON_DEVICE_THERMAL'
    WHEN 6 THEN 'INTERNAL_STOP_REASON_RESTRICTED_BUCKET'
    WHEN 7 THEN 'INTERNAL_STOP_REASON_UNINSTALL'
    WHEN 8 THEN 'INTERNAL_STOP_REASON_DATA_CLEARED'
    WHEN 9 THEN 'INTERNAL_STOP_REASON_RTC_UPDATED'
    WHEN 10 THEN 'INTERNAL_STOP_REASON_SUCCESSFUL_FINISH'
    WHEN 11 THEN 'INTERNAL_STOP_REASON_USER_UI_STOP'
    WHEN 12 THEN 'INTERNAL_STOP_REASON_ANR'
    WHEN 13 THEN 'INTERNAL_STOP_REASON_TIMEOUT_ABANDONED'
    ELSE 'INTERNAL_STOP_REASON_UNKNOWN'
  END;

-- Converts public stop reason integer to string name.
CREATE PERFETTO FUNCTION _get_public_stop_reason_name(reason LONG)
RETURNS STRING
AS
SELECT
  CASE $reason
    WHEN 0 THEN 'STOP_REASON_UNDEFINED'
    WHEN 1 THEN 'STOP_REASON_CANCELLED_BY_APP'
    WHEN 2 THEN 'STOP_REASON_PREEMPT'
    WHEN 3 THEN 'STOP_REASON_TIMEOUT'
    WHEN 4 THEN 'STOP_REASON_DEVICE_STATE'
    WHEN 5 THEN 'STOP_REASON_CONSTRAINT_BATTERY_NOT_LOW'
    WHEN 6 THEN 'STOP_REASON_CONSTRAINT_CHARGING'
    WHEN 7 THEN 'STOP_REASON_CONSTRAINT_CONNECTIVITY'
    WHEN 8 THEN 'STOP_REASON_CONSTRAINT_DEVICE_IDLE'
    WHEN 9 THEN 'STOP_REASON_CONSTRAINT_STORAGE_NOT_LOW'
    WHEN 10 THEN 'STOP_REASON_QUOTA'
    WHEN 11 THEN 'STOP_REASON_BACKGROUND_RESTRICTION'
    WHEN 12 THEN 'STOP_REASON_APP_STANDBY'
    WHEN 13 THEN 'STOP_REASON_USER'
    WHEN 14 THEN 'STOP_REASON_SYSTEM_PROCESSING'
    WHEN 15 THEN 'STOP_REASON_ESTIMATED_APP_LAUNCH_TIME_CHANGED'
    WHEN 16 THEN 'STOP_REASON_TIMEOUT_ABANDONED'
    ELSE 'STOP_REASON_UNDEFINED'
  END;

-- Converts backoff policy integer to string name.
CREATE PERFETTO FUNCTION _get_backoff_policy(policy_type LONG)
RETURNS STRING
AS
SELECT
  CASE $policy_type
    WHEN 0 THEN 'UNKNOWN_POLICY'
    WHEN 1 THEN 'LINEAR'
    WHEN 2 THEN 'EXPONENTIAL'
    ELSE 'UNKNOWN_POLICY'
  END;

-- Converts proc state integer to string name.
CREATE PERFETTO FUNCTION _get_proc_state_name(state LONG)
RETURNS STRING
AS
SELECT
  CASE $state
    WHEN 0 THEN 'PROCESS_STATE_UNSPECIFIED'
    WHEN 998 THEN 'PROCESS_STATE_UNKNOWN_TO_PROTO'
    WHEN 999 THEN 'PROCESS_STATE_UNKNOWN'
    WHEN 1000 THEN 'PROCESS_STATE_PERSISTENT'
    WHEN 1001 THEN 'PROCESS_STATE_PERSISTENT_UI'
    WHEN 1002 THEN 'PROCESS_STATE_TOP'
    WHEN 1003 THEN 'PROCESS_STATE_FOREGROUND_SERVICE'
    WHEN 1004 THEN 'PROCESS_STATE_BOUND_FOREGROUND_SERVICE'
    WHEN 1005 THEN 'PROCESS_STATE_IMPORTANT_FOREGROUND'
    WHEN 1006 THEN 'PROCESS_STATE_IMPORTANT_BACKGROUND'
    WHEN 1007 THEN 'PROCESS_STATE_TRANSIENT_BACKGROUND'
    WHEN 1008 THEN 'PROCESS_STATE_BACKUP'
    WHEN 1009 THEN 'PROCESS_STATE_SERVICE'
    WHEN 1010 THEN 'PROCESS_STATE_RECEIVER'
    WHEN 1011 THEN 'PROCESS_STATE_TOP_SLEEPING'
    WHEN 1012 THEN 'PROCESS_STATE_HEAVY_WEIGHT'
    WHEN 1013 THEN 'PROCESS_STATE_HOME'
    WHEN 1014 THEN 'PROCESS_STATE_LAST_ACTIVITY'
    WHEN 1015 THEN 'PROCESS_STATE_CACHED_ACTIVITY'
    WHEN 1016 THEN 'PROCESS_STATE_CACHED_ACTIVITY_CLIENT'
    WHEN 1017 THEN 'PROCESS_STATE_CACHED_RECENT'
    WHEN 1018 THEN 'PROCESS_STATE_CACHED_EMPTY'
    WHEN 1019 THEN 'PROCESS_STATE_NONEXISTENT'
    WHEN 1020 THEN 'PROCESS_STATE_BOUND_TOP'
    ELSE 'PROCESS_STATE_UNKNOWN'
  END;

-- Extracts trace tag from job name if job name is in format #tag#...
CREATE PERFETTO FUNCTION _extract_trace_tag(job_name STRING)
RETURNS STRING
AS
SELECT
...
CREATE PERFETTO FUNCTION _get_priority_name(priority LONG)
RETURNS STRING
AS
SELECT
  CASE
    WHEN $priority = 100 THEN 'PRIORITY_MIN'
    WHEN $priority = 200 THEN 'PRIORITY_LOW'
    WHEN $priority = 300 THEN 'PRIORITY_DEFAULT'
    WHEN $priority = 400 THEN 'PRIORITY_HIGH'
    WHEN $priority = 500 THEN 'PRIORITY_MAX'
    ELSE 'PRIORITY_UNKNOWN'
  END;

CREATE PERFETTO FUNCTION _extract_package_name(job_name STRING)
RETURNS STRING
AS
SELECT
  coalesce(
...
CREATE PERFETTO FUNCTION _extract_js_arg(arg_set_id LONG, field_name STRING)
RETURNS STRING
AS
SELECT extract_arg($arg_set_id, 'job_scheduler_job.' || $field_name);
...
SELECT
  extract_arg(
    $arg_set_id,
    'thermal_throttling_severity_state_changed.' || $field_name
  );

-- Extracts namespace from job name if job name is in format @namespace@...
CREATE PERFETTO FUNCTION _extract_job_namespace(job_name STRING)
RETURNS STRING
AS
SELECT
  -- This regex is "greedy": the '.*' will match as many characters as possible
...
SELECT
  ...
  _get_standby_bucket_name(
    CAST(_extract_js_arg(arg_set_id, 'standby_bucket') AS INTEGER)
  ) AS standby_bucket,
  _get_priority_name(
    CAST(_extract_js_arg(arg_set_id, 'requested_priority') AS INTEGER)
  ) AS requested_priority,
  _get_priority_name(
    CAST(_extract_js_arg(arg_set_id, 'effective_priority') AS INTEGER)
  ...
  _get_proc_state_name(
    CAST(_extract_js_arg(arg_set_id, 'proc_state') AS INTEGER)
  ) AS proc_state,
  _get_internal_stop_reason_name(
    CAST(_extract_js_arg(arg_set_id, 'internal_stop_reason') AS INTEGER)
  ) AS internal_stop_reason,
  _get_public_stop_reason_name(
    CAST(_extract_js_arg(arg_set_id, 'public_stop_reason') AS INTEGER)
  ...
  _get_backoff_policy(
    CAST(_extract_js_arg(arg_set_id, 'back_off_policy_type') AS INTEGER)
  ) AS back_off_policy_type,
...
SELECT
  ...
  CAST((trace_start() / (1e9 * 60.0)) AS INTEGER) AS uptime_mins
...
WITH
  events AS (
    SELECT
      ...
      extract_arg(
        s.arg_set_id,
        'scheduled_job_state_changed.attribution_node[0].uid'
      ) AS uid,
...
SELECT ts, dur, uid, job_name FROM windowed WHERE state = 'STARTED' ORDER BY ts;
