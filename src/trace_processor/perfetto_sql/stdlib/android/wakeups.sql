...
CREATE PERFETTO PIPELINE android_wakeups(
  -- Timestamp.
  ts TIMESTAMP,
...
SUBPIPELINE wakeup_attribution AS (
  ...
  FROM track AS t
  |> JOIN instant AS i ON t.id = i.track_id
...
FROM step7
|> SELECT
     ts, dur, raw_wakeup, on_device_attribution, suspend_quality,
     backoff_state, backoff_reason, backoff_count, backoff_millis, type,
     item_0 AS item
|> UNION ALL (
     FROM step7
     |> WHERE item_1 IS NOT NULL
     |> SELECT
          ts, dur, raw_wakeup, on_device_attribution, suspend_quality,
          backoff_state, backoff_reason, backoff_count, backoff_millis, type,
          item_1 AS item
