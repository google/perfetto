...
--
-- Contains decoded field values, string content, and primitive array blob
-- references. Only populated for HPROF (ART) heap dumps, not for proto
-- heap graphs. Join with heap_graph_object on object_id.
CREATE PERFETTO VIEW heap_graph_object_data (
  ...
  -- Join key with heap_graph_object_field containing primitive field values.
  field_set_id JOINID(heap_graph_object_field.field_set_id),
  -- Decoded string value for java.lang.String instances, or text
  -- representation for primitive arrays.
  value_string STRING,
...
-- Primitive field values and array elements for heap graph objects.
--
-- This associates the object with given field_set_id with its primitive
-- field values (for instances) or array elements (for primitive arrays).
CREATE PERFETTO VIEW heap_graph_object_field (
  ...
  object_id JOINID(heap_graph_object.id),
  -- The field name. E.g. Foo.count, or [0] for array elements.
  field_name STRING,
  ...
  double_value DOUBLE,
  -- Decoded string value for java.lang.String fields.
  string_value STRING
) AS
SELECT
  id,
  ...
  double_value,
  string_value
FROM __intrinsic_heap_graph_object_field;
