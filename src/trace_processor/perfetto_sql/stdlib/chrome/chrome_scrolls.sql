...
-- type.
CREATE PERFETTO PIPELINE chrome_scroll_update_refs (
...
CREATE PERFETTO PIPELINE chrome_scroll_update_input_pipeline (
...
-- into a frame) stages of a scroll.
CREATE PERFETTO PIPELINE chrome_scroll_update_frame_pipeline (
  -- Id of the `LatencyInfo.Flow` slices corresponding to this scroll event.
  id LONG,
...
CREATE PERFETTO PIPELINE chrome_scrolls (
...
--                 +---------------------------+
CREATE PERFETTO PIPELINE chrome_scroll_update_info (
  -- Id of the `LatencyInfo.Flow` slices corresponding to this scroll event.
  id LONG,
  -- Id of the scroll this scroll update belongs to.
  scroll_id LONG,
...
-- metadata.
CREATE PERFETTO PIPELINE chrome_scroll_frame_info (
...
CREATE PERFETTO PIPELINE chrome_scroll_update_info_step_templates (
...
FROM (VALUES
  ...
  ('ScrollUpdateProcessing', 'scroll_update_created_ts', 'scroll_update_processing_dur'),
  ('BrowserMainToRendererCompositor', 'scroll_update_created_end_ts', 'browser_to_compositor_delay_dur'),
  ('RendererCompositorDispatch', 'compositor_dispatch_ts', 'compositor_dispatch_dur'),
  ('RendererCompositorDispatchToOnBeginFrame', 'compositor_dispatch_end_ts', 'compositor_dispatch_to_on_begin_frame_delay_dur'),
  ('RendererCompositorBeginFrame', 'compositor_on_begin_frame_ts', 'compositor_on_begin_frame_dur'),
  ('RendererCompositorBeginToGenerateFrame', 'compositor_on_begin_frame_end_ts', 'compositor_on_begin_frame_to_generation_delay_dur'),
  ('RendererCompositorGenerateToSubmitFrame', 'compositor_generate_compositor_frame_ts', 'compositor_generate_frame_to_submit_frame_dur'),
  ('RendererCompositorSubmitFrame', 'compositor_submit_compositor_frame_ts', 'compositor_submit_frame_dur'),
  ('RendererCompositorToViz', 'compositor_submit_compositor_frame_end_ts', 'compositor_to_viz_delay_dur'),
  ('VizReceiveFrame', 'viz_receive_compositor_frame_ts', 'viz_receive_compositor_frame_dur'),
  ('VizReceiveToDrawFrame', 'viz_receive_compositor_frame_end_ts', 'viz_wait_for_draw_dur'),
  ...
  ('VizSwapBuffersToLatch', 'viz_swap_buffers_end_ts', 'viz_swap_buffers_to_latch_dur'),
  ('VizLatchToPresentation', 'latch_timestamp', 'viz_latch_to_presentation_dur'),
  ('Presentation', 'presentation_timestamp', NULL)) AS steps(step_name, ts_column_name, dur_column_name);
