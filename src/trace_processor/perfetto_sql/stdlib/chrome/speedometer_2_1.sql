...
CREATE PERFETTO PIPELINE _chrome_speedometer_2_1_mark_name (
  -- Expected slice name
  name STRING,
  -- Suite name
  suite_name STRING,
...
SUBPIPELINE data AS (
  ...
  |> SELECT column1 AS suite_name, column2 AS test_name
)
SUBPIPELINE mark_type AS (
  FROM (VALUES
    ('start'),
    ('sync-end'),
    ('async-end')) AS _values
  |> SELECT column1 AS mark_type
)
FROM data
|> JOIN mark_type ON TRUE
|> SELECT
     data.suite_name || '.' || data.test_name || '-' || mark_type.mark_type AS name,
     data.suite_name,
     data.test_name,
     mark_type.mark_type;
...
CREATE PERFETTO PIPELINE chrome_speedometer_2_1_measure (
...
-- Get the 3 test timestamps (start, sync-end, async-end) in one row. Using
-- the LAG window function and partitioning by test. 2 out of the 3 rows
-- generated per test will have some NULL ts values.
SUBPIPELINE filtered AS (
  FROM slice AS s
  |> JOIN _chrome_speedometer_2_1_mark_name AS m USING (name)
  |> WHERE s.category = 'blink.user_timing'
  |> SELECT
       rank() OVER (PARTITION BY s.name ORDER BY s.ts ASC) AS iteration,
       m.suite_name,
       m.test_name,
       s.ts,
       s.ts AS async_end_ts,
       lag(s.ts, 1) OVER (
         PARTITION BY rank() OVER (PARTITION BY s.name ORDER BY s.ts ASC),
                      m.suite_name, m.test_name
         ORDER BY s.ts ASC
       ) AS sync_end_ts,
       lag(s.ts, 2) OVER (
         PARTITION BY rank() OVER (PARTITION BY s.name ORDER BY s.ts ASC),
  ...
  -- This serves 2 purposes: make sure we have all the marks (think truncated
...
SUBPIPELINE base AS (
  FROM filtered
  |> SELECT
       filtered.sync_end_ts AS ts,
       filtered.async_end_ts - filtered.sync_end_ts AS dur,
       filtered.iteration,
       filtered.suite_name,
       filtered.test_name,
  ...
  |> UNION ALL (
       FROM filtered
       |> SELECT
            filtered.start_ts AS ts,
            filtered.sync_end_ts - filtered.start_ts AS dur,
            filtered.iteration,
            filtered.suite_name,
            filtered.test_name,
...
|> SELECT
     base.ts,
     base.dur,
     base.suite_name || '.' || base.test_name || '-' || base.measure_type AS name,
     base.iteration,
     base.suite_name,
     base.test_name,
     base.measure_type;
...
CREATE PERFETTO PIPELINE chrome_speedometer_2_1_iteration (
