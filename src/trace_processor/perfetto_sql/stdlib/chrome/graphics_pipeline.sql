...
CREATE PERFETTO PIPELINE chrome_graphics_pipeline_surface_frame_steps (
...
|> WHERE
     ...
     AND extract_arg(arg_set_id, 'chrome_graphics_pipeline.surface_frame_trace_id') IS NOT NULL
|> SELECT
     id,
     ts,
     dur,
...
CREATE PERFETTO PIPELINE chrome_graphics_pipeline_display_frame_steps (
  -- Slice Id of the `Graphics.Pipeline` slice.
  id LONG,
  -- The start timestamp of the slice/step.
  ts TIMESTAMP,
...
|> WHERE
     name = 'Graphics.Pipeline'
     AND extract_arg(arg_set_id, 'chrome_graphics_pipeline.display_trace_id') IS NOT NULL
|> SELECT
     id,
     ts,
     dur,
...
|> SELECT id, ts, dur, step, display_trace_id, utid, task_start_time_ts;
...
-- `display_trace_id`.
CREATE PERFETTO PIPELINE chrome_surface_frame_id_to_first_display_id (
  -- Id of the graphics pipeline, pre-surface aggregation.
  surface_frame_trace_id LONG,
  -- Id of the graphics pipeline, post-surface aggregation.
  display_trace_id LONG
...
CREATE PERFETTO PIPELINE chrome_graphics_pipeline_inputs_to_surface_frames (
...
|> SELECT args.int_value AS latency_id, surface_frame_trace_id;
