...
CREATE PERFETTO PIPELINE chrome_histograms (
...
|> JOIN thread_track ON thread_track.id = hist.track_id
...
|> UNION ALL (
     ...
     -- "org.chromium.histogram_sample" data source. These samples are
     -- associated with a process track.
     FROM hist
     |> JOIN process_track ON process_track.id = hist.track_id
     |> JOIN process USING (upid)
     |> SELECT
          hist.name,
          hist.value,
