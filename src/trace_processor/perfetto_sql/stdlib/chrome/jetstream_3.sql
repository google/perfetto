...
-- benchmarks.
CREATE PERFETTO PIPELINE _chrome_jetstream_3_benchmark_name (
  name STRING,
...
FROM (
  ...
  ) AS data(name, top_level_name, worst_case_count)
|> SELECT name, COALESCE(top_level_name, name) AS top_level_name, worst_case_count;
...
CREATE PERFETTO PIPELINE _chrome_jetstream_3_measure_slice AS
FROM slice
...
-- Find WSL slices.
CREATE PERFETTO PIPELINE _chrome_jetstream_3_wsl_slice AS
FROM slice
...
CREATE PERFETTO PIPELINE chrome_jetstream_3_measure (
  -- The slice id.
  id LONG,
  -- The slice timestamp.
  ts TIMESTAMP,
...
SUBPIPELINE iteration_ranked AS (
  FROM _chrome_jetstream_3_measure_slice
  |> WHERE iteration > 0
  |> EXTEND RANK() OVER (PARTITION BY name ORDER BY dur DESC) AS rank_within_name
)
-- Average: All iterations (excluding the first and worst ones).
SUBPIPELINE average AS (
  FROM iteration_ranked
  |> WHERE rank_within_name > worst_case_count
  |> SELECT id, ts, dur, name, top_level_name, iteration, 'Average' AS subtest
)
-- First: The first iteration (iteration 0).
SUBPIPELINE first AS (
  FROM _chrome_jetstream_3_measure_slice
  |> WHERE iteration = 0
  |> SELECT id, ts, dur, name, top_level_name, iteration, 'First' AS subtest
)
-- WSL: Specific WSL slices.
SUBPIPELINE wsl AS (
  FROM _chrome_jetstream_3_wsl_slice
  |> SELECT id, ts, dur, name, top_level_name, iteration, subtest
)
-- Worst: The slowest N iterations (excluding the first).
FROM iteration_ranked
|> WHERE rank_within_name <= worst_case_count
|> SELECT id, ts, dur, name, top_level_name, iteration, 'Worst' AS subtest
|> UNION ALL (FROM average)
|> UNION ALL (FROM first)
|> UNION ALL (FROM wsl);

-- Calculate score for each subtest classification.
--
-- A subtest score is 5000 / arithmetic mean of durations in milliseconds.
CREATE PERFETTO PIPELINE _chrome_jetstream_3_subtest_score AS
...
CREATE PERFETTO PIPELINE _chrome_jetstream_3_leaf_benchmark_score AS
FROM _chrome_jetstream_3_subtest_score
|> AGGREGATE EXP(AVG(LN(score))) AS score GROUP BY top_level_name, name;
...
-- score.
CREATE PERFETTO PIPELINE chrome_jetstream_3_benchmark_score (
  -- The top-level benchmark group name.
  top_level_name STRING,
...
|> AGGREGATE EXP(AVG(LN(score))) AS score GROUP BY top_level_name;
