...
CREATE PERFETTO TABLE _tasks_summary AS
SELECT
  sched.ts,
  sched.dur,
  sched.cpu,
...
CREATE PERFETTO INDEX all_irq_filter_idx ON _all_irq_slices(is_hard_irq, parent_id);

-- Flatten hard IRQs, since they can preempt each other.
CREATE PERFETTO TABLE _hard_irq_flattened_slices AS
WITH
  root_slices AS (
    SELECT
      id,
...
CREATE PERFETTO TABLE _soft_irq_slices AS
SELECT
  _all_irq_slices.ts,
  _all_irq_slices.dur,
...
CREATE PERFETTO TABLE _all_irqs_flattened_slices AS
WITH
  base_name AS (
    SELECT
      ts,
