...
-- Step 1: Find active GPU islands (contiguous freq > 0)
CREATE PERFETTO TABLE _gpu_pulses AS
WITH
...
-- Step 2: Find tasks within pulses
CREATE PERFETTO TABLE _gpu_pulse_tasks AS
SELECT
  ii.ts,
  ii.dur,
  ta.uid,
...
FROM _interval_intersect!(
  (
    _ii_subquery!(_gpu_pulses),
    _ii_subquery!(_gpu_tasks)
  ),
  ()
) AS ii
JOIN _gpu_pulses AS p
  ON p._auto_id = id_0
JOIN _gpu_tasks AS ta
  ON ta._auto_id = id_1;

-- Step 3: Find pulse task boundaries
CREATE PERFETTO VIEW _gpu_pulse_boundaries AS
WITH
  ranked AS (
    SELECT
      pulse_id,
      ...
      dur,
      uid,
      row_number() OVER (PARTITION BY pulse_id ORDER BY ts ASC) AS rnk_asc,
      row_number() OVER (PARTITION BY pulse_id ORDER BY ts DESC) AS rnk_desc
    FROM _gpu_pulse_tasks
  )
SELECT
  pulse_id,
  min(ts) AS min_ts,
...
-- Step 4: Classify gaps within pulses
...
CREATE PERFETTO TABLE _gaps_in_pulses AS
SELECT
  ii.ts,
  ii.dur,
  id_0 AS pulse_id
FROM _interval_intersect!(
  (
    (SELECT _auto_id AS id, ts, dur FROM _gpu_pulses),
...
CREATE PERFETTO TABLE _gpu_pulse_gaps AS
SELECT
  g.ts,
  g.dur,
  CASE
    ...
    THEN b.first_uid
    WHEN g.ts >= b.max_end_ts
    THEN b.last_uid
    ELSE (
      SELECT
        uid
      FROM _gpu_pulse_tasks AS t
      WHERE
        t.pulse_id = g.pulse_id AND t.ts >= g.ts + g.dur
      ORDER BY
        ts ASC
      LIMIT 1
    )
  END AS uid
FROM _gaps_in_pulses AS g
JOIN _gpu_pulse_boundaries AS b
  ON g.pulse_id = b.pulse_id;
...
FROM _interval_intersect!(
  (
    _ii_subquery!(_gpu_pulse_gaps),
    _ii_subquery!(_gpu_estimates_mw)
  ),
  ()
) AS ii
JOIN _gpu_pulse_gaps AS ta
  ON ta._auto_id = id_0
