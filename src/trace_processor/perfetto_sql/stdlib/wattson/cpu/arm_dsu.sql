...
CREATE PERFETTO PIPELINE _arm_l3_miss_rate MATERIALIZED AS
...
|> FORK AS base
|> AGGREGATE ANY_VALUE(trace_start()) AS ts, min(ts) - ANY_VALUE(trace_start()) AS dur, ANY_VALUE(0) AS l3_miss_rate
|> UNION ALL (FROM base |> SELECT ts, dur, l3_miss_rate)
|> UNION ALL (
     FROM (SELECT trace_start() AS ts, trace_dur() AS dur, 0 AS l3_miss_rate)
...
CREATE PERFETTO PIPELINE _arm_l3_hit_rate MATERIALIZED AS
...
|> FORK AS base
|> AGGREGATE ANY_VALUE(trace_start()) AS ts, min(ts) - ANY_VALUE(trace_start()) AS dur, ANY_VALUE(0) AS l3_hit_rate
|> UNION ALL (FROM base |> SELECT ts, dur, l3_hit_rate)
|> UNION ALL (
     FROM (SELECT trace_start() AS ts, trace_dur() AS dur, 0 AS l3_hit_rate)
     |> WHERE NOT EXISTS (SELECT 1 FROM base)
   );

-- Combine L3 hit and miss rates into a single table.
CREATE PERFETTO PIPELINE _arm_l3_rates MATERIALIZED AS
...
CREATE PERFETTO PIPELINE _wattson_dsu_frequency MATERIALIZED AS
SUBPIPELINE base AS (
