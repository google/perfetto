...
CREATE PERFETTO PIPELINE wattson_window_markers(
...
|> SELECT ts, dur, 1 AS period_id;

-- Standardized window for the full trace duration
CREATE PERFETTO PIPELINE wattson_window_trace(
  -- Start timestamp of the window
  ts TIMESTAMP,
  -- Duration of the window
  dur DURATION,
...
FROM (SELECT trace_start() AS ts, trace_dur() AS dur, 1 AS period_id);

-- Standardized window for Android app startups
CREATE PERFETTO PIPELINE wattson_window_app_startup(
  -- Start timestamp of the window
  ts TIMESTAMP,
  -- Duration of the window
  dur DURATION,
...
|> SELECT ts, dur, startup_id AS period_id;

-- Standardized window for Android CUJs
CREATE PERFETTO PIPELINE wattson_window_atrace_apps(
  -- Start timestamp of the window
  ts TIMESTAMP,
  -- Duration of the window
  dur DURATION,
...
|> SELECT ts, dur, cuj_id AS period_id;
