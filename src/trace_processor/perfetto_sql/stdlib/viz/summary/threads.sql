...
SELECT utid, count() AS stack_sample_cnt FROM stack_sample GROUP BY utid;
...
WITH
  raw AS (
    SELECT
      ...
      (SELECT stack_sample_cnt FROM _stack_sample_summary WHERE utid = t.utid) AS stack_sample_count
    FROM thread AS t
