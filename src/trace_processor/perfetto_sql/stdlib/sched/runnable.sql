...
CREATE PERFETTO PIPELINE sched_previous_runnable_on_thread(
...
|> SELECT
     id,
     state,
     max(id) FILTER (WHERE state != 'Running')
       OVER (PARTITION BY utid ORDER BY id) AS prev_runnable_id,
     max(id) FILTER (
       WHERE NOT (waker_utid IS NULL)
       AND (irq_context IS NULL OR irq_context != 1)
     ) OVER (PARTITION BY utid ORDER BY id) AS prev_wakeup_runnable_id
|> WHERE state = 'Running'
