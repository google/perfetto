...
-- Grabs the processing frame slices from the Twoshay touch driver, and parses the hardware input event timestamp (IN_TS) and resample offset.
CREATE PERFETTO TABLE _pixel_touch_twoshay_events AS
SELECT
  s.id AS twoshay_slice_id,
  s.ts AS twoshay_ts,
  s.dur AS twoshay_dur,
  CAST(STR_SPLIT(STR_SPLIT(s.name, 'IN_TS=', 1), '.', 0) AS INT64) AS in_ts,
  IFNULL(
    (
      SELECT c.value
      FROM counter AS c
...
-- Maps Twoshay touch events to core Android input events by correlating the hardware timestamp (IN_TS / event_time).
...
WITH
  bh_base AS (
    SELECT
      ...
      dur AS bh_dur,
      track_id,
      CAST(STR_SPLIT(STR_SPLIT(name, 'IRQ_IDX=', 1), '.', 0) AS INT64) AS irq_idx
    FROM slice
    WHERE
...
SELECT
  bh_base.*,
  (
    SELECT CAST(STR_SPLIT(STR_SPLIT(s.name, 'IN_TS=', 1), '.', 0) AS INT64)
    FROM descendant_slice(bh_base.bh_slice_id) AS s
    WHERE
      s.name GLOB 'goog_offload_populate_frame:*'
    LIMIT 1
  ) AS in_ts
FROM bh_base;
...
-- Google Pixel driver-level touch event, including
-- hardware top-half IRQ, bottom-half thread, and Twoshay touch processing.
...
LEFT JOIN _pixel_touch_bottom_half_events AS bh
  ON IFNULL(
    bh.in_ts = t.in_ts,
    ((CAST(bh.in_ts AS INT64) / 1000) * 1000)
    = ((CAST(t.in_ts AS INT64) / 1000) * 1000)
  )
