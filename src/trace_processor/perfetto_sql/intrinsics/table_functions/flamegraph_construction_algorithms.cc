...
struct MergedCallsite {
  std::optional<StringId> frame_name;
  std::optional<StringId> mapping_name;
...
std::vector<MergedCallsite> GetMergedCallsites(TraceStorage* storage,
  ...
  std::optional<StringId> mapping_name =
      mapping_tbl.FindById(frame->mapping())->name();
  std::optional<uint32_t> symbol_set_id = frame->symbol_set_id();

  if (!symbol_set_id) {
    std::optional<StringId> frame_name = frame->name();
  ...
  uint32_t symbol_set_idx = symbols_tbl.FindById(SymbolId(*symbol_set_id))
                                ->ToRowNumber()
                                .row_number();
  for (uint32_t i = symbol_set_idx; i < symbols_tbl.row_count(); ++i) {
    auto symbol = symbols_tbl[i];
    if (symbol.symbol_set_id() != *symbol_set_id) {
      break;
    }
    result.emplace_back(MergedCallsite{symbol.name(), mapping_name,
                                       symbol.source_file(),
                                       symbol.line_number(), std::nullopt});
  }
    ...
    for (MergedCallsite& merged_callsite : callsites) {
      ...
      if (it == merged_callsites_to_table_idx.end()) {
        ...
        if (parent_idx) {
          ...
          row.parent_id = (*tbl)[*parent_idx].parent_id();
        } else {
          row.depth = 0;
        ...
        row.name = merged_callsite.frame_name.value_or(kNullStringId);
        row.map_name = merged_callsite.mapping_name.value_or(kNullStringId);
        tbl->Insert(row);
...
BuildFlamegraphTableHeapSizeAndCount(
  ...
  for (; !it.Eof(); it.Next()) {
    ...
    if (count > 0) {
      ref.set_alloc_count(ref.alloc_count() + size);
    }
...
std::unique_ptr<tables::ExperimentalFlamegraphTable> BuildHeapProfileFlamegraph(
  ...
  // DNS: need to set the filter value for the timestamp and upid.
  cursor.Execute();
  if (cursor.Eof()) {
    return nullptr;
  }
