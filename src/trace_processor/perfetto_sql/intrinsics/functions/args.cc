  ...
  switch (node.GetType()) {
    case ArgNode::Type::kPrimitive: {
      ...
      switch (v.type) {
        ...
        case Variadic::Type::kPointer:
          ss << '"';
          ss << base::Uint64ToHexString(v.pointer_value);
          ss << '"';
          break;
        case Variadic::Type::kJson:
          ss << storage->GetString(v.json_value).c_str();
          break;
      }
      break;
    }
    case ArgNode::Type::kArray: {
      ...
      for (size_t i = 0; i < array.size(); i++) {
        if (i > 0)
          ss << ',';
        WriteAsJson(array[i], storage, ss);
      }
      ss << ']';
      break;
    }
    case ArgNode::Type::kDict: {
      ...
      for (size_t i = 0; i < dict.size(); i++) {
        if (i > 0)
          ss << ',';
        ss << '"';
        ss << dict[i].first.c_str();
        ss << "\":";
        WriteAsJson(dict[i].second, storage, ss);
      }
      ss << '}';
      break;
    }
...
void PrintArgs::Step(sqlite3_context* ctx, int, sqlite3_value** argv) {
  ...
  std::ostringstream ss;
  WriteAsJson(arg_set.root(), storage, ss);
