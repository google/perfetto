...
base::Status GetSourceSchema(PerfettoSqlEngine* engine,
  ...
  // Use PRAGMA table_info for tables, or query with LIMIT 0 for subqueries
  std::string query;
  ...
  if (is_subquery) {
    ...
    // For tables, use PRAGMA
    query = "PRAGMA table_info(" + source_table + ")";
  }
  ...
  if (is_subquery) {
    // For subqueries, get column info from statement
    ...
    for (int i = 0; i < col_count; i++) {
      ...
      // We don't know types for subqueries, leave empty
