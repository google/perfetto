...
struct PipelineModule : sqlite::Module<PipelineModule> {
  ...
  struct Context {
    ...
    // there and drop them at the next execution boundary without an active
    // transaction; locked tables are retried on the following boundary.
  ...
  static void CacheColumnReaders(Cursor*);

  // Surfaces the executor's error, if any, once rows stop.
  static int CheckStatus(Cursor*);
