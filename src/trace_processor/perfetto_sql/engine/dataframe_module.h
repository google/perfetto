...
struct DataframeModule : sqlite::Module<DataframeModule> {
  ...
  struct SqliteValueFetcher final : dataframe::ValueFetcher {
    ...
    int64_t GetInt64Value(uint32_t idx) override {
      return sqlite::value::Int64(sqlite_value[idx]);
    }
    double GetDoubleValue(uint32_t idx) override {
      return sqlite::value::Double(sqlite_value[idx]);
    }
    const char* GetStringValue(uint32_t idx) override {
      return sqlite::value::Text(sqlite_value[idx]);
    }
    Type GetValueType(uint32_t idx) override {
      static_assert(static_cast<Type>(sqlite::Type::kInteger) == Type::kInt64);
      static_assert(static_cast<Type>(sqlite::Type::kFloat) == Type::kDouble);
      static_assert(static_cast<Type>(sqlite::Type::kText) == Type::kString);
      static_assert(static_cast<Type>(sqlite::Type::kBlob) == Type::kBytes);
      static_assert(static_cast<Type>(sqlite::Type::kNull) == Type::kNull);
