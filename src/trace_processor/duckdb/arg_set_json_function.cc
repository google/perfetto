...
void ArgSetToJsonTrampoline(duckdb_function_info info,
                            duckdb_data_chunk in,
                            duckdb_vector out) {
  const auto* converter = static_cast<const ArgSetJsonConverter*>(
  ...
  for (idx_t row = 0; row < n; ++row) {
    ...
    std::optional<std::string> json = (*converter)(in_data[row]);
    if (!json) {
      duckdb_validity_set_row_invalid(out_valid, row);
      continue;
    }
    duckdb_vector_assign_string_element_len(out, row, json->data(),
                                            json->size());
...
base::Status RegisterArgSetJsonFunction(duckdb_connection conn,
  ...
  duckdb_scalar_function_set_name(f, "__intrinsic_arg_set_to_json");
  ...
  duckdb_scalar_function_set_extra_info(
      f, const_cast<ArgSetJsonConverter*>(converter), /*destroy=*/nullptr);
  duckdb_scalar_function_set_function(f, ArgSetToJsonTrampoline);
  ...
  if (st == DuckDBError) {
    return base::ErrStatus(
        "RegisterArgSetJsonFunction: failed to register "
