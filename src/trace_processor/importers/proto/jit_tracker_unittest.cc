...
TEST_F(JitTrackerTest, MappingLargerThanJitRange) {
  ...
  // This should NOT be resolved by JIT cache, and should NOT increment
  // jit_unknown_frame
  EXPECT_THAT(context_.stats_tracker->GetStats(stats::jit_unknown_frame),
              Eq(0));
  ...
  EXPECT_THAT(context_.stats_tracker->GetStats(stats::jit_unknown_frame),
              Eq(0));
  auto frame_normal =
      context_.storage->stack_profile_frame_table()[frame_id_normal];
  EXPECT_THAT(context_.storage->GetString(frame_normal.name()),
              Eq("normal_func"));
...
TEST_F(JitTrackerTest, TwoJitCachesForSameMapping) {
  ...
  const StringId function_name_1 =
      context_.storage->InternString("JitFunction1");
  const StringId source_file = context_.storage->InternString("SourceFile");
  const int64_t create_ts = 12345;
  ...
  const StringId function_name_2 =
      context_.storage->InternString("JitFunction2");
  ...
  auto frame_jit_1 =
      context_.storage->stack_profile_frame_table()[frame_id_jit_1];
  ...
  auto frame_jit_2 =
      context_.storage->stack_profile_frame_table()[frame_id_jit_2];
  ...
  EXPECT_THAT(context_.stats_tracker->GetStats(stats::jit_unknown_frame),
              Eq(0));
  auto frame_normal_2 =
      context_.storage->stack_profile_frame_table()[frame_id_normal_2];
  EXPECT_THAT(context_.storage->GetString(frame_normal_2.name()),
              Eq("normal_func_2"));
