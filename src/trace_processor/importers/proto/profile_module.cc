  ...
  for (auto callstack_it = packet.callstack_iid(); callstack_it;
    ...
    if (!timestamp_it) {
      context_->global_context->storage->IncrementStats(
          stats::stackprofile_parser_error);
      PERFETTO_ELOG(
...
void ProfileModule::ParsePerfSample(
  ...
  const UniqueTid utid =
      context_->machine_context->process_tracker->UpdateThread(sample.tid(),
                                                               sample.pid());
...
void ProfileModule::ParseProfilePacket(
  ...
  for (auto it = packet.process_dumps(); it; ++it) {
    ...
    if (entry.client_error()) {
      context_->global_context->storage->SetIndexedStats(
          stats::heapprofd_client_error, pid, entry.client_error());
    }
    ...
    for (auto sample_it = entry.samples(); sample_it; ++sample_it) {
      ...
      if (entry.heap_name().size != 0) {
        src_allocation.heap_name =
            context_->global_context->storage->InternString(entry.heap_name());
      } else {
...
void ProfileModule::ParseModuleSymbols(ConstBytes blob) {
  ...
  auto mappings = context_->machine_context->mapping_tracker->FindMappings(
      module_symbols.path(), build_id);
  if (mappings.empty()) {
    context_->global_context->storage->IncrementStats(
  ...
  for (auto addr_it = module_symbols.address_symbols(); addr_it; ++addr_it) {
    ...
    for (auto line_it = address_symbols.lines(); line_it; ++line_it) {
      ...
      context_->global_context->storage->mutable_symbol_table()->Insert(
          {symbol_set_id,
           ...
           file_name.size == 0
               ? kNullStringId
               : context_->global_context->storage->InternString(file_name),
    ...
    for (VirtualMemoryMapping* mapping : mappings) {
      context_->trace_context->args_translation_table
          ->AddNativeSymbolTranslationRule(
      ...
      for (const FrameId frame_id : frame_ids) {
        auto* frames = context_->global_context->storage
...
void ProfileModule::ParseSmapsPacket(int64_t ts, ConstBytes blob) {
  ...
  auto upid =
      context_->machine_context->process_tracker->GetOrCreateProcess(sp.pid());
  ...
  for (auto it = sp.entries(); it; ++it) {
    ...
    context_->global_context->storage->mutable_profiler_smaps_table()->Insert(
        {upid, ts, context_->global_context->storage->InternString(e.path()),
         static_cast<int64_t>(e.size_kb()),
         static_cast<int64_t>(e.private_dirty_kb()),
         static_cast<int64_t>(e.swap_kb()),
         context_->global_context->storage->InternString(e.file_name()),
         static_cast<int64_t>(e.start_address()),
         static_cast<int64_t>(e.module_timestamp()),
         context_->global_context->storage->InternString(e.module_debugid()),
         context_->global_context->storage->InternString(e.module_debug_path()),
...
void ProfileModule::NotifyEndOfFile() {
  for (auto it =
           context_->global_context->storage->stack_profile_mapping_table()
               .IterateRows();
       it; ++it) {
    NullTermStringView path =
        context_->global_context->storage->GetString(it.name());
    NullTermStringView build_id =
        context_->global_context->storage->GetString(it.build_id());
