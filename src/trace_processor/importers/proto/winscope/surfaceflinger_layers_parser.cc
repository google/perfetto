...
void SurfaceFlingerLayersParser::Parse(int64_t timestamp,
  ...
  const std::unordered_map<int, protozero::ConstBytes>& layers_by_id =
      surfaceflinger_layers::extractor::ExtractLayersById(layers_decoder);

  const std::vector<protozero::ConstBytes>& layers_top_to_bottom =
      surfaceflinger_layers::extractor::ExtractLayersTopToBottom(
          layers_decoder);

  std::unordered_map<int, surfaceflinger_layers::VisibilityProperties>
      computed_visibility =
          surfaceflinger_layers::VisibilityComputation()
              ...
              .SetStringPool(context_->trace_processor_context_->storage
                                 ->mutable_string_pool())
              .Compute();

  const auto& layer_rects =
      surfaceflinger_layers::RectComputation()
          ...
          .SetRectTracker(&(context_->rect_tracker_))
          .SetTransformTracker(&(context_->transform_tracker_))
          .Compute();
...
void SurfaceFlingerLayersParser::TryAddBlockingLayerArgs(
    const std::vector<int>& blocking_layers,
...
SurfaceFlingerLayersParser::InsertDisplayRectRow(
  ...
  if (rect.IsEmpty()) {
    ...
    rect = winscope::geometry::Rect(0, 0, size.w, size.h);
  }
