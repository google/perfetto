...
class SurfaceFlingerLayersParser {
 public:
  explicit SurfaceFlingerLayersParser(winscope::WinscopeContext*);
 ...
 private:
  ...
  void ParseLayer(
      int64_t timestamp,
      protozero::ConstBytes blob,
      const tables::SurfaceFlingerLayersSnapshotTable::Id& snapshot_id,
  ...
  tables::SurfaceFlingerLayerTable::Id InsertLayerRow(
      ...
      const std::unordered_map<int, protozero::ConstBytes>& layers_by_id,
      const surfaceflinger_layers::SurfaceFlingerRects& rects);
  void TryAddBlockingLayerArgs(const std::vector<int>& blocking_layers,
  ...
  void ParseDisplay(
      ...
      const tables::SurfaceFlingerLayersSnapshotTable::Id& snapshot_id,
      int index,
      std::unordered_map<uint32_t, winscope::geometry::Rect>&
  ...
  winscope::WinscopeContext* const context_;
