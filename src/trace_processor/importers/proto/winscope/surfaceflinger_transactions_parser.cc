...
void SurfaceFlingerTransactionsParser::Parse(int64_t timestamp,
  ...
  for (auto it = snapshot_decoder.added_displays(); it; ++it) {
    ParseAddedDisplay(timestamp, *it, snapshot_id);
  }
...
void SurfaceFlingerTransactionsParser::ParseTransaction(
  ...
  if (has_display_changes) {
    for (auto it = transaction_decoder.display_changes(); it; ++it) {
      AddDisplayChangedRow(timestamp, *it, snapshot_id, transaction_id, pid,
                           uid);
    }
  }
}
...
void SurfaceFlingerTransactionsParser::AddDisplayChangedRow(
    int64_t timestamp,
    protozero::ConstBytes blob,
    tables::SurfaceFlingerTransactionsTable::Id snapshot_id,
    uint64_t transaction_id,
  ...
  transaction.transaction_type =
      context_->storage->mutable_string_pool()->InternString("DISPLAY_CHANGED");
...
void SurfaceFlingerTransactionsParser::ParseAddedDisplay(
    int64_t timestamp,
    protozero::ConstBytes blob,
    tables::SurfaceFlingerTransactionsTable::Id snapshot_id) {
  protos::pbzero::DisplayState::Decoder state_decoder(blob);
