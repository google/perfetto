...
inline geometry::TransformMatrix GetTransformFromTypeAndPosition(int type,
                                                                 double x,
                                                                 double y) {
  // IDENTITY
  if (!type) {
    return ApplyPositionToIdentityMatrix(x, y);
  }
  if (IsRotated270(type)) {
    return geometry::TransformMatrix{0, -1, x, 1, 0, y};
  }
  if (IsRotated180(type)) {
    return geometry::TransformMatrix{
        -1, 0, x, 0, -1, y,
    };
  }
  if (IsRotated90(type)) {
...
inline geometry::Rect GetBounds(const LayerDecoder& layer) {
...
inline std::optional<geometry::Rect> GetCroppedScreenBounds(
  ...
  auto screen_bounds_rect = geometry::Rect(screen_bounds);
...
inline geometry::TransformMatrix GetTransformMatrix(
  ...
  if (layer_decoder.has_transform()) {
    ...
    if (transform::IsSimpleTransform(type)) {
      matrix = transform::GetTransformFromTypeAndPosition(type, matrix.tx,
                                                          matrix.ty);
    } else {
...
inline geometry::Rect MakeLayerStackSpaceRect(
  ...
  return geometry::Rect(layer_stack_space_rect);
}
...
inline geometry::TransformMatrix GetTransformMatrix(
  ...
  if (display_decoder.has_transform()) {
    ...
    if (transform::IsSimpleTransform(type)) {
      matrix = transform::GetTransformFromTypeAndPosition(type, 0, 0);
    } else {
      matrix.dsdx = static_cast<double>(transform.dsdx());
      matrix.dtdx = static_cast<double>(transform.dtdx());
      matrix.dsdy = static_cast<double>(transform.dtdy());
