...
TrackEventParser::TrackEventParser(TraceProcessorContext* context,
    ...
    : args_parser_(*context->global_context->descriptor_pool_),
      ...
      counter_name_thread_time_id_(
          context->global_context->storage->InternString("thread_time")),
      ...
      log_message_body_key_id_(context->global_context->storage->InternString(
          "track_event.log_message")),
      ...
      log_message_source_location_file_name_key_id_(
          context->global_context->storage->InternString(
              "track_event.log_message.file_name")),
      log_message_source_location_line_number_key_id_(
          context->global_context->storage->InternString(
              "track_event.log_message.line_number")),
      log_message_priority_id_(context->global_context->storage->InternString(
          "track_event.priority")),
      source_location_function_name_key_id_(
          context->global_context->storage->InternString(
              "source.function_name")),
      source_location_file_name_key_id_(
          context->global_context->storage->InternString("source.file_name")),
      source_location_line_number_key_id_(
          context->global_context->storage->InternString("source.line_number")),
      raw_legacy_event_id_(context->global_context->storage->InternString(
          "track_event.legacy_event")),
      legacy_event_passthrough_utid_id_(
          context->global_context->storage->InternString(
              "legacy_event.passthrough_utid")),
      legacy_event_category_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.category")),
      legacy_event_name_key_id_(
          context->global_context->storage->InternString("legacy_event.name")),
      legacy_event_phase_key_id_(
          context->global_context->storage->InternString("legacy_event.phase")),
      legacy_event_duration_ns_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.duration_ns")),
      legacy_event_thread_timestamp_ns_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.thread_timestamp_ns")),
      legacy_event_thread_duration_ns_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.thread_duration_ns")),
      ...
      legacy_event_use_async_tts_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.use_async_tts")),
      legacy_event_unscoped_id_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.unscoped_id")),
      legacy_event_global_id_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.global_id")),
      legacy_event_local_id_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.local_id")),
      legacy_event_id_scope_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.id_scope")),
      legacy_event_bind_id_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.bind_id")),
      legacy_event_bind_to_enclosing_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.bind_to_enclosing")),
      legacy_event_flow_direction_key_id_(
          context->global_context->storage->InternString(
              "legacy_event.flow_direction")),
      histogram_name_key_id_(context->global_context->storage->InternString(
          "chrome_histogram_sample.name")),
      flow_direction_value_in_id_(
          context->global_context->storage->InternString("in")),
      flow_direction_value_out_id_(
          context->global_context->storage->InternString("out")),
      flow_direction_value_inout_id_(
          context->global_context->storage->InternString("inout")),
      chrome_legacy_ipc_class_args_key_id_(
          context->global_context->storage->InternString("legacy_ipc.class")),
      chrome_legacy_ipc_line_args_key_id_(
          context->global_context->storage->InternString("legacy_ipc.line")),
      chrome_host_app_package_name_id_(
          context->global_context->storage->InternString(
              "chrome.host_app_package_name")),
      chrome_crash_trace_id_name_id_(
          context->global_context->storage->InternString(
              "chrome.crash_trace_id")),
      chrome_process_label_flat_key_id_(
          context->global_context->storage->InternString(
              "chrome.process_label")),
      chrome_process_type_id_(context_->global_context->storage->InternString(
          "chrome.process_type")),
      event_category_key_id_(
          context_->global_context->storage->InternString("event.category")),
      event_name_key_id_(
          context_->global_context->storage->InternString("event.name")),
      correlation_id_key_id_(
      ...
      chrome_string_lookup_(context->global_context->storage.get()),
...
UniquePid TrackEventParser::ParseProcessDescriptor(
  ...
  if (decoder.has_chrome_process_type()) {
    ...
    // Don't override system-provided names.
    context_->machine_context->process_tracker->SetProcessNameIfUnset(upid,
                                                                      name_id);
  }
  int label_index = 0;
  for (auto it = decoder.process_labels(); it; it++) {
    StringId label_id = context_->global_context->storage->InternString(*it);
    ...
    context_->machine_context->process_tracker->AddArgsTo(upid).AddArg(
        chrome_process_label_flat_key_id_,
        context_->global_context->storage->InternString(base::StringView(key)),
...
void TrackEventParser::ParseChromeProcessDescriptor(
  ...
  ArgsTracker::BoundInserter process_args =
      context_->machine_context->process_tracker->AddArgsTo(upid);
...
UniqueTid TrackEventParser::ParseThreadDescriptor(
  ...
  if (decoder.has_thread_name() && decoder.thread_name().size) {
    name_id =
        context_->global_context->storage->InternString(decoder.thread_name());
...
void TrackEventParser::ParseTrackEvent(int64_t ts,
  ...
  if (context_->global_context->config.drop_track_event_data_before ==
