...
TEST_F(WattsonSocModelModuleTest, ParseWattsonSocModel) {
  ...
  // Policy config for policy 0: no deep idle offset, vote by power
  {
    auto* pc = topology->add_policy_configs();
    pc->set_policy(0);
    pc->set_deep_idle_offset_ns(0);
  }
  ...
  // 1D power curve: policy 0 @ 1000000 kHz
  {
    ...
    c1d->set_static_mw(5.5f);
    c1d->set_active_mw(45.0f);
    protozero::PackedFixedSizeInt<float> idle;
    idle.Append(1.25f);
    idle.Append(0.125f);
    c1d->set_idle_mw(idle);
  }

  // 1D power curve for a policy whose curve set only characterises the
  // shallowest idle level: the deeper one has no entry in the array.
  {
    auto* c1d = model->add_cpu_1d_curves();
    c1d->set_policy(4);
    c1d->set_freq_khz(2000000);
    c1d->set_static_mw(9.5f);
    c1d->set_active_mw(120.0f);
    protozero::PackedFixedSizeInt<float> idle;
    idle.Append(3.75f);
    c1d->set_idle_mw(idle);
  }
  ...
  // 2D power curve: policy 7 @ 3000000 kHz, dep_policy 4 @ 2000000 kHz
  {
    ...
    c2d->set_static_mw(25.0f);
    c2d->set_active_mw(350.0f);
    protozero::PackedFixedSizeInt<float> idle;
    idle.Append(2.5f);
    idle.Append(0.25f);
    c2d->set_idle_mw(idle);
    c2d->set_interconnect_mw(18.0f);
  }
  ...
  // L3 curve
  {
    ...
    l3->set_l3_hit_mw(12.5f);
    l3->set_l3_miss_mw(34.0f);
  }
  ...
  // GPU curve
  {
    ...
    gpu->set_active_mw(280.0f);
    protozero::PackedFixedSizeInt<float> idle;
    idle.Append(6.0f);
    idle.Append(1.5f);
    gpu->set_idle_mw(idle);
  }
  ...
  const auto& dev_table = storage_->wattson_external_device_info_table();
  ...
  const auto& policy_table = storage_->wattson_external_cpu_policy_table();
  ...
  // 3. Verify Deep Idle Offsets: policy 0's offset fans out to CPUs 0-3, and
  // policy 7's to CPU 7. Policy 4 has no config, so CPUs 4-6 get no row.
  const auto& dio_table = storage_->wattson_external_deep_idle_offset_table();
  ASSERT_EQ(dio_table.row_count(), 5u);
  EXPECT_EQ(dio_table[0].cpu(), 0);
  EXPECT_EQ(dio_table[0].offset_ns(), 0);
  EXPECT_EQ(dio_table[3].cpu(), 3);
  EXPECT_EQ(dio_table[3].offset_ns(), 0);
  EXPECT_EQ(dio_table[4].cpu(), 7);
  EXPECT_EQ(dio_table[4].offset_ns(), 200000);

  // 4. Verify Idle State Map
  const auto& idle_table = storage_->wattson_external_idle_state_map_table();
  ...
  const auto& vbf_table = storage_->wattson_external_vote_by_freq_table();
  ASSERT_EQ(vbf_table.row_count(), 1u);
  EXPECT_EQ(vbf_table[0].cpu(), 7);

  // 6. Verify 1D Curves
  const auto& c1d_table = storage_->wattson_external_curves_cpu_1d_table();
  ASSERT_EQ(c1d_table.row_count(), 2u);
  ...
  EXPECT_THAT(c1d_table[0].idle0_mw(), DoubleEq(1.25));
  EXPECT_THAT(c1d_table[0].idle1_mw(), DoubleEq(0.125));
  // The second curve only characterises the shallowest idle level, so the
  // deeper one stays at 0.
  EXPECT_EQ(c1d_table[1].policy(), 4);
  EXPECT_THAT(c1d_table[1].idle0_mw(), DoubleEq(3.75));
  EXPECT_THAT(c1d_table[1].idle1_mw(), DoubleEq(0.0));

  // 7. Verify 2D Curves
  const auto& c2d_table = storage_->wattson_external_curves_cpu_2d_table();
  ...
  EXPECT_THAT(c2d_table[0].idle0_mw(), DoubleEq(2.5));
  EXPECT_THAT(c2d_table[0].idle1_mw(), DoubleEq(0.25));
  EXPECT_THAT(c2d_table[0].interconnect_mw(), DoubleEq(18.0));

  // 8. Verify L3 Curves
  const auto& l3_table = storage_->wattson_external_curves_l3_table();
  ...
  // 9. Verify GPU Curves. The GPU curve set starts at idle level 1, so the
  // shallowest entry of the array lands in `idle1_mw`.
  const auto& gpu_table = storage_->wattson_external_curves_gpu_table();
  ASSERT_EQ(gpu_table.row_count(), 1u);
  EXPECT_EQ(gpu_table[0].freq_khz(), 800000);
  EXPECT_THAT(gpu_table[0].active_mw(), DoubleEq(280.0));
  EXPECT_THAT(gpu_table[0].idle1_mw(), DoubleEq(6.0));
  EXPECT_THAT(gpu_table[0].idle2_mw(), DoubleEq(1.5));

  // 10. Verify TPU Curves
  const auto& tpu_table = storage_->wattson_external_curves_tpu_table();
...
TEST_F(WattsonSocModelModuleTest, IgnoresDuplicatePackets) {
  protozero::HeapBuffered<protos::pbzero::Trace> trace;
  ...
  // Packet 2: Duplicate / Second packet in the same trace should be ignored
  {
    protozero::HeapBuffered<WattsonSocModel> model;
    model->set_device_name("Tensor G5");
    auto* c1d = model->add_cpu_1d_curves();
    c1d->set_policy(0);
    c1d->set_freq_khz(2000000);
    c1d->set_active_mw(90.0f);
    auto model_bytes = model.SerializeAsArray();
