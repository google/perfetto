...
class HeapGraphTracker : public Destructible {
 public:
  // Represents a discriminator field on an object instance.
  // The value of these fields at runtime is used to disambiguate
  // merged classes.
  struct MergedClassDiscriminator {
    MergedClassDiscriminator() = default;
    MergedClassDiscriminator(StringId name, int32_t val)
        : field_name(name), value(val) {}

    StringId field_name;
    int32_t value = 0;
  };

  using DiscriminatorList = base::SmallVector<MergedClassDiscriminator, 2>;
  ...
  // Holds an object row number and its discriminator fields for optimizer
  // merged class deobfuscation.
  struct DisambiguatedObject {
    tables::HeapGraphObjectTable::RowNumber row_number;
    DiscriminatorList discriminators;
  };

  // Inserts discriminator fields for an object instance belonging to a class.
  // These discriminators (such as synthetic $cid fields generated by
  // optimizers) are used during deobfuscation to disambiguate optimizer merged
  // classes and resolve each instance to its original class type.
  void InsertObjectDiscriminators(tables::HeapGraphClassTable::Id class_id,
                                  tables::HeapGraphObjectTable::RowNumber row,
                                  DiscriminatorList discriminators);

  const std::vector<DisambiguatedObject>* ObjectsForMergedClass(
      tables::HeapGraphClassTable::Id class_id) const {
    return class_to_disambiguated_objects_.Find(class_id);
  }

  void ClearDisambiguatedObjects() { class_to_disambiguated_objects_.Clear(); }
 ...
 private:
  ...
  // Ephemeral map from class ID to its optimizer merged class disambiguated
  // objects and their discriminator field values: (field_name, field_value).
  // Cleared after deobfuscation is complete.
  base::FlatHashMap<tables::HeapGraphClassTable::Id,
                    std::vector<DisambiguatedObject>>
      class_to_disambiguated_objects_;
};
