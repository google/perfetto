...
base::Status RecordParser::ParseFork(int64_t, Record record) {
  ...
  uint32_t tid;
  uint32_t parent_tid;
  if (!reader.Read(pid) || !reader.Read(parent_pid) || !reader.Read(tid) ||
      !reader.Read(parent_tid)) {
    return base::ErrStatus("Failed to parse PERF_RECORD_FORK");
  }

  if (pid != parent_pid) {
    UniquePid parent_upid =
        context_->process_tracker->GetOrCreateProcess(parent_pid);
