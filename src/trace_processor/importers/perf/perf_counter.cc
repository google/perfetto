...
tables::CounterTable::Id PerfCounter::AddCount(int64_t ts,
  ...
  // In thread-targeted mode, perf_event_open creates a separate file
  // descriptor per (tid, cpu) pair, each with its own independent counter
  // baseline starting at 0. When a thread migrates to a different CPU, raw
  // counter values reflect the new CPU's fd baseline rather than the
  // previous CPU's cumulative total. Track per-CPU baselines to compute each
  // CPU's incremental delta and accumulate into the thread total.
  if (cpu.has_value()) {
