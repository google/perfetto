 ...
 public:
  SchedEventTrackerTest() {
    ...
    context.machine_context->sched_event_tracker =
        std::make_unique<SchedEventTracker>(&context);
...
TEST_F(SchedEventTrackerTest, InsertSecondSched) {
  ...
  ASSERT_EQ(context.global_context->storage->sched_slice_table().row_count(),
            1ul);
  ...
  const auto& sched = context.global_context->storage->sched_slice_table();
  ASSERT_EQ(sched[0].ts(), timestamp);
  ASSERT_EQ(context.global_context->storage->thread_table()[1].start_ts(),
            std::nullopt);

  auto name = context.global_context->storage->GetString(
      *context.global_context->storage->thread_table()[1].name());
  ASSERT_STREQ(name.c_str(), kCommProc1);
  ASSERT_EQ(context.global_context->storage->sched_slice_table()[0].utid(), 1u);
  ASSERT_EQ(context.global_context->storage->sched_slice_table()[0].dur(), 1);
}
...
TEST_F(SchedEventTrackerTest, InsertThirdSched_SameThread) {
  ...
  ASSERT_EQ(context.global_context->storage->sched_slice_table()[0].ts(),
            timestamp);
  ASSERT_EQ(context.global_context->storage->thread_table()[1].start_ts(),
            std::nullopt);
  ASSERT_EQ(context.global_context->storage->sched_slice_table()[0].dur(), 1u);
  ASSERT_EQ(context.global_context->storage->sched_slice_table()[1].dur(),
            11u - 1u);
  ASSERT_EQ(context.global_context->storage->sched_slice_table()[2].dur(),
            31u - 11u);
  ASSERT_EQ(context.global_context->storage->sched_slice_table()[0].utid(),
            context.global_context->storage->sched_slice_table()[2].utid());
}
...
TEST_F(SchedEventTrackerTest, UpdateThreadMatch) {
  ...
  context.machine_context->process_tracker->SetProcessMetadata(
      2, std::nullopt, "test", base::StringView());
  context.machine_context->process_tracker->UpdateThread(4, 2);

  ASSERT_EQ(context.global_context->storage->thread_table()[1].tid(), 4u);
  ASSERT_EQ(context.global_context->storage->thread_table()[1].upid().value(),
            1u);
  ASSERT_EQ(context.global_context->storage->process_table()[1].pid(), 2u);
  ASSERT_EQ(context.global_context->storage->process_table()[1].start_ts(),
            std::nullopt);
}
