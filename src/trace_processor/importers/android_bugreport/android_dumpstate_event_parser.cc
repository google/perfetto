...
AndroidDumpstateEventParser::ProcessBatteryStatsHistoryEvent(
  ...
  const int32_t uid = tracker_->GetUid(hsp_index);
  const std::string& event_str = tracker_->GetString(hsp_index);
...
AndroidDumpstateEventParser::ProcessBatteryStatsHistoryWakeLocks(
  ...
  if (tracker_->battery_stats_version() < 36) {
  ...
  if (item.prefix == "+") {
    StringId name_id =
        context_->storage->InternString(tracker_->GetString(hsp_index));
