...
// 1. Buffers incoming TraceBlobView chunks until the trace start time can be extracted.
// 2. Extracts TraceEdge messages as TraceBlobView slices, and calculates their absolute timestamps using the start time.
// 3. Pushes (timestamp, TraceBlobView) pairs to the TraceSorter stream for full parsing.
base::Status PrimesTraceTokenizer::Parse(TraceBlobView blob) {
  ...
  if (!slice.has_value()) {
    return base::ErrStatus("Slicing TraceBlobView for Primes trace proto unexpectedly failed.");
  }
  auto decoder = protozero::ProtoDecoder(slice->data(), slice->size());

  // Start time needs to be extracted before the timestamp of any edge can be calculated, as edge
  // timestamps are stored as an offset to the trace start time.
  if (!start_time_) {
  ...
  while(field.valid()) {
    ...
    // To send to the trace parser we need to create a TraceBlobView slice of the edge, and
    // extract its timestamp relative to start_time_.
    if (field.id() == kEdgesFieldNumber) {
      ...
      auto start_offset_field =
          protozero::ProtoDecoder(field_bytes.data, field_bytes.size)
            ...
            .as_bytes();
      auto ts_decoder = protozero::ProtoDecoder(start_offset_field.data, start_offset_field.size);
      ...
      int64_t edge_timestamp = start_time_ +
          unify_time(seconds_field.as_int64(), nanos_field.as_int32());

      TraceBlobView edge_slice = slice->slice_off(field_start_offset, field_end_offset - field_start_offset);
...
base::Status PrimesTraceTokenizer::NotifyEndOfFile() {
  if (!start_time_) {
    return base::ErrStatus("Did not find a valid trace start time before EOF. Malformed Primes trace proto?");
  }
  return base::OkStatus();
}
