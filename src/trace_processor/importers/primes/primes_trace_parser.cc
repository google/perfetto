...
#include "include/perfetto/base/compiler.h"
...
PrimesTraceParser::PrimesTraceParser(TraceProcessorContext* ctx) : context_(ctx) {}
...
void PrimesTraceParser::HandleSliceBegin(
  ...
  // Primes executors are mapped to Perfetto tracks (with each track having its own thread).
  TrackId track_id;

  // If this edge has its own executor ID that means it's the root slice for the executor.
  // Create a new thread and track for it.
  // Otherwise, inherit the executor ID from the parent slice.
  if (sb_decoder.has_executor_id()) {
    UniqueTid utid = context_->process_tracker->GetOrCreateThread(
        (int64_t)sb_decoder.executor_id());
    StringId executor_name = context_->storage->InternString(sb_decoder.executor_name());
    ...
    // The parent ought to have already been processed. If not, log an error and return.
    auto it = parsing_state_.edge_id_to_track_id_map.find(details_decoder.parent_id());
  ...
  if (!slice_id) {
    PERFETTO_ELOG("Failed to begin slice for edge_id=%" PRIu64, edge_decoder.id());
    return;
  }
...
void PrimesTraceParser::HandleSliceEnd(int64_t ts,
                                     primespb::TraceEdge_Decoder& edge_decoder) {
...
void PrimesTraceParser::HandleMark(int64_t ts,
  ...
  auto details_decoder =
      primespb::TraceEdge_TraceEntityDetails_Decoder(mark_decoder.entity_details());
  ...
  std::optional<SliceId> slice_id =
    context_->slice_tracker->Scoped(ts, track_id, kNullStringId, slice_name, 0);
  if (!slice_id) {
    PERFETTO_ELOG("Failed to add mark slice for edge_id=%" PRIu64, edge_decoder.id());
    return;
  }
  parsing_state_.edge_id_to_slice_id_map[edge_decoder.id()] = slice_id.value();
...
// e.g., A -> B -> C).
//
// For follows_from: Creates a direct flow from the leader slice to the current slice.
// For flow_ids: Manages the flow chain state (Begin/Step) to link the current
// slice to the previous slice in the same flow chain.
