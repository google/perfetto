...
MappingId CreateMapping(TraceProcessorContext* context,
  ...
  StringId build_id =
      context->global_context->storage->InternString(base::StringView(
          params.build_id ? params.build_id->ToHex() : std::string()));
  MappingId mapping_id =
      context->global_context->storage->mutable_stack_profile_mapping_table()
          ->Insert({build_id, static_cast<int64_t>(params.exact_offset),
...
FrameId VirtualMemoryMapping::InternFrame(uint64_t rel_pc,
  ...
  if (was_inserted) {
    ...
    context_->trace_context->stack_profile_tracker->OnFrameCreated(frame_id);
  }
  return frame_id;
}
...
std::pair<FrameId, bool> VirtualMemoryMapping::InternFrameImpl(
  ...
  const FrameKey frame_key{
      rel_pc, context_->global_context->storage->InternString(function_name)};
...
FrameId DummyMemoryMapping::InternDummyFrame(base::StringView function_name,
  ...
  DummyFrameKey key{
      context()->global_context->storage->InternString(function_name),
      context()->global_context->storage->InternString(source_file)};
  ...
  uint32_t symbol_set_id =
      context()->global_context->storage->symbol_table().row_count();

  tables::SymbolTable::Id symbol_id =
      context()
          ->global_context->storage->mutable_symbol_table()
  ...
  const FrameId frame_id =
      context()
          ->global_context->storage->mutable_stack_profile_frame_table()
