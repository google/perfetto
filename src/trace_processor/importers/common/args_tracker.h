...
class ArgsTracker {
  ...
  class BoundInserter {
   public:
    ...
    virtual BoundInserter& AddArg(
        ...
        Variadic v,
        UpdatePolicy update_policy = UpdatePolicy::kAddOrUpdate) {
      args_tracker_->AddArg(dataframe_, column_, row_, flat_key, key, v,
                            update_policy);
      return *this;
    }
    ...
    size_t GetNextArrayEntryIndex(StringId key) {
      ...
      return args_tracker_
          ->array_indexes_[std::make_tuple(dataframe_, column_, row_, key)];
    }
   ...
   private:
    ...
    dataframe::Dataframe* dataframe_ = nullptr;
    uint32_t column_ = 0;
    uint32_t row_ = 0;
  };
 ...
 private:
  ...
  BoundInserter AddArgsTo(Table* table, typename Table::Id id) {
    static_assert(!Table::kSpec.column_specs[Table::ColumnIndex::arg_set_id]
                       .nullability.template Is<dataframe::SparseNull>());
    uint32_t row = table->FindById(id)->ToRowNumber().row_number();
    return BoundInserter(this, &table->dataframe(),
                         Table::ColumnIndex::arg_set_id, row);
  }

  void AddArg(dataframe::Dataframe* dataframe,
              uint32_t column,
              uint32_t row,
              StringId flat_key,
              StringId key,
  ...
  using ArrayKeyTuple = std::tuple<dataframe::Dataframe* /* dataframe */,
                                   uint32_t /* col */,
                                   uint32_t /*row*/,
                                   StringId /*key*/>;
