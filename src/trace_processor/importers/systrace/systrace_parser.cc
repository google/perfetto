...
void SystraceParser::ParseSdeTracingMarkWrite(int64_t ts,
  ...
  // SDE events come from the kernel so we can assume that they are always
  // generated by kernel threads. As we always make all kernel threads associate
  // to the kthreadd process in trace processor, do the same here instead of
  // using the tgid from the event.
  // TODO(lalitm): remove this hack when the UI has support for grouping kernel
  // threads (see b/136635390).
  point.tgid = kKthreaddPid;
  point.value = value;
