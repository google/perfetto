...
GenericKernelParser::GenericKernelParser(TraceProcessorContext* context)
    : context_(context),
      created_string_id_(
          context_->global_context->storage->InternString("Created")),
      running_string_id_(
          context_->global_context->storage->InternString("Running")),
      dead_string_id_(context_->global_context->storage->InternString("Z")),
      destroyed_string_id_(
          context_->global_context->storage->InternString("X")),
      task_states_({
          context_->global_context->storage->InternString("Unknown"),
          context_->global_context->storage->InternString("Created"),
          context_->global_context->storage->InternString("R"),
          context_->global_context->storage->InternString("Running"),
          context_->global_context->storage->InternString("S"),
          context_->global_context->storage->InternString("D"),
          context_->global_context->storage->InternString("T"),
          context_->global_context->storage->InternString("Z"),
          context_->global_context->storage->InternString("X"),
      }) {}
...
void GenericKernelParser::ParseGenericTaskStateEvent(
  ...
  StringId comm_id =
      context_->global_context->storage->InternString(task_event.comm());
...
std::optional<UniqueTid> GenericKernelParser::GetUtidForState(int64_t ts,
  ...
  switch (state) {
    case TaskStateEnum::TASK_STATE_CREATED: {
      if (context_->machine_context->process_tracker->GetThreadOrNull(tid)) {
        context_->global_context->storage->IncrementStats(
            stats::generic_task_state_invalid_order);
        return std::nullopt;
      }
      UniqueTid utid =
          context_->machine_context->process_tracker->StartNewThread(ts, tid);
    ...
    case TaskStateEnum::TASK_STATE_DESTROYED: {
      return context_->machine_context->process_tracker->GetThreadOrNull(tid);
    }
    case TaskStateEnum::TASK_STATE_DEAD: {
      auto utid_opt =
          context_->machine_context->process_tracker->GetThreadOrNull(tid);
      if (!utid_opt) {
        utid_opt =
      ...
      context_->machine_context->process_tracker->EndThread(ts, tid);
      return utid_opt;
    }
...
GenericKernelParser::SchedSwitchType GenericKernelParser::PushSchedSwitch(
  ...
  if (state_string_id == running_string_id_) {
    ...
    if (pending_slice_idx < std::numeric_limits<uint32_t>::max()) {
      context_->machine_context->sched_event_tracker->ClosePendingSlice(
          pending_slice_idx, ts, kNullStringId);
  ...
  if (pending_slice_idx < std::numeric_limits<uint32_t>::max() &&
    ...
    context_->machine_context->sched_event_tracker->ClosePendingSlice(
        pending_slice_idx, ts, state_string_id);
  ...
  if (hanging_sched.has_value()) {
    ...
    auto close_ts = context_->machine_context->sched_event_tracker
                        ->GetEndTimestampForPendingSlice(sched_slice_idx);
    if (ts == close_ts) {
      context_->machine_context->sched_event_tracker
          ->SetEndStateForPendingSlice(sched_slice_idx, state_string_id);
...
void GenericKernelParser::ParseGenericTaskRenameEvent(
  ...
  StringId comm =
      context_->global_context->storage->InternString(task_rename_event.comm());
  context_->machine_context->process_tracker
      ->UpdateThreadNameAndMaybeProcessName(
          task_rename_event.tid(), comm,
