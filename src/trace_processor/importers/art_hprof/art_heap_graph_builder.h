...
class HprofObject {
 public:
  ...
  bool is_reachable() const { return is_reachable_; }
...
class HeapGraphBuilder {
 public:
  ...
  // Parses the chunk of HPROF file from the iterator
  bool Parse();

  // Builds and return the final heap graph
 ...
 private:
  ...
  // Utility methods
  std::vector<Field> GetFieldsForClassHierarchy(uint64_t class_id) const;
  size_t GetFieldTypeSize(FieldType type) const;
  std::string GetString(uint64_t id) const;
  static uint64_t ExtractIdFromBytes(const std::vector<uint8_t>& data,
                                     size_t offset,
                                     uint32_t id_size);
