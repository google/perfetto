...
void EtmTracker::AddSessionData(tables::EtmV4SessionTable::Id session_id,
  ...
  for (auto& trace : traces) {
    auto trace_id =
        context_->global_context->storage->mutable_etm_v4_trace_table()
            ->Insert(
                {session_id, trace_set_id, static_cast<int64_t>(trace.size())})
            .id;
...
EtmTracker::InsertEtmV4Config(PerCpuConfiguration per_cpu_configs) {
  ...
  for (auto it = per_cpu_configs.GetIterator(); it; ++it) {
    ...
    row.core_profile = context_->global_context->storage->InternString(
        ToString(etm_v4_config.coreProfile()));
    row.arch_version = context_->global_context->storage->InternString(
        ToString(etm_v4_config.archVersion()));
    ...
    auto id =
        context_->global_context->storage->mutable_etm_v4_configuration_table()
            ->Insert(row)
            .id;
    res.insert(id);
