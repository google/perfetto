...
TEST(ExecutorContractTest, FirstSurvivorDoesNotTriggerLookahead) {
  ...
  Pipeline pipeline(source, std::move(ops));
  {
    ...
    // Consumer stops after one row. This tests current pull laziness, not a
    // future LIMIT operator or propagated executor demand.
  }
  EXPECT_EQ(source.pulls, 2u);
}
...
// Computed totals are published with an owner today, but the next Execute
// overwrites that same allocation. Retention must prevent this reuse.
...
// Differential selection test: materialized scalar rows are the independent
// oracle. Mixed columns have different physical mappings; indices may repeat.
...
TEST(ExecutorContractTest, PoolReusesOnlyReleasedBuffersAndTrimsUnusedStorage) {
  ...
  std::weak_ptr<CountedBuffer> lifetime = retained;
  pool.Trim();
  EXPECT_FALSE(lifetime.expired());
  retained.reset();
  pool.Trim();
