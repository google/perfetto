...
// Hands `out` the input plus a column of `totals`.
...
OpResult TreeAccumulateUp::Execute(const RowBatch& in,
  ...
  for (uint32_t row = 0; row < count; ++row) {
    ...
    Grow(&s.by_node, node);
    // Everything below this node has already gone past and left what it owed
    // here, so the total is settled the moment the node arrives.
