...
// Turns whatever a relation calls a row and its parent into node numbers,
// handed out densely from zero in order of first sighting.
//
// Everything downstream of this deals in node numbers and never learns what
// an id was, which is why it is the only operator that has to know: an id is
// whatever the relation stored, of any width, and a filtered relation's ids
// are a scattering. Numbering them makes an array indexed by node the size of
// the input rather than of whatever it was filtered out of.
