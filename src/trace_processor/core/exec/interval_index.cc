...
void IntervalIndex::Build() {
  ...
  // than one sort over everything with the lane as the leading key. A single
  // lane is already grouped and takes the entries as they stand.
  auto count = static_cast<uint32_t>(entries_.size());
  std::vector<Entry> grouped;
  if (lanes_.size() == 1) {
    lanes_[0] = {0, count, true};
    grouped = std::move(entries_);
  } else {
  ...
  ends_.resize(count);
  rows_.resize(count);
  // Intervals which arrive in time order, as those read from a table sorted
  // by ts do, are already where they belong.
  auto by_start = [](const Entry& a, const Entry& b) {
    return a.start < b.start;
  };
  for (const Lane& lane : lanes_) {
    auto begin = grouped.begin() + lane.begin;
    auto end = grouped.begin() + lane.end;
    if (!std::is_sorted(begin, end, by_start)) {
      std::stable_sort(begin, end, by_start);
    }
  }
