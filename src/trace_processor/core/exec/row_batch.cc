...
bool RowBatch::AdoptPhysicalRows(Span<const uint32_t> rows) {
  ...
  for (TransientColumn& vector : columns_) {
    vector.SetBorrowedRows(rows);
  }
  cardinality_ = count;
  return true;
}
