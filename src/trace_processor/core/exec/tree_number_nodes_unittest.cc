...
// One batch of ids and parent ids of whatever type, run through the operator.
template <typename T>
struct Numbered {
...
// A filtered relation's ids are a scattering; numbering makes an array
// indexed by node the size of the input.
...
// An Id column has no storage: the value is the row it sits at.
...
// Numbers are handed out once and stay put across batches.
