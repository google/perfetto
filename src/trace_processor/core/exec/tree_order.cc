...
void TreeOrder::Note(State& s,
  ...
  s.has_row.set(node);
  if (buffering) {
    if (s.row_of_node.size() < s.nodes_seen) {
...
// only has to tell which order they arrived in, so the arrays a sort would
// need are not built at all: a streaming fold keeps a bit per node and nothing
// per row.
bool TreeOrder::Consume(RowBatch& batch, State& s, bool buffering) const {
...
bool TreeOrder::Fill(State& s) const {
  while (input_.GetData(s.input_batch, *s.input)) {
    if (!Consume(s.input_batch, s, /*buffering=*/true)) {
      return false;
    }
  }
...
bool TreeOrder::GetData(RowBatch& out, OperatorState& state) const {
  ...
  if (mode_ == Mode::kStreaming) {
    ...
    if (!Consume(out, s, /*buffering=*/false)) {
      return false;
    }
