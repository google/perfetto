...
//
// The input streams through and must arrive ordered by key columns then ts
// within each batch, as BatchOrder leaves it: each run of one key then walks
// the operand's lane for that key once, in step with the probes. Only the ts
// order within a run is enforced; a key which comes back starts a new run,
// which walks its lane again. The operand
// is read in full the first time it is needed and indexed by key, so it should
// be the smaller side. A negative duration is an interval which never ends.
//
// Output rows are the input columns followed by the operand columns, in the
// order of the input rows within a batch; the matches of one input row are
// ordered by operand start, then by operand row.
