...
TEST(TreeOrderTest, RowsInTheOtherOrderAreTurnedRound) {
  ...
  EXPECT_THAT(out.payload, ElementsAre(100, 101, 102, 103));
  // Every parent now comes out before its children.
...
// Without being told the input's order, all the rows have to be buffered: by
// the time a row disproves a guess, the rows before it have already gone.
...
TEST(TreeOrderTest, ChildFirstIsADepthFirstPostOrder) {
  std::mt19937 rng(29);
  std::vector<Row> rows;
...
// A tree big enough to span several batches, with its rows shuffled.
