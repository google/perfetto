...
std::vector<Pair> Join(std::vector<Row> input,
  ...
  operators.push_back(
      std::make_unique<IntervalJoin>(operand_source, std::move(spec)));
  Pipeline pipeline(input_source, std::move(operators));
...
TEST(IntervalJoin, NegativeDurationNeverEnds) {
  ...
  EXPECT_THAT(pairs, ElementsAre(Pair(1, 100), Pair(2, 100), Pair(2, 101)));
}
...
TEST(IntervalJoin, KeepUnmatched) {
  ...
  EXPECT_THAT(pairs, ElementsAre(Pair(1, std::nullopt), Pair(2, 100),
                                 Pair(3, std::nullopt)));
}
...
TEST(IntervalJoin, OneRowWithMoreMatchesThanABatchHolds) {
  ...
  ASSERT_EQ(pairs.size(), 1u + kOperandRows + 2u);
...
// join emits them: by input row, then operand start, then operand row.
...
TEST(IntervalJoin, MatchesTheDefinitionsOnRandomIntervals) {
  ...
  for (R relationship : kRelationships) {
    ...
    EXPECT_EQ(Join(input, operand, Spec(relationship, true, false), 64),
              Expected(relationship, input, operand))
...
TEST(IntervalJoin, ADisjointLaneCanEndInARowWhichNeverEnds) {
  std::vector<Pair> pairs =
      Join({Interval(3, 0, 1), Interval(12, 100, 2), Interval(9, 2, 3)},
           ...
           Spec(R::kOverlappingBounds, false, false));
  EXPECT_THAT(pairs, ElementsAre(Pair(1, 100), Pair(2, 102), Pair(3, 101),
                                 Pair(3, 102)));
}
