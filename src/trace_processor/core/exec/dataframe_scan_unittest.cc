...
// Bigger than an int32, so a column holding one is stored as an int64.
constexpr int64_t kBig = int64_t{1} << 40;

class DataframeScanTest : public ::testing::Test {
 protected:
  // A dataframe of one column, nullable where `present` says otherwise. The
  // values are big so that the builder settles the column on int64 and the
  // test can read it as one.
...
// The point of it: the batch reads the dataframe's own storage.
...
TEST_F(DataframeScanTest, AColumnWithoutASlotPerRowIsExpanded) {
  ...
  // Readable everywhere, whether or not the row holds anything.
  EXPECT_EQ(data[1], 0);
