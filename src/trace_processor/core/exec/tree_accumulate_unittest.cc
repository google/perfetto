...
// Hands over id, parent id and a value, in whatever order it was given them.
class RowSource final : public Source {
 public:
  // `order` is the order the rows are handed over in, which is not the order
...
// The definitions, written out. Up is everything below a node; down is
// everything above it.
...
// Runs the whole thing and returns the totals by id, plus how many batches
// came out.
struct Result {
...
Result Accumulate(const std::vector<int64_t>& parent,
  ...
  // ids become node numbers before anything else looks at them; from here on
  // nothing knows what an id was.
...
// Nothing is held once the rows arrive the right way round: a batch goes out
// for every batch that comes in. Holding, when it happens, is the ordering
// operator's doing and not the fold's.
...
// A running total carried between batches has to be thrown away when the plan
// is run again.
...
// Rows in no order at all have to be sorted before either fold can read them,
// which is the ordering operator's job and not this one's.
