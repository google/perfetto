...
// An operator which hands the same input back twice, standing in for one
// whose output does not fit an input batch.
...
// One input batch can make more than one output batch: the operator says so
// and is asked again with the same input.
...
// The deepest operator with more to give is drained before the one above it
// is asked again.
