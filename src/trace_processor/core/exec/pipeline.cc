...
// Whether the chunk still has rows for the consumer once every operator has
// seen it.
bool ExecuteOperators(RowBatch& chunk,
                      const std::vector<std::unique_ptr<Operator>>& operators) {
  if (chunk.size() == 0) {
    return false;
  }
  for (const std::unique_ptr<Operator>& op : operators) {
    if (op->Execute(chunk) == OpResult::kDrop || chunk.size() == 0) {
      return false;
    }
  }
  return true;
