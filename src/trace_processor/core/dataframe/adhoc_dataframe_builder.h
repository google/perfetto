...
class AdhocDataframeBuilder {
 ...
 private:
  ...
  // Maps value type T to the corresponding core storage type tag.
  template <typename T>
  using StorageTypeFor = std::conditional_t<
      std::is_same_v<T, int64_t>,
      Int64,
      std::conditional_t<std::is_same_v<T, double>,
                         Double,
                         std::conditional_t<std::is_same_v<T, StringPool::Id>,
                                            String,
                                            void>>>;
  ...
  PERFETTO_ALWAYS_INLINE bool PushNonNullInternal(uint32_t col,
    ...
    using TypeTag = StorageTypeFor<T>;
    auto& state = column_states_[col];
    if (!state.storage) {
