...
void UpdateDur(int64_t new_ts, uint32_t row, tables::ThreadStateTable* table) {
...
std::unique_ptr<Table> ThreadStateGenerator::ComputeTable(
  ...
  if (!unsorted_thread_state_table_) {
    ...
    // We explcitily sort by ts here as ComputeThreadStateTable does not insert
...
void ThreadStateGenerator::AddWakingEvent(
  ...
  if (it != state_map.end()) {
    ...
    //    event).
    // 2. We're close to the end of the trace and we missed the switch out
    //    event for a thread but we see a waking after (as wakings can happen
    //    on a difference CPU and we're not guaranteed to see up to the same
    //    timestamp on all cpus).
    // Either way, we should just drop the waking.
    ...
    if (table->state()[it->second] == running_string_id_) {
      PERFETTO_DCHECK(table->dur()[it->second] == -1);
      return;
    }

    // If neither of these cases hold, update the duration on the existing
    // event.
    if (it != state_map.end()) {
      UpdateDur(ts, it->second, table);
    }
