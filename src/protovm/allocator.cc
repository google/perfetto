...
StatusOr<Node::Bytes> Allocator::AllocateAndCopyBytes(
  ...
  auto copy = UniquePtr<void>{malloc(data.size)};
  if (!copy) {
    PERFETTO_ABORT("Failed to malloc %zu bytes", data.size);
  }
...
void Allocator::DeallocateBytes(UniquePtr<void> p, size_t size) {
  free(p.release());
