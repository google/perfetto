...
StatusOr<void> RoCursor::EnterField(uint32_t field_id) {
  ...
  PERFETTO_RETURN_IF_STATUS_NOT_OK(status_or_data);
...
StatusOr<void> RoCursor::EnterRepeatedFieldByIndex(uint32_t field_id,
                                                   uint32_t index) {
  auto status_or_data = GetLengthDelimitedData();
  PERFETTO_RETURN_IF_STATUS_NOT_OK(status_or_data);
...
StatusOr<Scalar> RoCursor::GetScalar() const {
  if (!IsScalar()) {
    PERFETTO_ABORT("Attempted to access length-delimited field as a scalar");
  }
