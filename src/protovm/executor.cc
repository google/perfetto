...
StatusOr<void> Executor::EnterField(Cursors& cursors, uint32_t field_id) const {
  ...
  PERFETTO_RETURN_IF_STATUS_NOT_OK(has_field);
...
StatusOr<void> Executor::EnterRepeatedFieldByIndex(Cursors& cursors,
                                                   uint32_t field_id,
                                                   uint32_t index) const {
  if (cursors.selected == CursorEnum::SRC) {
    return cursors.src.EnterRepeatedFieldByIndex(field_id, index);
  }

  return cursors.dst.EnterRepeatedFieldByIndex(field_id, index);
}
...
StatusOr<uint64_t> Executor::ReadRegister(uint8_t reg_id) const {
  if (reg_id >= registers_.size()) {
    PERFETTO_ABORT("Register (id = %d) is out of bounds", reg_id);
  }
  if (!registers_[reg_id]) {
    PERFETTO_ABORT("Register (id = %d) is not initialized)", reg_id);
  }
...
StatusOr<void> Executor::WriteRegister(const Cursors& cursors, uint8_t reg_id) {
  ...
  PERFETTO_RETURN_IF_STATUS_NOT_OK(status_or_scalar);
...
StatusOr<void> Executor::Set(Cursors& cursors) const {
  ...
  PERFETTO_ABORT("Attempted SET operation but src cursor has no valid data");
}
