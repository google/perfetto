...
class RwProtoTest : public ::testing::Test {
 protected:
  ...
  void PopulateRwProtoWithTwoElements() {
    auto root = data_trace_entry_with_two_elements_.Root();

    // elements[0]
    {
      auto element = root;
      element.EnterRepeatedFieldByIndex(
          protos::TraceEntry::kElementsFieldNumber, 0);
    ...
    // elements[1]
    {
      auto element = root;
      element.EnterRepeatedFieldByIndex(
          protos::TraceEntry::kElementsFieldNumber, 1);
...
TEST_F(RwProtoTest, HasField_IncompatibleWireType) {
  RwProto::Cursor cursor = data_trace_entry_with_two_elements_.Root();
  cursor.EnterRepeatedFieldByIndex(protos::TraceEntry::kElementsFieldNumber, 0);
...
TEST_F(RwProtoTest, HasField_FieldNotAvailable) {
  {
    RwProto::Cursor cursor = data_empty_.Root();
...
TEST_F(RwProtoTest, HasField_FieldAvailable) {
  ...
  {
    RwProto::Cursor cursor = data_trace_entry_with_two_elements_.Root();
    cursor.EnterRepeatedFieldByIndex(protos::TraceEntry::kElementsFieldNumber,
                                     0);
...
TEST_F(RwProtoTest, EnterField_IncompatibleWireType) {
  auto cursor = data_trace_entry_with_two_elements_.Root();
  ASSERT_TRUE(cursor
                  .EnterRepeatedFieldByIndex(
                      protos::TraceEntry::kElementsFieldNumber, 0)
                  .IsOk());
...
TEST_F(RwProtoTest, EnterIndexedRepeatedField_IncompatibleWireType) {
  ...
  cursor.EnterRepeatedFieldByIndex(protos::TraceEntry::kElementsFieldNumber, 0);
  cursor.EnterField(protos::Element::kIdFieldNumber);
  ASSERT_TRUE(cursor.EnterRepeatedFieldByIndex(0, 0).IsAbort());
}
...
// index (EnterRepeatedFieldByIndex). While it's technically possible to allow
...
TEST_F(RwProtoTest,
  ...
  // Attempt to enter field_index (3) > elements.size() (2) => abort
  {
    ...
    ASSERT_TRUE(cursor
                    .EnterRepeatedFieldByIndex(
                        protos::TraceEntry::kElementsFieldNumber, 3)
                    .IsAbort());
  }
}
...
TEST_F(RwProtoTest, EnterIndexedRepeatedField_FieldAvailable) {
  ...
  {
    auto id = cursor;
    ASSERT_TRUE(id.EnterRepeatedFieldByIndex(
                      protos::TraceEntry::kElementsFieldNumber, 0)
                    .IsOk());
    id.EnterField(protos::Element::kIdFieldNumber);
...
TEST_F(RwProtoTest, Merge_RepeatedField) {
  ...
  // initialize elements = [{id: 0, value: 1}]
  {
    ...
    element0.EnterRepeatedFieldByIndex(protos::TraceEntry::kElementsFieldNumber,
                                       0);
...
TEST_F(RwProtoTest, SetBytes_UpdatesExistingField) {
  auto root = data_empty_.Root();

  // elements[0]
  {
    auto element = root;
    element.EnterRepeatedFieldByIndex(protos::TraceEntry::kElementsFieldNumber,
                                      0);
