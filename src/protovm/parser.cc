...
StatusOr<void> Parser::ParseInstructions(
  ...
  for (; it_instruction; ++it_instruction) {
    ...
    if (status.IsAbort()) {
      PERFETTO_RETURN(status, "instruction[%d]", instruction_index);
    }
...
StatusOr<void> Parser::ParseSelect(
  ...
  if (cursors_.selected == CursorEnum::SRC && cursors_.create_if_not_exist) {
    PERFETTO_ABORT(
...
StatusOr<void> Parser::ParseSelectRec(
  ...
  if (!curr_component.has_field_id()) {
    PERFETTO_ABORT("Invalid path. Expected path component with field_id.");
  }

  // iterate repeated field
  if (curr_component.is_repeated()) {
    if (cursors_.selected == CursorEnum::SRC) {
      auto status_or_it = executor_.IterateRepeatedField(
          cursors_.src, curr_component.field_id());
      PERFETTO_RETURN_IF_STATUS_NOT_OK(
          status_or_it, "iterate repeated field (id = %d)",
      ...
      for (auto it = *status_or_it; it; ++it) {
        ...
        PERFETTO_RETURN_IF_STATUS_NOT_OK(
            status, "repeated field (id = %d, index = %d)",
            static_cast<int>(curr_component.field_id()), index);
        ++index;
      }
    } else if (cursors_.selected == CursorEnum::DST) {
      auto status_or_it = executor_.IterateRepeatedField(
  ...
  if (next_component && next_component->has_array_index()) {
    ++it_path_component;
    auto status_enter = executor_.EnterRepeatedFieldByIndex(
        cursors_, curr_component.field_id(), next_component->array_index());
    PERFETTO_RETURN_IF_STATUS_NOT_OK(status_enter,
                                     "enter indexed repeated field");
    auto status_select = ParseSelectRec(instruction, it_path_component);
    PERFETTO_RETURN(status_select, "repeated field (id = %d, index = %d)",
                    static_cast<int>(curr_component.field_id()),
  ...
  if (next_component && next_component->has_map_key_field_id()) {
    ...
    PERFETTO_RETURN_IF_STATUS_NOT_OK(key, "enter mapped repeated field");
    auto status_enter = executor_.EnterRepeatedFieldByKey(
        cursors_, curr_component.field_id(), next_component->map_key_field_id(),
        static_cast<uint32_t>(*key));
    PERFETTO_RETURN_IF_STATUS_NOT_OK(status_enter,
                                     "enter mapped repeated field");
    auto status_select = ParseSelectRec(instruction, it_path_component);
    PERFETTO_RETURN(status_select, "mapped repeated field (id = %d, key = %d)",
                    static_cast<int>(curr_component.field_id()),
                    static_cast<int>(*key));
  }
  ...
  PERFETTO_RETURN_IF_STATUS_NOT_OK(status, "enter field (id = %d)",
                                   static_cast<int>(curr_component.field_id()));
