...
perfetto::base::StatusOr<std::string> LoadDescriptors(
  ...
  if (!fds.SerializeToString(&fds_bytes)) {
    return perfetto::base::ErrStatus("Failed to serialize FileDescriptorSet");
  }
  return fds_bytes;
}
}  // namespace
...
int main(int argc, char** argv) {
  ...
  if (auto status = compiler.RegisterDescriptors(
          reinterpret_cast<const uint8_t*>(status_or_descriptors->data()),
          status_or_descriptors->size());
      !status.ok()) {
    PERFETTO_ELOG("Failed to register descriptors: %s", status.c_message());
    return 1;
  }
