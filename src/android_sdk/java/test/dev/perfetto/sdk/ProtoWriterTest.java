...
public class ProtoWriterTest {
  ...
  public void varInt() {
    ...
    assertEquals(0, readVarInt(b, off));
    ...
    assertEquals(-1L, readVarInt(b, off));
  }
  ...
  public void sInt() {
    ...
    int[] off = {0};
    readVarInt(b, off);
    assertEquals(0, readVarInt(b, off)); // zigzag(0) = 0
    readVarInt(b, off);
    assertEquals(1, readVarInt(b, off)); // zigzag(-1) = 1
    readVarInt(b, off);
    assertEquals(2, readVarInt(b, off)); // zigzag(1) = 2
  }
  ...
  public void boolField() {
    ...
    int[] off = {0};
    readVarInt(b, off);
    assertEquals(1, readVarInt(b, off));
    readVarInt(b, off);
    assertEquals(0, readVarInt(b, off));
  }
  ...
  public void fixed() {
    ...
    assertEquals(
        0xDEADBEEFL,
        ByteBuffer.wrap(b, off[0], 4).order(ByteOrder.LITTLE_ENDIAN).getInt() & 0xFFFFFFFFL);
    off[0] += 4;
    ...
    assertEquals(
        0xCAFEBABEDEADFEEDL,
        ByteBuffer.wrap(b, off[0], 8).order(ByteOrder.LITTLE_ENDIAN).getLong());
    off[0] += 8;

    readVarInt(b, off);
    assertEquals(
        3.14f, ByteBuffer.wrap(b, off[0], 4).order(ByteOrder.LITTLE_ENDIAN).getFloat(), 0);
    off[0] += 4;

    readVarInt(b, off);
    assertEquals(
        2.71828, ByteBuffer.wrap(b, off[0], 8).order(ByteOrder.LITTLE_ENDIAN).getDouble(), 0);
  }
  ...
  public void stringAscii() {
    ...
    assertEquals("hello", new String(b, off[0], len, StandardCharsets.UTF_8));
  }
  ...
  public void stringUtf8() {
    ...
    readVarInt(b, off);
    int len = (int) readVarInt(b, off);
    assertEquals(s, new String(b, off[0], len, StandardCharsets.UTF_8));
  }

  @Test
  public void stringUtf8OverflowsScratch() {
    // Force the rare scratch-too-small path: > UTF8_SCRATCH_SIZE/3 non-ASCII.
    StringBuilder sb = new StringBuilder();
    for (int i = 0; i < 400; i++) {
      sb.append('世'); // 3 bytes each in UTF-8.
    }
  ...
  public void appendRawBytes() {
    byte[] pre = {0x0A, 0x03, 'a', 'b', 'c'};
    w.appendRawBytes(pre, 0, pre.length);
    assertArrayEquals(pre, out());
  }
  ...
  public void nestedEmpty() {
    ...
    int[] off = {0};
    readVarInt(b, off);
    // Redundant varint: size 0 written as exactly 4 bytes.
  ...
  public void nestedWithFields() {
    ...
    assertEquals("test", new String(b, off[0], slen, StandardCharsets.UTF_8));
    off[0] += slen;
  ...
  public void triplyNested() {
    ...
    int[] off = {0};
    readVarInt(b, off);
    int s1 = (int) readVarInt(b, off);
    int e1 = off[0] + s1;
    readVarInt(b, off);
    int s2 = (int) readVarInt(b, off);
    int e2 = off[0] + s2;
    readVarInt(b, off);
    int s3 = (int) readVarInt(b, off);
    int e3 = off[0] + s3;
    readVarInt(b, off);
    assertEquals(42, readVarInt(b, off));
  ...
  public void bufferGrowth() {
    ...
    // Output is still valid after the grow + copy.
  ...
  public void largeRedundantVarInt() {
    // Nested message > 127 bytes still uses a fixed 4-byte redundant varint.
    int tok = w.beginNested(1);
    for (int i = 0; i < 40; i++) {
