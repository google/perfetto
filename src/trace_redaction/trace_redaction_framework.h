...
// The augment and reduce primitives are special in that they can both produce
// and consume trace packets, they also allow local encapsulation of Context
// which avoids polluting the global context with information that is only
// relevant to the specific primitive. These primities work as mini-redaction
// passes that occur after the Trasnforming the entire trace, so they tend to
// be used to consolidate information post-redaction and eliminate redundancy
// generated by it, for instance, merging redundant data.
//
// The cycle of an augment reduce pass is:
// 1. Collect() is called for every packet of the trace. This lets you grab
//    all the context required to operate Augment/Reduce.
// 2. Call Augment() to generate new trace packets (this may happen multiple
//    times).
// 3. Call Reduce() on all packets to drop or update existing packets.
class AugmentReducePrimitive {
 public:
  virtual ~AugmentReducePrimitive();

  // Reads a trace packet and collects information about the entire trace
  virtual base::Status Collect(
      const protos::pbzero::TracePacket::Decoder& packet,
      const Context* context) = 0;

  // Called repeatedly once per trace until an empty string is returned.
  // Each non-empty string returned should be a serialized TracePacket to be
  // appended to the trace.
  virtual std::string Augment(const Context& context) = 0;

  // Modifies a packet using data from the context. Empty strings are treated
  // as a request to drop the packet.
  virtual base::Status Reduce(const Context* context,
                              std::string* packet) const = 0;
};
