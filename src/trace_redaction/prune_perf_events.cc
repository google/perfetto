...
base::Status PrunePerfEvents::Transform(const Context& context,
  ...
  if (!packet_decoder.has_timestamp()) {
    return base::ErrStatus(
        "Redactor failed to redact packet because"
        " packet does not have a timestamp");
  }
...
base::Status PrunePerfEvents::OnPerfSample(
  ...
  if (!decoder.has_pid()) {
    // Skip any samples without a PID as its required to compute belongs to
    // relationship
    return base::OkStatus();
  }
  ...
  if (PERFETTO_LIKELY(!trace_packet_clock_id.has_value())) {
    ...
    if (!trusted_packet_sequence_id.has_value()) {
      return base::ErrStatus(
          "Redactor could not find trusted_packet_sequence_id in perf sample"
