...
base::Status PrunePerfEvents::OnPerfSample(
  ...
  if (ts == 255619121828) {
    PERFETTO_ELOG("EDGAR: Found ts=255619121828 converted to ts=%" PRIu64,
                  trace_ts);
  }
  if (filter_->Includes(context, trace_ts, pid)) {
    proto_util::AppendField(perf_sample_field, message);
  } else if (ts == 255619121828) {
    PERFETTO_ELOG("EDGAR: Decided to drop perf sample for ts=255619121828");
  }
