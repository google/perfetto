...
bool ParsePackagesListStream(
  ...
  compiled_regexes.reserve(package_name_filter_regex.size());
  for (const auto& pattern : package_name_filter_regex) {
...
PackagesListDataSource::PackagesListDataSource(
  ...
  for (auto re = cfg.package_name_filter_regex(); re; ++re) {
    package_name_filter_regex_.emplace_back((*re).ToStdString());
  }
...
void PackagesListDataSource::Start() {
  ...
  if (fs) {
    packages_parse_error_ = ParsePackagesListStream(
        packages_, fs, package_name_filter_, package_name_filter_regex_);
    if (ferror(*fs))
