...
using sd_journal_get_realtime_usec_t = int (*)(sd_journal*, uint64_t*);
...
struct SdJournalApi {
  ...
  static std::unique_ptr<SdJournalApi> Load() {
    void* handle = dlopen("libsystemd.so.0", RTLD_NOW | RTLD_LOCAL);
    if (!handle) {
      PERFETTO_ELOG("Failed to dlopen libsystemd.so.0: %s", dlerror());
      return nullptr;
    }

    auto api = std::make_unique<SdJournalApi>();
    auto load_sym = [&](const char* name) -> void* {
      void* sym = dlsym(handle, name);
      if (!sym)
        PERFETTO_ELOG("dlsym(%s) failed: %s", name, dlerror());
      return sym;
    };

#define LOAD(fn)                                                          \
  api->fn =                                                               \
      reinterpret_cast<sd_journal_##fn##_t>(load_sym("sd_journal_" #fn)); \
  if (!api->fn)                                                           \
    return nullptr;
    ...
    LOAD(close)
    LOAD(add_match)
    LOAD(add_disjunction)
    LOAD(add_conjunction)
    LOAD(seek_tail)
    LOAD(previous)
    LOAD(next)
    LOAD(get_fd)
    LOAD(process)
    LOAD(get_realtime_usec)
    LOAD(get_monotonic_usec)
    LOAD(get_data)
#undef LOAD
...
void JournaldDataSource::Start() {
  ...
  for (uint32_t p = 0; p <= min_prio_; ++p) {
    ...
    sd_->add_match(journal_, match.c_str(), match.size());
    if (p < min_prio_)
      sd_->add_disjunction(journal_);
  }

  // If identifier filters: add them conjuncted with the priority block.
  if (!filter_identifiers_.empty()) {
    sd_->add_conjunction(journal_);
  ...
  sd_->seek_tail(journal_);
  sd_->previous(journal_);

  int fd = sd_->get_fd(journal_);
  if (fd < 0) {
    PERFETTO_ELOG("sd_journal_get_fd failed: %d", -fd);
    sd_->close(journal_);
    journal_ = nullptr;
    return;
  }
...
void JournaldDataSource::ReadJournalEntries() {
  ...
  while (sd_->next(journal_) > 0 && n < kMaxEventsPerRead) {
    ...
    sd_id128_t boot_id{};
    sd_->get_realtime_usec(journal_, &realtime_us);
    sd_->get_monotonic_usec(journal_, &monotonic_us, &boot_id);
