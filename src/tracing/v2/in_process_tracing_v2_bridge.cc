...
uint32_t InProcessTracingV2Bridge::NumChunksForCapacity(size_t capacity_bytes,
  ...
  if (chunks_that_fit < kMinChunksPerRing)
    return 0;
...
void InProcessTracingV2Bridge::OnChunkRead(
  ...
  if (PERFETTO_UNLIKELY(state->target_buffer != contents.target_buffer)) {
    DiscardCurrentPacket(state, TracePacket::DATA_LOSS_CHUNK_CORRUPTED);
  ...
  for (uint32_t i = 0; i < contents.num_fragments; ++i) {
    ...
    if (PERFETTO_UNLIKELY(!state->discarding_packet &&
                          fragment.size >
                              kMaxPacketSize - state->partial_packet.size())) {
      state->discarding_packet = true;
      AddDataLoss(&state->pending_data_loss);
    }
...
void InProcessTracingV2Bridge::OnDataLoss(WriterID writer_id) {
  ...
  // The reader discards both loss-flagged and malformed chunks through this
  // callback. It does not distinguish the cause, so report generic loss.
  // Clear the partial packet and keep the report for the next complete one.
  DiscardCurrentPacket(state, /*reason=*/0);
}
