...
// correct implementation comes anywhere near. Wake tests supply a deadline
// shorter than the futex timeout so timeout cannot count as progress. A caller
// must still unblock and join every thread it spawned before asserting on the
// result.
template <typename ConditionFn>
bool SpinUntil(ConditionFn condition,
               base::TimeMillis deadline = base::GetWallTimeMs() +
                                           base::TimeMillis(30000)) {
...
TEST(SharedRingBufferTest, ReserveUntilFull) {
  ...
  for (uint32_t expected_write_pos = 0; expected_write_pos < 4;
       ++expected_write_pos) {
    const auto reservation = ring->TryReserveWritePos();
    ASSERT_EQ(reservation.result, ReserveResult::kReserved);
    EXPECT_EQ(reservation.write_pos, expected_write_pos);
  }
...
TEST(SharedRingBufferTest, CapacityFollowsReadPos) {
  ...
  ASSERT_EQ(ring->TryReserveWritePos().result, ReserveResult::kReserved);
  ASSERT_EQ(ring->TryReserveWritePos().result, ReserveResult::kFull);
  ...
  const auto reservation = ring->TryReserveWritePos();
  ASSERT_EQ(reservation.result, ReserveResult::kReserved);
  EXPECT_EQ(reservation.write_pos, 2u);
...
TEST(SharedRingBufferTest, CapacityAcrossPositionRollover) {
  ...
  for (uint32_t i = 0; i < 8; ++i) {
    ...
    EXPECT_EQ(reservation.write_pos, kSeedPos + i) << i;
  }
  ...
  const auto reservation = ring->TryReserveWritePos();
  ASSERT_EQ(reservation.result, ReserveResult::kReserved);
  EXPECT_EQ(reservation.write_pos, 4u);
...
TEST(SharedRingBufferTest, ReservationLosesToPublication) {
  ...
  ASSERT_EQ(ring->TryReserveWritePos().write_pos, 1u);
...
TEST(SharedRingBufferTest, ReservationLossFindsFull) {
  ...
  ASSERT_EQ(ring->TryReserveWritePos().write_pos, 1u);  // Now (2, 0): full.
...
TEST(SharedRingBufferTest, PublicationLosesToReservation) {
  test::SharedRingBufferForTesting ring(4, kChunkSize);
  ...
  ASSERT_EQ(ring->TryReserveWritePos().write_pos,
...
TEST(SharedRingBufferTest, OneChunkRing) {
  ...
  const auto reservation = ring->TryReserveWritePos();
  ASSERT_EQ(reservation.result, ReserveResult::kReserved);
  EXPECT_EQ(reservation.write_pos, 0u);
...
TEST(SharedRingBufferTest, Claim) {
  // A claim takes the chunk of the reserved position.
  test::SharedRingBufferForTesting ring(4, kChunkSize);

  const auto reservation = ring->TryReserveWritePos();
  ASSERT_EQ(reservation.result, ReserveResult::kReserved);
  EXPECT_TRUE(ring->TryAcquireChunkForWriting(reservation.write_pos,
...
TEST(SharedRingBufferTest, StaleClaimFails) {
  test::SharedRingBufferForTesting ring(4, kChunkSize);
  ...
  ASSERT_EQ(stale.write_pos, 4u);

  // The reader consumes positions 0 to 4 as holes, so chunk 0 ends up Free
  // with position 8's wrap count.
  for (uint32_t chunk_pos = 0; chunk_pos <= 4; ++chunk_pos) {
    const auto chunk_idx = ChunkIndex::FromPosition(chunk_pos, 4);
  ...
  EXPECT_FALSE(ring->TryAcquireChunkForWriting(stale.write_pos,
                                               BeingWrittenWord(kWriterA)));
...
TEST(SharedRingBufferTest, OnlyMatchingPositionClaims) {
...
// The wrap count in a Free word is 16 bits wide, so the same physical chunk
// answers to the same word after min(num_chunks * 65536, 2^32) reservations.
// That period is an accepted limit of the ABI, pinned here so the test
// fails if the wrap-count width silently changes.
TEST(SharedRingBufferTest, WrapCountPeriod) {
  test::SharedRingBufferForTesting ring(2, kChunkSize);
...
TEST(SharedRingBufferTest, AcknowledgeThenReclaim) {
...
TEST(SharedRingBufferTest, FailedReclaimReportsWord) {
  test::SharedRingBufferForTesting ring(4, kChunkSize);
...
TEST(SharedRingBufferTest, OnlyReaderWritesFree) {
  test::SharedRingBufferForTesting ring(4, kChunkSize);
...
TEST(SharedRingBufferTest, ReclaimAcrossWrapCountRollover) {
  // The 16-bit wrap count rolls over every num_chunks * 65536 positions, long
...
// ---------------------------------------------------------------------------
// The three chunk state-word races, both orders each.
//
// Both sides compare against the same exact word, so the second one fails and
// receives the winner's word. These tests pin that word. The stress tests in
// shared_ring_buffer_concurrency_unittest.cc run the same transitions across
// real threads.
// ---------------------------------------------------------------------------
...
TEST(SharedRingBufferTest, UnclaimedAdvanceLosesToClaim) {
  ...
  for (uint32_t chunk_pos = 0; chunk_pos < 8; ++chunk_pos) {
    ...
    const auto chunk_idx = ChunkIndex::FromPosition(chunk_pos, kNumChunks);
...
TEST(SharedRingBufferTest, ClaimLosesToUnclaimedAdvance) {
  constexpr uint32_t kNumChunks = 2;
  test::SharedRingBufferForTesting ring(kNumChunks, kChunkSize);
...
// Race 2: a writer's publication against the reader's scrape. Both compare
// against the BeingWritten word the reader loaded.

TEST(SharedRingBufferTest, ScrapeLosesToPublication) {
  test::SharedRingBufferForTesting ring(2, kChunkSize);
  ASSERT_TRUE(ring->TryAcquireChunkForWriting(0, BeingWrittenWord(kWriterA)));
...
// it. Both compare against the Complete word.
...
TEST(SharedRingBufferTest, WriterWakesOnPublish) {
  ...
  std::thread writer([&] {
    ...
    reserved.store(ring->TryReserveWritePos().result ==
                   ReserveResult::kReserved);
  });

  about_to_wait.Wait();
  // Registration precedes both the user-space recheck and kernel entry, so
  // publication can race either check or wake a sleeping writer.
  ...
  // the writer thread finish - and be joined - if registration was never
  // observed.
  ring->PublishReadPos(1);
  writer.join();
...
TEST(SharedRingBufferTest, WaitReturnsIfReadPosMoved) {
  if (!SharedRingBuffer::SupportsWriterWait())
    GTEST_SKIP() << "The futex wait is not available on this platform";
  test::SharedRingBufferForTesting ring(2, kChunkSize);
...
// With no publication, this exercises the wait and timeout-cleanup path. Only
// the result and waiter cleanup are asserted: a timeout, signal or spurious
// wake all return kRetry, and the writer's deadline loop decides when to give
// up.
//
// The setup keeps write_pos nonzero and read_pos unchanged. A wait on the
// wrong half of the packed word would also return kRetry, just at once, so
// this test cannot tell that apart. WriterWakesOnPublish can: its writer only
// reserves if the wait outlives the publication.
TEST(SharedRingBufferTest, WaitTimesOut) {
  if (!SharedRingBuffer::SupportsWriterWait())
    GTEST_SKIP() << "The futex wait is not available on this platform";
  test::SharedRingBufferForTesting ring(2, kChunkSize);
  ASSERT_EQ(ring->TryReserveWritePos().result, ReserveResult::kReserved);
...
TEST(SharedRingBufferTest, AllWaitersReleased) {
  // One publication releases every waiting writer.
  if (!SharedRingBuffer::SupportsWriterWait())
    GTEST_SKIP() << "The futex wait is not available on this platform";
  ...
  const auto progress_deadline = base::GetWallTimeMs() + base::TimeMillis(5000);
  std::atomic<uint32_t> completed{0};
  std::vector<SharedRingBuffer::WriterWaitResult> outcomes(
      kNumWaiters, WriterWaitResult::kUnavailable);
  ...
  const bool all_registered = SpinUntil(
      [&] {
  ...
  ring->PublishReadPos(1);
  const bool completed_promptly = SpinUntil(
      [&] { return completed.load() == kNumWaiters; }, progress_deadline);
  // Retry the wake for cleanup on failure, then join before asserting. Even a
  // broken wake remains bounded by the production wait timeout.
  if (!completed_promptly)
    ring->PublishReadPos(1);
  for (auto& waiter : waiters)
    waiter.join();

  ASSERT_TRUE(all_registered);
  EXPECT_TRUE(completed_promptly);
  EXPECT_EQ(completed.load(), kNumWaiters);
...
// may lose it. Each round starts a waiter, publishes as soon as the hint says
// it is registered - which is before it has necessarily reached the kernel -
// and asserts completion within five seconds of starting the thread, before
// the 30-second futex timeout could return kRetry.
TEST(SharedRingBufferTest, NoLostWake) {
  if (!SharedRingBuffer::SupportsWriterWait())
    GTEST_SKIP() << "The futex wait is not available on this platform";
  ...
  for (uint32_t round = 0; round < kRounds; ++round) {
    ...
    ASSERT_TRUE(completed_promptly) << round;
    ASSERT_EQ(outcome, WriterWaitResult::kRetry) << round;
    ASSERT_EQ(Internals::GetNumWritersWaiting(ring.get()), 0u) << round;
  }
}

// Publishing without waiters leaves the syscall-elision hint at zero.
TEST(SharedRingBufferTest, PublishWithoutWaitersLeavesHintClean) {
  if (!SharedRingBuffer::SupportsWriterWait())
    GTEST_SKIP() << "The futex wait is not available on this platform";
  test::SharedRingBufferForTesting ring(2, kChunkSize);
  EXPECT_EQ(Internals::GetNumWritersWaiting(ring.get()), 0u);
