...
class StepSequencer {
 public:
  ...
  static uint64_t PrepareStep(uint32_t round) {
    return static_cast<uint64_t>(round) * kStepsPerRound;
  }
...
TEST(SharedRingBufferConcurrencyTest, ClaimVersusUnclaimedAdvance) {
  ...
  test::SharedRingBufferForTesting ring(kNumChunks, 256);
  ...
  for (uint32_t round = 0; round < kRounds; ++round) {
    ...
    const uint32_t chunk_idx = ChunkIndexOfPosition(position, kNumChunks);
    ...
    if (!sequencer.WaitForStep(StepSequencer::PrepareStep(round + 1)))
      return;
    const uint32_t after = ring->LoadChunkStateWord(chunk_idx);
    if (advanced) {
      ...
      ASSERT_EQ(WrapCountOf(after),
                WrapCountForPosition(position + kNumChunks, ring->num_chunks()))
          << round;
      ASSERT_FALSE(
      ...
      ASSERT_TRUE(
          ring->TryReleaseChunkAsComplete(chunk_idx, &being_written, complete))
          << round;
...
TEST(SharedRingBufferConcurrencyTest, PublishVersusScrape) {
  ...
  std::thread writer([&] {
    for (uint32_t round = 0; round < kRounds; ++round) {
      ...
      if (ring->TryReleaseChunkAsComplete(0, &observed, kComplete)) {
  ...
  for (uint32_t round = 0; round < kRounds; ++round) {
    if (!sequencer.WaitForStep(StepSequencer::PrepareStep(round)))
      return;
    Internals::SetChunkStateWord(ring.get(), 0, BeingWrittenWord(kWriterA));
...
TEST(SharedRingBufferConcurrencyTest, ReuseVersusReclaim) {
  ...
  for (uint32_t round = 0; round < kRounds; ++round) {
    if (!sequencer.WaitForStep(StepSequencer::PrepareStep(round)))
      return;
    Internals::SetChunkStateWord(ring.get(), 0, kComplete);
    uint32_t observed = ring->LoadChunkStateWord(0);
    sequencer.FinishStep(StepSequencer::PrepareStep(round));
...
// TracingV2SharedRingBufferTest.ReservationLosesToPublication,
// ReservationLossFindsFull and PublicationLosesToReservation.
// ---------------------------------------------------------------------------
...
TEST(SharedRingBufferConcurrencyTest, ReservationVersusReadPosPublication) {
  ...
  for (uint32_t round = 0; round < kRounds; ++round) {
    ...
    ASSERT_EQ(Internals::GetReadPos(ring.get()), round) << round;
  }
  writer.join();
}
...
void RunStress(const StressParams& params, StressStats* stats) {
  ...
  // leaves its loop within one BeginFragment() call.
    ...
    writer_threads.emplace_back([&, w] {
      ...
      for (uint32_t n = 0; n < params.fragments_per_writer; ++n) {
        ...
        const SharedRingBufferWriter::FragmentRange range =
            writer.BeginFragment(size, false);
        if (range.result !=
  ...
  for (;;) {
    ...
    if (base::GetWallTimeMs() >= drain_deadline) {
      ...
      ADD_FAILURE() << "Stress drain made no progress within its deadline: "
                    ...
                    << ", last result " << static_cast<int>(result.last_result);
      break;
    }
...
TEST(SharedRingBufferConcurrencyTest, StressAcrossWrapIdentityRollover) {
  ...
  EXPECT_LT(WrapCountForPosition(stats.final_read_pos, 4),
            WrapCountForPosition(4u * 65536 - 64u, 4));
}
...
TEST(SharedRingBufferConcurrencyTest, StressStallPolicy) {
  ...
  if (SharedRingBuffer::SupportsWriterWait()) {
    ...
    // whose chunks are pinned by writers the reader has marked for rewrite:
    // that is NoChunkAvailable, not Full, and the two are different events.
    // Whatever is lost that way is accounted for exactly, which the per-writer
    // check inside RunStress() has already asserted.
