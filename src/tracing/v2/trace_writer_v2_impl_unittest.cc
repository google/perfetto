...
class PacketReassembler : public SharedRingBufferReader::Delegate {
 public:
  void OnChunkRead(
    ...
    EXPECT_EQ(contents.payload_flags & kFlagDataLoss, 0u);
    ...
    for (uint32_t i = 0; i < contents.num_fragments; ++i) {
      ...
      // The earlier bytes were lost. Later fragments can start new packets.
      if (continues_from_prev && pending.empty())
        continue;
...
TEST(TraceWriterV2ImplTest, DirectFinalizationThenFlushClosesFragmentOnce) {
  Fixture f;
  {
    ...
    packet->Finalize();
    // Message finalization alone has not closed the fragment.
...
TEST(TraceWriterV2ImplTest, PacketSpanningSeveralChunksReconstructsExactly) {
  ...
  // Matching continuation flags must reconstruct one complete packet.
  const std::vector<protos::gen::TracePacket> packets = f.reassembler.Decode();
  ASSERT_EQ(packets.size(), 1u);
...
TEST(TraceWriterV2ImplTest, FinalizingAPacketCanCrossAChunkBoundary) {
  ...
  auto packet = f.writer->NewTracePacket();
  packet->set_for_testing()->set_str(payload);
  ASSERT_EQ(f.ring->LoadWritePosRelaxed(), 1u);
  f.writer->FinishTracePacket();
  EXPECT_EQ(f.ring->LoadWritePosRelaxed(), 2u);
  f.writer->Flush();
  f.reader.Drain(64);
...
// A packet larger than the ring buffer needs the reader to free chunks:
// - Notify at each fragment boundary so writing can continue.
// - Waiting until packet completion would fill both chunks first.
...
TEST(TraceWriterV2ImplTest, PacketLargerThanTheWholeRingSurvives) {
  ...
  // The packet crossed the ring buffer several times, so the reader was told
...
TEST(TraceWriterV2ImplTest,
...

  {
    auto packet = f.writer->NewTracePacket();
    packet->set_timestamp(2);
    // The reader copies the first packet while the second is still open.
...
TEST(TraceWriterV2ImplTest, DropModeCountsDropsAndReportsTheGapAfterwards) {
  ...
  // Consume the unclaimed reservations so the writer can reserve again.
  // Its next chunk carries the loss flag, so packet 2 is discarded too.
  f.reader.Drain(64);
  {
    auto packet = f.writer->NewTracePacket();
    packet->set_timestamp(2);
...
TEST(TraceWriterV2ImplTest, APartiallyDroppedPacketIsNotResumed) {
...

  {
    ...
    // The packet started in the drop buffer. Make ring buffer space available
  ...
  // Packet 2 shares the chunk's loss flag. Packet 3 starts an unflagged chunk.
  EXPECT_TRUE(reassembler.packets.empty());
...
TEST(TraceWriterV2ImplTest, LossDiscardsPartialPacketsAndOrphanContinuations) {
  ...
  // saves its published fragments while waiting for a continuation.
  {
    auto packet = f.writer->NewTracePacket();
    packet->set_for_testing()->set_str(std::string(1024, 'x'));
  }
  f.reader.Drain(64);
  EXPECT_EQ(f.writer->drop_count(), 1u);
  EXPECT_TRUE(f.reassembler.packets.empty());
  EXPECT_EQ(f.reassembler.data_loss_reports, 0u);

  // Packet 2 spans two chunks:
  // - The first carries the pending loss. Discard it and packet 1's fragments.
  // - The second is unflagged, but starts with an orphan continuation.
  {
    auto packet = f.writer->NewTracePacket();
    packet->set_for_testing()->set_str(std::string(300, 'y'));
  }
  // Packet 3 shares the second chunk and must survive the orphan's discard.
  {
    auto packet = f.writer->NewTracePacket();
    packet->set_timestamp(3);
  }
...
TEST(TraceWriterV2ImplTest, DropBufferSupportsContiguousStreamReservations) {
  ...
  // The first chunk after the drop reports the loss and is discarded.
  EXPECT_TRUE(f.reassembler.packets.empty());
  EXPECT_EQ(f.reassembler.data_loss_reports, 1u);
  {
    auto recovered = f.writer->NewTracePacket();
    recovered->set_timestamp(43);
  }
  f.writer->Flush();
  f.reader.Drain(64);
  ...
  const auto packets = f.reassembler.Decode();
  ASSERT_EQ(packets.size(), 1u);
  EXPECT_EQ(packets[0].timestamp(), 43u);
  EXPECT_EQ(f.writer->drop_count(), 1u);
  EXPECT_EQ(f.reassembler.data_loss_reports, 1u);
}
