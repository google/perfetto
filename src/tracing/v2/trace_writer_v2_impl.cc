...
TraceWriterV2Impl::TraceWriterV2Impl(SharedRingBufferArbiterImpl* arbiter,
                                     SharedRingBuffer* ring,
                                     WriterID id,
                                     BufferID target_buffer,
                                     BufferExhaustedPolicy policy)
    : arbiter_(arbiter),
      ring_writer_(ring, id, target_buffer, policy, arbiter),
      stream_writer_(this),
...
TraceWriterV2Impl::~TraceWriterV2Impl() {
  FinishTracePacket();
  // Publish the last chunk before the arbiter releases this WriterID.
  // Ring reservations preserve the order across ID reuse.
  ring_writer_.FinishCurrentChunk();
  stream_writer_.Reset({nullptr, nullptr});
  arbiter_->OnWriterDestroyed(writer_id());
}
...
TraceWriter::TracePacketHandle TraceWriterV2Impl::NewTracePacket() {
  ...
  const auto range = ring_writer_.BeginFragment(kMinFragmentPayloadSize,
  ...
  cur_packet_->ResetForProtoGroup(&stream_writer_);
  packet_open_ = true;
...
void TraceWriterV2Impl::Flush(std::function<void()> callback) {
  ...
  // Publish its remaining fragment before the flush.
  // Raw stream callers bypass message finalization. They must call
  // FinishTracePacket() themselves.
  if (packet_open_ && cur_packet_->is_finalized())
    FinishTracePacket();
  PERFETTO_CHECK(!packet_open_);

  // A completed fragment leaves its chunk cached for later packets. Release
  // that chunk before arbiter_->Flush() so later packets use new reservations.
  ring_writer_.FinishCurrentChunk();
  stream_writer_.Reset({nullptr, nullptr});
  arbiter_->Flush(std::move(callback));
}
...
void TraceWriterV2Impl::ClosePacketFragment(bool continues_on_next) {
  ...
  const auto result = ring_writer_.EndFragment(used, continues_on_next);
...
protozero::ContiguousMemoryRange TraceWriterV2Impl::EnterDropMode() {
  ...
  if (drop_buffer_.empty())
    drop_buffer_.resize(ring_writer_.chunk_size());
