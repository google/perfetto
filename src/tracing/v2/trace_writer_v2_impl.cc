...
TraceWriter::TracePacketHandle TraceWriterV2Impl::NewTracePacket() {
  ...
  const auto range = ring_buffer_writer_.BeginFragment(
      kMinFragmentPayloadSize, /*continues_from_prev=*/false);
...
protozero::ContiguousMemoryRange TraceWriterV2Impl::GetNewBuffer() {
  ...
  // Keep the packet in the drop buffer and try the ring buffer when the next
  // packet starts.
  if (in_drop_mode_)
    return EnterDropMode();
  ...
  const auto range = ring_buffer_writer_.BeginFragment(
      kMinFragmentPayloadSize, /*continues_from_prev=*/true);
  if (range.result != SharedRingBufferWriter::BeginFragmentResult::kSuccess)
    return EnterDropMode();
...
protozero::ContiguousMemoryRange TraceWriterV2Impl::EnterDropMode() {
  ...
  if (!in_drop_mode_) {
    in_drop_mode_ = true;
    ++drop_count_;
    // Flag the loss on the next publication, including in a cached chunk.
