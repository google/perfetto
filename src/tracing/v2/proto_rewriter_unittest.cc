...
TEST(ProtoRewriterTest, OutputLimitFitsEmbeddedLinks) {
  std::vector<uint8_t> out;
  const std::vector<uint8_t> in = Bytes({0x0b, 0x04});
  ASSERT_EQ(Rewrite(in, &out, UINT32_MAX), RewriteResult::kSuccess);
  EXPECT_EQ(out, Bytes({0x0a, 0x80, 0x80, 0x80, 0x00}));
...
TEST(ProtoRewriterTest, OrdinaryFieldsRespectOutputLimit) {
  ...
  for (const auto& field : fields) {
    // Copy a prefix before reaching the field that exceeds the limit.
    std::vector<uint8_t> in = Bytes({0x08, 0x2a});
    in.insert(in.end(), field.begin(), field.end());
    std::vector<uint8_t> out;
    ASSERT_EQ(Rewrite(in, &out, in.size()), RewriteResult::kSuccess);
    EXPECT_EQ(out, in);
    EXPECT_EQ(Rewrite(in, &out, in.size() - 1), RewriteResult::kOutputTooLarge);
    EXPECT_TRUE(out.empty());
  }
}
...
TEST(ProtoRewriterTest, GeneratedTracePacketRoundTrips) {
  ...
  packet.Reset(&writer, protozero::NestedMessageEncoding::kProtoGroup);
