...
void TracingMuxerImpl::Shutdown() {
  ...
  owned_task_runner->PostTask([muxer, &shutdown_done] {
    ...
    // Make sure no trace writers are lingering around on the muxer thread.
    // DestroyAllTraceWritersForCurrentThread will destory the active writers
    // Before when the thread exits, later delete muxer frees the SMB.
    muxer->DestroyAllTraceWritersForCurrentThread();
