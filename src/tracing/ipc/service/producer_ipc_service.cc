...
void ProducerIPCService::InitializeConnection(
  ...
  args.shared_memory_size_hint_bytes = req.shared_memory_size_hint_bytes();
  args.smb_scraping_mode = smb_scraping_mode;
  args.shared_memory_page_size_hint_bytes =
      req.shared_memory_page_size_hint_bytes();
  args.shm = std::move(shmem);
  args.sdk_version = req.sdk_version();
  args.machine_name = client_info.machine_name();
#if PERFETTO_BUILDFLAG(PERFETTO_OS_LINUX) || \
    PERFETTO_BUILDFLAG(PERFETTO_OS_ANDROID)
  args.supports_tracing_v2 =
      HasMemfdSupport() && !ipc::Service::use_shmem_emulation() &&
      req.ring_buffer_abi_version() == tracing_v2::kRingBufferAbiVersion;
#endif
  producer->service_endpoint = core_service_->ConnectProducer(
      producer.get(), client_identity, req.producer_name(), std::move(args));
  ...
  const uint32_t ring_buffer_abi_version =
      producer->service_endpoint->SupportsTracingV2()
...
void ProducerIPCService::OfferRingBuffer(
    const protos::gen::OfferRingBufferRequest& req,
    DeferredOfferRingBufferResponse resp) {
  // Take ownership even if the connection or offer is invalid.
  auto fd = ipc::Service::TakeReceivedFD();
  auto* producer = GetProducerForCurrentRequest();
  if (!producer || !producer->service_endpoint->SupportsTracingV2()) {
    resp.Resolve(
        ipc::AsyncResult<protos::gen::OfferRingBufferResponse>::Create());
    return;
  }
  ...
  // The response stays alive until the endpoint completes the offer.
  auto pending =
      std::make_shared<DeferredOfferRingBufferResponse>(std::move(resp));
  producer->service_endpoint->OfferRingBuffer(
