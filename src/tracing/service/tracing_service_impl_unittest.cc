...
TEST_F(TracingServiceImplTest, InProcessInstanceWriterUsesV1) {
  ...
  EXPECT_CALL(producer, SetupDataSource(_, _))
      .WillOnce([&](DataSourceInstanceID id, const DataSourceConfig& setup) {
        ...
        EXPECT_FALSE(setup.tracing_v2_eligible());
...
TEST_F(TracingServiceImplTest, RejectedRingBufferOfferDoesNotPreventRetry) {
  ...
  // The endpoint still supports v2 — a second offer could succeed.
  EXPECT_TRUE(endpoint->SupportsTracingV2());
  EXPECT_CALL(producer, OnDisconnect()).Times(1);
  task_runner.RunUntilIdle();
  endpoint.reset();
}
