...
TEST_F(TracingServiceImplTest, WriteIntoFileCloneSession) {
  auto write_into_file_file = base::TempFile::Create();
  unlink(write_into_file_file.path().c_str());
  ...
  ds_config->set_target_buffer(0);
  trace_config.set_write_into_file(true);
  trace_config.set_output_path(write_into_file_file.path());
  consumer->EnableTracing(trace_config);
  ...
  // don't flush, keep data in the buffer.
  ...
  // Should have only one 'read_tracing_buffers_completed: true' service_event,
  // all the data is already written into file.
  auto clone_consumer_buffers_after_clone = clone_consumer->ReadBuffers();
  EXPECT_THAT(
      clone_consumer_buffers_after_clone,
  ...
  PERFETTO_DLOG("clone_consumer_buffers_after_clone.size(): %zu",
                clone_consumer_buffers_after_clone.size());

  consumer->DisableTracing();
  producer->WaitForDataSourceStop("data_source");
  consumer->WaitForTracingDisabled();

  auto extract_for_testing_strs_fn = [](protos::gen::Trace trace) {
    std::vector<std::string> strs;
    for (const auto& packet : trace.packet()) {
      if (packet.has_for_testing()) {
        strs.push_back(packet.for_testing().str());
      }
    }
    return strs;
  };

  // Verify the content of the 'session.write_into_file' file.
  {
    std::string trace_raw;
    ASSERT_TRUE(base::ReadFile(write_into_file_file.path(), &trace_raw));
    protos::gen::Trace trace;
    ASSERT_TRUE(trace.ParseFromString(trace_raw));
    auto payload = extract_for_testing_strs_fn(trace);
    EXPECT_THAT(payload, ElementsAre("payload #1", "payload #2"));
  }

  // Verify the content of the cloned session file.
  {
    std::string trace_raw;
    ASSERT_TRUE(base::ReadFile(cloned_session_file.path(), &trace_raw));
    protos::gen::Trace trace;
    ASSERT_TRUE(trace.ParseFromString(trace_raw));
    auto payload = extract_for_testing_strs_fn(trace);
    EXPECT_THAT(payload, ElementsAre("payload #1"));
  }
}
...
TEST_F(TracingServiceImplTest, WriteIntoFileBugreport) {
  std::unique_ptr<MockConsumer> consumer_write_into_file = CreateMockConsumer();
  consumer_write_into_file->Connect(svc.get());

  std::unique_ptr<MockConsumer> consumer_write_into_file_clone =
      CreateMockConsumer();
  consumer_write_into_file_clone->Connect(svc.get());

  std::unique_ptr<MockProducer> producer_write_into_file = CreateMockProducer();
  producer_write_into_file->Connect(svc.get(), "mock_producer_write_into_file");
  producer_write_into_file->RegisterDataSource("data_source_write_into_file");

  std::unique_ptr<MockConsumer> consumer = CreateMockConsumer();
  consumer->Connect(svc.get());

  std::unique_ptr<MockConsumer> consumer_clone = CreateMockConsumer();
  consumer_clone->Connect(svc.get());

  std::unique_ptr<MockProducer> producer = CreateMockProducer();
  producer->Connect(svc.get(), "mock_producer");
  producer->RegisterDataSource("data_source");

  auto create_trace_config = [](const std::string& session_name,
  ...
  base::TempFile write_into_file_dump = base::TempFile::Create();

  {
    TraceConfig trace_config = create_trace_config(
        "session_write_into_file", "data_source_write_into_file");
    trace_config.set_write_into_file(true);
    consumer_write_into_file->EnableTracing(
        trace_config, base::ScopedFile(dup(write_into_file_dump.fd())));
  }

  {
    TraceConfig trace_config = create_trace_config("session", "data_source");
    consumer->EnableTracing(trace_config);
  }
  ...
  producer_write_into_file->WaitForTracingSetup();
  producer_write_into_file->WaitForDataSourceSetup(
      "data_source_write_into_file");
  producer_write_into_file->WaitForDataSourceStart(
      "data_source_write_into_file");
  ...
  {
    auto tp = writer->NewTracePacket();
    tp->set_for_testing()->set_str("payload_on_disk #1");
  }

  writer->Flush();

  std::unique_ptr<TraceWriter> writer_into_file =
      producer_write_into_file->CreateTraceWriter(
          "data_source_write_into_file");
  {
    auto tp = writer_into_file->NewTracePacket();
    tp->set_for_testing()->set_str("payload_on_disk #1");
  }

  writer_into_file->Flush();

  base::TempFile write_into_file_clone_dump = base::TempFile::Create();

  auto do_clone_session = [&](std::unique_ptr<MockConsumer>& clone_consumer,
                              ConsumerEndpoint::CloneSessionArgs clone_args) {
    static int i = 0;
    auto checkpoint_name = "on_clone_done_" + std::to_string(i++);
    PERFETTO_DLOG("do_clone_session, checkpoint_name: %s",
...

  {
    ...
    // args.output_file_for_bugreport = write_into_file_clone_dump.path();
    args.unique_session_name = "session_write_into_file";
    do_clone_session(consumer_write_into_file_clone, args);
  }

  {
    ConsumerEndpoint::CloneSessionArgs args;
    args.unique_session_name = "session";
    do_clone_session(consumer_clone, args);
  }

  // {
  //   auto tp = writer->NewTracePacket();
  //   tp->set_for_testing()->set_str("payload_on_disk #2");
  // }
  //
  // writer->Flush();
  consumer_write_into_file->DisableTracing();

  {
    const auto& packets = consumer_clone->ReadBuffers();
    PERFETTO_LOG("consumer_clone buffer:");
    for (const auto& packet : packets) {
      if (packet.has_for_testing()) {
        PERFETTO_LOG("packet: %s", packet.for_testing().str().c_str());
      }
    }
    PERFETTO_LOG("=======================");
  }

  const auto& pppp = consumer_clone->ReadBuffers();
  for (const auto& it : pppp) {
    if (it.has_for_testing()) {
      const auto& ft = it.for_testing();
      PERFETTO_LOG("ft: %s", ft.str().c_str());
    }
  }
  ...
  // producer->WaitForDataSourceStop("data_source");
  // consumer->WaitForTracingDisabled();
  // consumer2->WaitForTracingDisabled();

  auto print_trace_fn = [](const std::string& file_path) {
    PERFETTO_LOG("print_trace_fn, file_path: %s", file_path.c_str());
    std::string trace_raw;
    ASSERT_TRUE(base::ReadFile(file_path, &trace_raw));
    protos::gen::Trace trace;
    ASSERT_TRUE(trace.ParseFromString(trace_raw));
    ...
    PERFETTO_LOG("print_trace_fn =======================");
  };

  print_trace_fn(write_into_file_dump.path());
  print_trace_fn(write_into_file_clone_dump.path());
}
