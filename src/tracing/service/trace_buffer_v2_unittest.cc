...
tracing_v2::Fragment MakeFragView(const std::vector<uint8_t>& data) {
  return {data.data(), static_cast<uint32_t>(data.size())};
}

// Identity for ordered proto-group admission.
TraceBufferV2::ProtoGroupSequence MakeV2SeqProps(ProducerID p,
                                                 WriterID w,
                                                 uid_t uid = 42) {
...
TEST_F(TraceBufferV2Test, V2Admission_SingleWholePacket) {
  ...
  auto result = trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0);
  ASSERT_EQ(result, TraceBufferV2::ProtoGroupAppendResult::kStored);
...
TEST_F(TraceBufferV2Test, V2Admission_MultiFragmentPacket) {
  ...
  auto r1 = trace_buffer()->AppendProtoGroupFragments(
      seq, &fv1, 1, tracing_v2::kFlagContinuesOnNextChunk);
  ASSERT_EQ(r1, TraceBufferV2::ProtoGroupAppendResult::kStored);

  // Second chunk: one fragment that continues from the previous chunk.
  auto r2 = trace_buffer()->AppendProtoGroupFragments(
      seq, &fv2, 1, tracing_v2::kFlagContinuesFromPrevChunk);
  ASSERT_EQ(r2, TraceBufferV2::ProtoGroupAppendResult::kStored);
...
TEST_F(TraceBufferV2Test, V2Admission_MultipleWritersInterleaved) {
  ...
  trace_buffer()->AppendProtoGroupFragments(seq1, &fv_a, 1, 0);
  trace_buffer()->AppendProtoGroupFragments(seq2, &fv_b, 1, 0);
  trace_buffer()->AppendProtoGroupFragments(seq1, &fv_c, 1, 0);
...
TEST_F(TraceBufferV2Test, V2Loss_LossAttachesToNextPacket) {
  ...
  trace_buffer()->AppendProtoGroupFragments(seq, &fv_a, 1, 0);
  trace_buffer()->RecordProtoGroupLoss(seq);
  trace_buffer()->AppendProtoGroupFragments(seq, &fv_c, 1, 0);
...
TEST_F(TraceBufferV2Test, V2Loss_RejectedAppendRecordsLoss) {
  ...
  TraceBufferV2::ProtoGroupAppendResult r =
      TraceBufferV2::ProtoGroupAppendResult::kStored;
  int stored_count = 0;
  for (int i = 0; i < 100; i++) {
    r = trace_buffer()->AppendProtoGroupFragments(seq, &fv_big, 1, 0);
    if (r == TraceBufferV2::ProtoGroupAppendResult::kStored) {
      stored_count++;
    } else {
      break;
    }
  ...
  ASSERT_EQ(r, TraceBufferV2::ProtoGroupAppendResult::kDroppedCapacity);
...
TEST_F(TraceBufferV2Test, V2Loss_OverwriteEviction) {
  ...
  for (int i = 0; i < 30; i++) {
    trace_buffer()->AppendProtoGroupFragments(seq, &fv, 1, 0);
  }
...
TEST_F(TraceBufferV2Test, V2Admission_RejectsSmbSequence) {
  ...
  auto frag = MakeFragView(packet);
  EXPECT_EQ(trace_buffer()->AppendProtoGroupFragments(MakeV2SeqProps(1, 7),
                                                      &frag, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kInvalid);
  EXPECT_EQ(trace_buffer()->stats().chunks_written(), 1u);
  EXPECT_TRUE(trace_buffer()->HasSequence(1, 7));
  EXPECT_FALSE(trace_buffer()->HasSequence(1, 8));
...
TEST_F(TraceBufferV2Test, V2Admission_RejectsSmbChunksAndPatches) {
  ...
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  CreateChunk(ProducerID(1), WriterID(7), ChunkID(0))
      .AddPacket(4, 'v')
      .CopyIntoTraceBuffer();
...
TEST_F(TraceBufferV2Test, V2Protocol_SameWriterDifferentProducers) {
  ...
  trace_buffer()->AppendProtoGroupFragments(seq_a, &fv_a, 1, 0);
  trace_buffer()->AppendProtoGroupFragments(seq_b, &fv_b, 1, 0);
...
TEST_F(TraceBufferV2Test, V2Fragmentation_OrphanContinuation) {
  ...
  trace_buffer()->AppendProtoGroupFragments(
      seq, &fv, 1, tracing_v2::kFlagContinuesFromPrevChunk);

  // A second normal packet.
  auto pkt2 = MakeSimpleGroupPacket(1, 51);
  auto fv2 = MakeFragView(pkt2);
  trace_buffer()->AppendProtoGroupFragments(seq, &fv2, 1, 0);
...
TEST_F(TraceBufferV2Test, V2Rewrite_MalformedInput) {
  ...
  trace_buffer()->AppendProtoGroupFragments(seq, &fv_good, 1, 0);
...
TEST_F(TraceBufferV2Test, V2Discard_BufferFull) {
  ...
  for (int i = 0; i < 100; i++) {
    auto r = trace_buffer()->AppendProtoGroupFragments(seq, &fv, 1, 0);
    if (r == TraceBufferV2::ProtoGroupAppendResult::kStored)
      stored++;
    else
      dropped_count++;
...
TEST_F(TraceBufferV2Test, V2Admission_EmptyFragment) {
  ...
  auto r = trace_buffer()->AppendProtoGroupFragments(seq, &fv, 1, 0);
  ASSERT_EQ(r, TraceBufferV2::ProtoGroupAppendResult::kStored);
...
TEST_F(TraceBufferV2Test, V2Admission_ZeroFragmentsRejected) {
  ResetBuffer(4096);
  auto seq = MakeV2SeqProps(1, 1);
  auto r = trace_buffer()->AppendProtoGroupFragments(seq, nullptr, 0, 0);
  ASSERT_EQ(r, TraceBufferV2::ProtoGroupAppendResult::kInvalid);
}
...
TEST_F(TraceBufferV2Test, V2Loss_MultipleLossCoalesce) {
  ...
  // Record loss twice before any packet.
  trace_buffer()->RecordProtoGroupLoss(seq);
  trace_buffer()->RecordProtoGroupLoss(seq);
  ...
  auto result = ReadPacket(&props, &dropped);
  ASSERT_FALSE(result.empty());
  // Loss should be reported on the first packet after the gap.
  EXPECT_NE(dropped, 0u);
  EXPECT_THAT(ReadPacket(), IsEmpty());
}
...
TEST_F(TraceBufferV2Test, V2Loss_RejectedAppendPreservesPosition) {
  ...
  tracing_v2::Fragment first[] = {MakeFragView(a), {b.data(), 1}};
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(
                seq, first, 2, tracing_v2::kFlagContinuesOnNextChunk),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  std::vector<uint8_t> huge(65536);
  auto rejected = MakeFragView(huge);
  EXPECT_NE(trace_buffer()->AppendProtoGroupFragments(seq, &rejected, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  tracing_v2::Fragment last[] = {{b.data() + 1, 1}, MakeFragView(c)};
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(
                seq, last, 2, tracing_v2::kFlagContinuesFromPrevChunk),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  trace_buffer()->BeginRead();
  TraceBuffer::PacketSequenceProperties props{};
  uint32_t loss = 0;
...
TEST_F(TraceBufferV2Test, V2Loss_ClonesAndConsumedBoundary) {
  ...
  auto a = MakeSimpleGroupPacket(1, 11);
  auto b = MakeSimpleGroupPacket(1, 22);
  tracing_v2::Fragment fragments[] = {MakeFragView(a), MakeFragView(b)};
  trace_buffer()->RecordProtoGroupLoss(seq);
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, fragments, 2, 0),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
...
TEST_F(TraceBufferV2Test, V2Loss_EmptyStateIsBoundedAndConservative) {
  ResetBuffer(4096);
  auto seq = MakeV2SeqProps(1, 1);
  auto packet = MakeSimpleGroupPacket(1, 42);
  auto frag = MakeFragView(packet);
  ...
  for (WriterID writer = 2; writer < 2000; ++writer)
    trace_buffer()->RecordProtoGroupLoss(MakeV2SeqProps(1, writer));
  EXPECT_LE(sequence_count(), 1152u);
  EXPECT_EQ(sequence_count(), empty_sequence_count());
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  trace_buffer()->BeginRead();
  TraceBuffer::PacketSequenceProperties props{};
...
TEST_F(TraceBufferV2Test, V2Loss_RejectedEmptyStateIsBounded) {
  ResetBuffer(4096, TraceBuffer::kDiscard);
  auto packet = MakeSimpleGroupPacket(1, 42);
  auto frag = MakeFragView(packet);
  auto seq = MakeV2SeqProps(1, 1);
  while (trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0) ==
         TraceBufferV2::ProtoGroupAppendResult::kStored) {
  }
  for (WriterID writer = 2; writer < 2000; ++writer) {
    EXPECT_EQ(trace_buffer()->AppendProtoGroupFragments(
                  MakeV2SeqProps(1, writer), &frag, 1, 0),
              TraceBufferV2::ProtoGroupAppendResult::kDroppedCapacity);
  }
  EXPECT_LE(sequence_count(), 1153u);
  EXPECT_EQ(sequence_count() - 1, empty_sequence_count());
}
...
TEST_F(TraceBufferV2Test, V2Admission_SizeOverflowAndNullPayload) {
  ...
  tracing_v2::Fragment huge{&byte, UINT32_MAX};
  EXPECT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &huge, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kDroppedCapacity);
  tracing_v2::Fragment invalid{nullptr, 1};
  EXPECT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &invalid, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kInvalid);
  auto packet = MakeSimpleGroupPacket(1, 42);
  auto frag = MakeFragView(packet);
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0),
...
TEST_F(TraceBufferV2Test, V2Rewrite_PreservesEarlierLossReasons) {
  ...
  tracing_v2::Fragment bad{&invalid, 1};
  trace_buffer()->AppendProtoGroupFragments(seq, &bad, 1, 0);
  auto packet = MakeSimpleGroupPacket(1, 42);
  auto frag = MakeFragView(packet);
  trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0);
  trace_buffer()->BeginRead();
  TraceBuffer::PacketSequenceProperties props{};
  uint32_t loss = 0;
  ASSERT_FALSE(ReadPacket(&props, &loss).empty());
...
TEST_F(TraceBufferV2Test, V2Admission_RejectsProtoVmDestination) {
  ResetBuffer(4096);
  trace_buffer()->MaybeSetUpProtoVm("test", "", 1024, 1);
  ASSERT_EQ(trace_buffer()->GetProtoVmInstances().size(), 1u);
  auto packet = MakeSimpleGroupPacket(1, 42);
  auto frag = MakeFragView(packet);
  EXPECT_EQ(trace_buffer()->AppendProtoGroupFragments(MakeV2SeqProps(1, 1),
                                                      &frag, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kInvalid);
...
TEST_F(TraceBufferV2Test, LegacyFlagsCannotSelectProtoGroupAdmission) {
  ResetBuffer(4096);
  CreateChunk(ProducerID(1), WriterID(1), ChunkID(0))
      .AddPacket(4, 'a')
      .SetFlags(
...
TEST_F(TraceBufferV2Test, V2Loss_EvictedBoundaryMarksSurvivingPacket) {
  ...
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(MakeV2SeqProps(1, 2),
                                                      &padding_frag, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  trace_buffer()->AppendProtoGroupFragments(MakeV2SeqProps(1, 2), &frag, 1, 0);
...
TEST_F(TraceBufferV2Test, V2Overwrite_CrossesUsedWatermarkAfterWrap) {
  ...
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(MakeV2SeqProps(1, 3),
                                                      &filler_frag, 1, 0),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  EXPECT_EQ(trace_buffer()->stats().chunks_overwritten(), 3u);
}

TEST_F(TraceBufferV2Test, V2Admission_RejectsLossAndUnknownFlags) {
  const uint32_t invalid_flags[] = {
      tracing_v2::kFlagDataLoss,
      tracing_v2::kFlagDataLoss | tracing_v2::kFlagContinuesOnNextChunk,
      1u << 31};
  for (uint32_t flags : invalid_flags) {
    SCOPED_TRACE(flags);
    ResetBuffer(4096);
    auto seq = MakeV2SeqProps(1, 1);
    ...
    ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0),
              TraceBufferV2::ProtoGroupAppendResult::kStored);
    EXPECT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, flags),
              TraceBufferV2::ProtoGroupAppendResult::kInvalid);
    ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0),
              TraceBufferV2::ProtoGroupAppendResult::kStored);
...
TEST_F(TraceBufferV2Test, V2Admission_InvalidBatchDoesNotEvict) {
  ...
  tracing_v2::Fragment rejected[] = {MakeFragView(large), {nullptr, 1}};
  EXPECT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, rejected, 2, 0),
            TraceBufferV2::ProtoGroupAppendResult::kInvalid);
...
TEST_F(TraceBufferV2Test, V2Rewrite_EmptyFragmentedPacketPreservesLoss) {
  ResetBuffer(4096);
  auto seq = MakeV2SeqProps(1, 1);
  tracing_v2::Fragment empty{nullptr, 0};
  trace_buffer()->RecordProtoGroupLoss(seq);
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(
                seq, &empty, 1, tracing_v2::kFlagContinuesOnNextChunk),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(
                seq, &empty, 1, tracing_v2::kFlagContinuesFromPrevChunk),
            TraceBufferV2::ProtoGroupAppendResult::kStored);
  auto bytes = MakeSimpleGroupPacket(1, 42);
  auto frag = MakeFragView(bytes);
  ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0),
...
TEST_F(TraceBufferV2Test, V2Rewrite_InputSizeLimit) {
  constexpr size_t kLimit = 4 * 1024 * 1024;
  for (size_t input_size : {kLimit, kLimit + 2}) {
    SCOPED_TRACE(input_size);
    ResetBuffer(8 * 1024 * 1024);
    auto seq = MakeV2SeqProps(1, 1);
    std::vector<uint8_t> bytes(input_size, 0);
    ...
    for (size_t off = 0; off < bytes.size();) {
      ...
      uint32_t flags = off ? tracing_v2::kFlagContinuesFromPrevChunk : 0;
      if (off + size < bytes.size())
        flags |= tracing_v2::kFlagContinuesOnNextChunk;
      tracing_v2::Fragment frag{bytes.data() + off,
                                static_cast<uint32_t>(size)};
      ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, flags),
                TraceBufferV2::ProtoGroupAppendResult::kStored);
      off += size;
    }
    auto next = MakeSimpleGroupPacket(1, 42);
    auto frag = MakeFragView(next);
    ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, 0),
              TraceBufferV2::ProtoGroupAppendResult::kStored);

    trace_buffer()->BeginRead();
    if (input_size == kLimit)
      EXPECT_EQ(ReadPacketBytes(trace_buffer()), bytes);
    TraceBuffer::PacketSequenceProperties props{};
    uint32_t loss = 0;
    EXPECT_THAT(ReadPacket(&props, &loss),
                ElementsAre(FakePacketFragment(next.data(), next.size())));
    EXPECT_EQ(loss != 0, input_size > kLimit);
    EXPECT_THAT(ReadPacket(), IsEmpty());
  }
}

TEST_F(TraceBufferV2Test, V2Rewrite_OutputSizeLimit) {
  ResetBuffer(4 * 1024 * 1024);
  auto seq = MakeV2SeqProps(1, 1);
  // Empty sibling messages grow from two bytes to five bytes each. The input
  // fits the rewrite limit, but the output exceeds it by one byte.
  constexpr size_t kNumMessages = (4 * 1024 * 1024) / 5 + 1;
  std::vector<uint8_t> bytes(kNumMessages * 2);
  for (size_t i = 0; i < bytes.size(); i += 2) {
    bytes[i] = 0x0b;
    bytes[i + 1] = 0x04;
  }
  for (size_t off = 0; off < bytes.size();) {
    ...
    if (off + size < bytes.size())
      flags |= tracing_v2::kFlagContinuesOnNextChunk;
    tracing_v2::Fragment frag{bytes.data() + off, static_cast<uint32_t>(size)};
    ASSERT_EQ(trace_buffer()->AppendProtoGroupFragments(seq, &frag, 1, flags),
              TraceBufferV2::ProtoGroupAppendResult::kStored);
    off += size;
  }
  ...
  EXPECT_THAT(ReadPacket(&props, &loss),
              ElementsAre(FakePacketFragment(next.data(), next.size())));
  EXPECT_NE(loss & DataLossReason::DATA_LOSS_CHUNK_CORRUPTED, 0u);
  EXPECT_THAT(ReadPacket(), IsEmpty());
}
