...
void ZstdPacketCompressor::PushData(const void* data, size_t size) {
  ...
  while (input.pos < input.size) {
    ...
    ZSTD_outBuffer output = {
        cur_slice_.get(), kZstdCompressSliceSize,
        kZstdCompressSliceSize -
            static_cast<size_t>(reinterpret_cast<uint8_t*>(
                                    cur_slice_.get() + kZstdCompressSliceSize) -
                                reinterpret_cast<uint8_t*>(cur_slice_.get()))};

    // Calculate current position in output buffer
    size_t output_pos = total_new_slices_size_ % kZstdCompressSliceSize;
    if (output_pos == 0 && total_new_slices_size_ > 0) {
      NewOutputSlice();
      output_pos = 0;
    }
    ...
    total_new_slices_size_ += (output.pos - output_pos);

    if (output.pos == kZstdCompressSliceSize) {
      PushCurSlice();
    }
  }
}
...
void ZstdPacketCompressor::PushCurSlice() {
  if (cur_slice_) {
    size_t slice_bytes = total_new_slices_size_ % kZstdCompressSliceSize;
    if (slice_bytes == 0 && !new_slices_.empty()) {
      slice_bytes = kZstdCompressSliceSize;
    }
    if (slice_bytes > 0) {
      new_slices_.push_back(
          Slice::TakeOwnership(std::move(cur_slice_), slice_bytes));
    }
  }
}
