...
// Counts calls to root finalization separately from Message::Finalize().
template <>
class RootMessage<RootOnlyMessage> : public RootOnlyMessage {
 public:
  RootMessage() { Reset(nullptr); }

  void Reset(ScatteredStreamWriter* writer) {
    arena_.Reset();
    Message::Reset(writer, &arena_);
  }
...
TEST(MessageHandleTest, RootHandleUsesRootFinalize) {
  ...
  EXPECT_EQ(message.finalize_count, 1u);
}

TEST(MessageHandleTest, NonRootHandleUsesMessageFinalize) {
  RootMessage<RootOnlyMessage> message;
  {
    MessageHandle<Message> handle(&message);
  }
  EXPECT_TRUE(message.is_finalized());
  EXPECT_EQ(message.finalize_count, 0u);
}
