...
// Generated by the cpp plugin.
namespace pbgen = protozero::test::protos::gen;
...
// Returns a message that can be interpreted as a LegacyFakeEvent, or a
// RealFakeEvent with BrowserExtension extension fields.
std::vector<uint8_t> CreateExtensionMessage() {
  HeapBuffered<Message> message;
  // RealFakeEvent field.
  message->AppendVarInt(1, 100);
  // BrowserExtension field.
  auto* extension_a = message->BeginNestedMessage<pbtest::SystemA>(10);
  extension_a->AppendVarInt(1, 101);
...
TEST(ProtoDecoderTest, GetFieldWithMetadata) {
  ...
  EXPECT_TRUE(legacy_decoder.has_base_int());
  EXPECT_TRUE(legacy_decoder.has_extension_a());
  EXPECT_FALSE(legacy_decoder.has_extension_b());

  EXPECT_TRUE(protozero::GetFieldWithMetadata<pbtest::LegacyFakeEvent>(
      legacy_decoder, pbtest::LegacyFakeEvent::kBaseInt));
  EXPECT_TRUE(protozero::GetFieldWithMetadata<pbtest::LegacyFakeEvent>(
      legacy_decoder, pbtest::LegacyFakeEvent::kExtensionA));
  EXPECT_FALSE(protozero::GetFieldWithMetadata<pbtest::LegacyFakeEvent>(
      legacy_decoder, pbtest::LegacyFakeEvent::kExtensionB));

  // Interpret message as RealFakeRevent. `extension_a` and `extension_b` can
  // only be accessed through GetFieldWithMetadata.
  pbtest::RealFakeEvent::Decoder real_decoder(proto.data(), proto.size());
  EXPECT_TRUE(real_decoder.has_base_int());

  EXPECT_TRUE(protozero::GetFieldWithMetadata<pbtest::RealFakeEvent>(
      real_decoder, pbtest::RealFakeEvent::kBaseInt));
  EXPECT_TRUE(protozero::GetFieldWithMetadata<pbtest::RealFakeEvent>(
...
TEST(ProtoDecoderTest, DecodeFieldWithMetadata) {
  ...
  std::optional<pbtest::SystemA::Decoder> legacy_decoder_a =
      protozero::DecodeFieldWithMetadata<pbtest::LegacyFakeEvent>(
          legacy_decoder, pbtest::LegacyFakeEvent::kExtensionA);
  ASSERT_TRUE(legacy_decoder_a.has_value());
  EXPECT_TRUE(legacy_decoder_a->has_int_a());
  EXPECT_EQ(legacy_decoder_a->int_a(), 101u);

  std::optional<pbtest::SystemB::Decoder> legacy_decoder_b =
      protozero::DecodeFieldWithMetadata<pbtest::LegacyFakeEvent>(
          legacy_decoder, pbtest::LegacyFakeEvent::kExtensionB);
  EXPECT_FALSE(legacy_decoder_b.has_value());

  // Interpret message as RealFakeRevent. `extension_a` and `extension_b` can
  // only be accessed through DecodeFieldWithMetadata.
  pbtest::RealFakeEvent::Decoder real_decoder(proto.data(), proto.size());

  std::optional<pbtest::SystemA::Decoder> extension_decoder_a =
      protozero::DecodeFieldWithMetadata<pbtest::RealFakeEvent>(
          real_decoder, pbtest::BrowserExtension::kExtensionA);
  ASSERT_TRUE(extension_decoder_a.has_value());
  EXPECT_TRUE(extension_decoder_a->has_int_a());
  EXPECT_EQ(extension_decoder_a->int_a(), 101u);

  std::optional<pbtest::SystemB::Decoder> extension_decoder_b =
      protozero::DecodeFieldWithMetadata<pbtest::RealFakeEvent>(
          real_decoder, pbtest::BrowserExtension::kExtensionB);
