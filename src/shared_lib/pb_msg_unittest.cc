...
// The public C ABI.
//
// The implementation reuses the two high bits of PerfettoPbMsg::size and
...
static_assert((PERFETTO_PB_MSG_USES_TERMINATOR_BIT &
               PERFETTO_PB_MSG_SIZE_MASK) == 0 &&
                  (PERFETTO_PB_MSG_TERMINATOR_WRITTEN_BIT &
                   PERFETTO_PB_MSG_SIZE_MASK) == 0,
...
TEST(PbMsgTest, DefaultInitIsLengthDelimited) {
  ...
  EXPECT_FALSE(PerfettoPbMsgUsesTerminator(&root));

  PerfettoPbMsg nested;
  PerfettoPbMsgBeginNested(&root, &nested, /*field_id=*/1);
  EXPECT_FALSE(PerfettoPbMsgUsesTerminator(&nested));
...
TEST(PbMsgTest, StartTagAndTerminatorEmptyNestedMessage) {
  FlatCBuffer buffer;
  PerfettoPbMsg root;
  PerfettoPbMsgInitWithEncoding(
      &root, buffer.writer(),
      PERFETTO_PB_MSG_ENCODING_START_TAG_AND_TERMINATOR);
  EXPECT_TRUE(PerfettoPbMsgUsesTerminator(&root));

  PerfettoPbMsg nested;
  PerfettoPbMsgBeginNested(&root, &nested, /*field_id=*/1);
  EXPECT_TRUE(PerfettoPbMsgUsesTerminator(&nested));
...
TEST(PbMsgTest, StartTagAndTerminatorNestedInsideNested) {
  FlatCBuffer buffer;
  PerfettoPbMsg root;
  PerfettoPbMsgInitWithEncoding(
      &root, buffer.writer(),
...
TEST(PbMsgTest, TerminatedRootEmitsNoTerminator) {
  FlatCBuffer buffer;
  PerfettoPbMsg root;
  PerfettoPbMsgInitWithEncoding(
      &root, buffer.writer(),
...
TEST(PbMsgTest, FinalizeIsIdempotentInBothModes) {
  {
    ...
    // A second call must not emit another terminator.
    EXPECT_EQ(PerfettoPbMsgFinalize(&nested), 1u);
    PerfettoPbMsgEndNested(&root);
...
TEST(PbMsgTest, SizeIsReportedWithTheStateBitsMaskedOff) {
  ...
  EXPECT_TRUE(PerfettoPbMsgUsesTerminator(&root));
  EXPECT_FALSE(PerfettoPbMsgTerminatorWritten(&root));
  ...
  EXPECT_TRUE(PerfettoPbMsgTerminatorWritten(&root));
...
TEST(PbMsgTest, LargeSizesDoNotReachTheStateBits) {
  // A size just below the four-byte varint limit must still leave both state
  ...
  EXPECT_TRUE(PerfettoPbMsgUsesTerminator(&msg));
  EXPECT_FALSE(PerfettoPbMsgTerminatorWritten(&msg));
...
TEST(PbMsgTest, CAndCppEmitIdenticalBytes) {
  EXPECT_EQ(
      BuildWithC(PERFETTO_PB_MSG_ENCODING_START_TAG_AND_TERMINATOR),
      BuildWithCpp(protozero::NestedMessageEncoding::kStartTagAndTerminator));
...
TEST(PbMsgTest, TerminatedMessagesCrossRanges) {
  // Small ranges so a few hundred bytes need several of them. In terminated
  // mode every size_field is null, so PerfettoPbMsgPatchStack() has to walk out
  // immediately instead of comparing a null pointer against the range.
  ...
  EXPECT_EQ(bytes.back(), protozero::proto_utils::kNestedMessageTerminator);
}
