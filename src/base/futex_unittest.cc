...
TEST(FutexTest, WaitReturnsValueChangedIfAlreadyDifferent) {
  if (!HasFutexSupport())
    GTEST_SKIP() << "Futex not available on this platform";

  uint32_t word = 42;
  auto result = FutexWait(&word, 0, 30000);
  EXPECT_EQ(result, FutexWaitResult::kValueChanged);
}
...
TEST(FutexTest, WaiterWokenByWake) {
  ...
  FutexWaitResult waiter_result = FutexWaitResult::kUnavailable;
  ...
  EXPECT_TRUE(waiter_result == FutexWaitResult::kWoken ||
              waiter_result == FutexWaitResult::kValueChanged);
}
