...
perfetto_tree_aggregate(A) ::= nm(F) LP nm(C) RP AS nm(N). {
    A = synq_parse_perfetto_tree_aggregate(pCtx, synq_span(pCtx, F),
        synq_span_dequote(pCtx, C), SYNTAQLITE_BOOL_FALSE,
        synq_span_dequote(pCtx, N));
}
perfetto_tree_aggregate(A) ::= nm(F) LP STAR RP AS nm(N). {
    A = synq_parse_perfetto_tree_aggregate(pCtx, synq_span(pCtx, F),
        SYNQ_NO_SPAN, SYNTAQLITE_BOOL_TRUE, synq_span_dequote(pCtx, N));
}
