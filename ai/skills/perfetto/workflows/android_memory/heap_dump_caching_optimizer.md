...
`../../infra-references/querying.md` first, then come back here to analyze it.
...
3. Retrieve the location of the heap dump if the user has not yet provided it, ask the user to provide the location either via a URL or a local file path, if it is a URL then download it to the local filesystem. then once the heap dump file is downloaded and available in a local path, then follow `../../infra-references/querying.md` to load it into trace processor and then come back here to analyze it.
...
5. Now that you have the heap dump location, execute the following PerfettoSQL queries (shipped alongside this workflow under `scripts/`) using trace processor on the heap dump file, e.g. `trace_processor query --query-file scripts/query_most_repeated_objects.sql HEAP_DUMP_FILE`.
 * Execute `scripts/query_most_repeated_objects.sql` to find the most repeated objects and their classes.
 * Execute `scripts/query_size_frequencies.sql` to find the objects whose sizes repeat the most.
...
The output of `scripts/query_most_repeated_objects.sql` will contain the following important pieces of data:
...
The output of `scripts/query_size_frequencies.sql` will contain the following important pieces of data:
* class_name: This value tells you the name of the class.
