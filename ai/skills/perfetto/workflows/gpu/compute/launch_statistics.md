...
- **`registers` and `shared_mem_*`** are the resources that most often cap
  ...
  Cross-check against the binding resource in [occupancy.md]($SKILL_ROOT/workflows/gpu/compute/occupancy.md).
...
config actually limited the kernel is answered by [occupancy.md]($SKILL_ROOT/workflows/gpu/compute/occupancy.md)
(resource limits) and [speed_of_light.md]($SKILL_ROOT/workflows/gpu/compute/speed_of_light.md) (bound type).
