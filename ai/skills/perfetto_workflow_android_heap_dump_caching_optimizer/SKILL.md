...
3. Retrieve the location of the heap dump if the user has not yet provided it, ask the user to provide the location either via a URL or a local file path, if it is a URL then download it to the local filesystem. then once the heap dump file is downloaded and available in a local path, then follow the `perfetto_infra_querying_traces` skill to load it into trace processor and then come back here to analyze it.
...
5. Now that you have the heap dump location, execute the following PerfettoSQL queries using trace processor on the heap dump file.
