...
def add_cuj_with_blocking_calls(trace, cuj_name, pid):
  ...
  trace.add_async_atrace_for_thread(ts=cuj_begin, ts_end=cuj_end, buf=cuj_name, tid=pid, pid=pid)
...
def setup_trace():
  ...
  trace.add_async_atrace_for_thread(ts=0, ts_end=5, buf="J<IGNORED>", tid=SYSUI_PID, pid=SYSUI_PID)
  return trace
