...
class Viz(TestSuite):
  ...
  def test_root_tracks_partitioning(self):
    return DiffTestBlueprint(
        trace=TextProto(r"""
        packet {
          track_descriptor {
        ...
        packet { trusted_packet_sequence_id: 1 timestamp: 1 track_event { track_uuid: 1 type: TYPE_INSTANT name: "e1" } }
        packet { trusted_packet_sequence_id: 1 timestamp: 1 track_event { track_uuid: 2 type: TYPE_INSTANT name: "e2" } }
        packet { trusted_packet_sequence_id: 1 timestamp: 1 track_event { track_uuid: 4 type: TYPE_INSTANT name: "e4" } }
        packet { trusted_packet_sequence_id: 1 timestamp: 1 track_event { track_uuid: 5 type: TYPE_INSTANT name: "e5" } }
        """),
        query="""
        INCLUDE PERFETTO MODULE viz.summary.track_event;
        SELECT name, upid IS NOT NULL AS has_upid, parent_id, order_id
        FROM _track_event_tracks_ordered_groups
        WHERE name LIKE 'GlobalRoot%' OR name LIKE 'Process1Child%' OR name IS NULL
        ORDER BY name;
        """,
        out=Csv("""
        "name","has_upid","parent_id","order_id"
        "[NULL]",1,"[NULL]",1
        "GlobalRoot1",0,"[NULL]",1
        "GlobalRoot2",0,"[NULL]",2
        "Process1Child1",1,"[NULL]",2
        "Process1Child2",1,"[NULL]",3
        """))
