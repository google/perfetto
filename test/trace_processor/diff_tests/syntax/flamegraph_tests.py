  ...
  def test_top_down(self):
    return DiffTestBlueprint(
        ...
        query="""
          ...
          INCLUDE PERFETTO MODULE std.trees.flamegraph;
          SELECT name, depth, self_value, cumulative_value, xStart, xEnd
          FROM _flamegraph_layout!((
            SELECT _tree_id, _tree_parent_id, depth, name,
        ...
        """,
        out=Csv("""
        "name","depth","self_value","cumulative_value","xStart","xEnd"
        "main",1,1,10,0,10
        "a",2,5,9,0,9
        "b",3,4,4,0,4
      """))
  ...
  def test_grouping_column(self):
    return DiffTestBlueprint(
        ...
        query="""
          ...
          INCLUDE PERFETTO MODULE std.trees.flamegraph;
          SELECT name, g, cumulative_value, xStart, xEnd
          FROM _flamegraph_layout!((
            SELECT _tree_id, _tree_parent_id, depth, name,
                   self_value, cumulative_value, g
            FROM fg(__intrinsic_flamegraph_config(
              'value', 'value', 'view', 'TOP_DOWN', 'grouping', 'g'))
          ), cumulative_value);
        """,
        out=Csv("""
        "name","g","cumulative_value","xStart","xEnd"
        "r","[NULL]",6,0,6
        "x","m2",3,0,3
        "x","m1",2,3,5
      """))
    ...
    return DiffTestBlueprint(
        ...
        query="""
          CREATE VIRTUAL TABLE fg USING __intrinsic_flamegraph((
            ...
            UNION ALL SELECT 2, 1, 'x', 2, 3, 'a', 'a'
            UNION ALL SELECT 3, 1, 'x', 3, 5, 'b', 'b'
          ));
          SELECT name, s, summary, concatenated
          FROM fg(__intrinsic_flamegraph_config(
            'value', 'value', 'view', 'TOP_DOWN',
            'aggregate', 'SUM', 's', 's',
        ...
        """,
        out=Csv("""
        "name","s","summary","concatenated"
        "r","[NULL]","root","root"
        "x",8,"a and 2 others","a,b"
      """))
