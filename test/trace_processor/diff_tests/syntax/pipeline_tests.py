...
class PerfettoPipeline(TestSuite):
  ...
  # The same totals as the descendant closure, which is what the operators
  # exist to avoid writing.
  ...
  def test_pipeline_errors_name_the_problem(self):
    return DiffTestBlueprint(
        ...
        """,
        out=ExpectedError('only SUM(column) is supported so far, not MAX'))
