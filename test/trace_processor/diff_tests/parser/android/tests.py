...
class AndroidParser(TestSuite):
  ...
  def test_android_framework_track_event_process(self):
    return DiffTestBlueprint(
        ...
        query="""
        SELECT p.pid, p.name, p.uid, t.start_seq_id, t.fw_start_ts, t.fw_end_ts, t.exit_reason, t.exit_sub_reason
        FROM __intrinsic_android_track_event_process t
        JOIN process p USING (upid);
        """,
        out=Csv("""
          "pid","name","uid","start_seq_id","fw_start_ts","fw_end_ts","exit_reason","exit_sub_reason"
          100,"com.example.app",10001,42,2000,5000,"APP_EXIT_REASON_ANR","APP_EXIT_SUBREASON_ANR_TYPE_BROADCAST_OF_INTENT"
        """))
