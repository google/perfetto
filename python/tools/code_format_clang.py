...
class ClangFormat(CodeFormatterBase):
  ...
  def __init__(self):
    super().__init__(
        name='clang-format', exts=['.cc', '.cpp', '.c', '.h', '.inc', '.proto'])
  ...
  def run_formatter(self, repo_root: str, check_only: bool, files: list[str]):
    ...
    if not os.path.exists(tool):
      ...
      return 127
    cmd = [tool, '--Werror'] + ['--dry-run' if check_only else '-i']
    cmd += files
    return self.check_call(cmd)

  def print_fix_hint(self):
    print('Run tools/format-cpp-sources to fix', file=sys.stderr)


if __name__ == '__main__':
  sys.exit(run_code_formatters([ClangFormat()]))
