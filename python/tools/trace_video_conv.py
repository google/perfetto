...
Requires ffmpeg on the PATH and the PyAV package (`pip install av`);
trace_processor is downloaded automatically unless --trace-processor points at a
local build.
...
def probe_dims(path):
  """(width, height) of the video in a file, via ffprobe."""
  proc = subprocess.run([
      'ffprobe', '-v', 'error', '-select_streams', 'v:0', '-show_entries',
      'stream=width,height', '-of', 'csv=s=,:p=0', path
  ],
                        capture_output=True,
                        text=True)
...
def probe_duration(path):
  """The video's duration in seconds, via ffprobe (0.0 if unknown)."""
  proc = subprocess.run([
