...
from perfetto.bigtrace.protos.perfetto.trace_processor.trace_processor_pb2 import QueryResult

import pandas as pd


class Bigtrace:
  # TODO(ivankc) Refactor QueryResultIterator and Row to be implemented
  # independently of Bigtrace and TraceProcessor to remove code
  # duplication
  ...
  class QueryResultIterator:
    ...
    def __init__(self, column_names, batches):
      ...
      while True:
        ...
        self.__data_lists[Bigtrace.QUERY_CELL_STRING_FIELD_ID].extend(
            strings_batch)
        self.__data_lists[Bigtrace.QUERY_CELL_VARINT_FIELD_ID].extend(
            batches[batch_index].varint_cells)
        self.__data_lists[Bigtrace.QUERY_CELL_FLOAT64_FIELD_ID].extend(
            batches[batch_index].float64_cells)
        self.__data_lists[Bigtrace.QUERY_CELL_BLOB_FIELD_ID].extend(
      ...
      if len(self.__cells) % self.__column_count != 0:
        raise Exception("Cell count " + str(len(self.__cells)) +
    ...
    def as_pandas_dataframe(self):
      ...
      for i in range(0, self.__count):
        ...
        for num in range(len(self.__column_names)):
          ...
          if col_type == Bigtrace.QUERY_CELL_INVALID_FIELD_ID:
            raise Exception('Invalid cell type')

          if col_type == Bigtrace.QUERY_CELL_NULL_FIELD_ID:
            row.append(None)
          else:
  ...
  def query(self, traces: List[str], sql_query: str):
    ...
    for response in self.stub.Query(args):
      for result in response.result:
        iterator = Bigtrace.QueryResultIterator(result.column_names,
                                                result.batch)
