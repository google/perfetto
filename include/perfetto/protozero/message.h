...
// How a message frames its nested messages on the wire. Selected once, when the
// root message is reset, immutable for that message lifetime and inherited by
// every child.
//
// kLengthDelimited is ordinary protobuf: a nested message is a wire-type-2
// field whose four-byte length is reserved up front and patched on Finalize().
//
// kStartTagAndTerminator is the tracing v2 private framing documented at
// proto_utils::kNestedMessageTerminator: a nested message opens with a
// wire-type-3 start tag and closes with the single terminator byte, so no
// length is ever backfilled after the bytes in front of it may already be
// visible to a concurrent reader. The tracing v2 relay rewrites it to ordinary
// length-delimited protobuf before the packet leaves the producer.
// The values are written into the C ABI and into PerfettoPbMsg::size, so they
// are spelled out rather than left to the compiler.
enum class NestedMessageEncoding : uint8_t {
  kLengthDelimited = 0,
  kStartTagAndTerminator = 1,
};
...
class PERFETTO_EXPORT_COMPONENT Message {
 public:
  ...
  // The two-argument form is the canonical protobuf default. It stays
  // out-of-line: it is an exported symbol of this component and code linked
  // against an older build resolves it by name.
  ...
  // Immutable for the message lifetime: set by Reset() and inherited by every
  // message created through BeginNestedMessage().
  NestedMessageEncoding nested_message_encoding() const {
    return nested_message_encoding_;
  }
  ...
  //
  // In kStartTagAndTerminator mode the bytes may carry ordinary wire types 0,
  // 1, 2 and 5, but not a standard protobuf group: the private framing reuses
  // wire type 3 as its opening marker and closes with a bare terminator, so a
  // field-numbered end-group tag has no meaning in it. This call does not parse
  // what it is given; the relay's rewriter is what rejects the resulting
  // packet.
 ...
 private:
  ...
  // the size of proto-encoded message in the pointed memory region. Always null
  // in kStartTagAndTerminator mode, which never reserves a length.
  uint8_t* size_field_;
  ...
  // See the enum's declaration. Kept apart from MessageState: the lifecycle and
  // the wire framing are independent facts about a message.
  NestedMessageEncoding nested_message_encoding_;

  // True only for a nested message created in kStartTagAndTerminator mode:
  // Finalize() then appends the single terminator byte. False for every root,
  // whose framing belongs to whoever owns the stream. Because the terminator
  // carries no field id, no opening field id has to be retained to close a
  // message.
  bool needs_terminator_;
