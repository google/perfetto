...
class PERFETTO_EXPORT_COMPONENT Message {
 public:
  ...
  enum class Encoding : uint8_t {
    ...
    kLengthDelimited = 0,
    // Append-only format for tracing v2. The reassembled packet is rewritten
    // to standard protobuf. See proto_utils::kProtoGroupEndByte.
    kProtoGroup = 1,
  };
  ...
  // child.
  void ResetWithEncoding(ScatteredStreamWriter*, MessageArena*, Encoding);
  ...
  void Reset(ScatteredStreamWriter* stream_writer, MessageArena* arena) {
    ResetWithEncoding(stream_writer, arena, Encoding::kLengthDelimited);
  }
 ...
 protected:
  ...
  // adds an unmatched closing byte, and the packet cannot be rewritten.
 ...
 private:
  ...
  // Implements Finalize() and FinalizeRoot(). |kIsRoot| drops the proto group
  // closing byte at compile time, so Message needs no runtime root flag.
