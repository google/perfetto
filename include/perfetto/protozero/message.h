...
template <typename T, bool kIsRoot>
class MessageHandle;
...
class PERFETTO_EXPORT_COMPONENT Message {
 public:
  ...
  // Encoding for nested messages. Each child inherits its parent's encoding.
  enum class NestedEncoding : uint8_t {
    // Standard protobuf. Finalize() writes each child's length into a reserved
    // field before the child's contents.
    kLengthDelimited = 0,
    // Append-only format for tracing v2. ProtoRewriter converts the reassembled
    // packet to standard protobuf. See proto_utils::kProtoGroupEndByte.
    kProtoGroup = 1,
  };

  // Initializes or resets this message with the specified encoding.
  //
  // RootMessage selects the encoding. BeginNestedMessage() passes it to each
  // child.
  void ResetWithEncoding(ScatteredStreamWriter*, MessageArena*, NestedEncoding);

  // Returns the encoding each new child inherits.
  NestedEncoding encoding() const { return encoding_; }
  ...
  // - Proto group: appends a closing byte. For a root, use
  //   RootMessage::Finalize() instead.
  //
  ...
  //
  // In proto group mode, this pointer must always be null.
  ...
  //
  // In kProtoGroup mode, |data| must not contain protobuf group fields. This
  // mode reserves wire types 3 and 4 for nested message boundaries.
 ...
 protected:
  // Finalizes the packet root and returns its encoded size.
  // In proto group mode, the packet boundary ends the root. This function
  // closes its children but adds no closing byte for the root.
  // In length-delimited mode, it behaves like Finalize().
  //
  // RootMessage and root MessageHandle instances select this function at
  // compile time. Message needs no runtime root flag.
  // Do not finalize a proto group root through Message* or T*. Their Finalize()
  // adds an unmatched closing byte, which ProtoRewriter rejects.
  uint32_t FinalizeRoot();

 private:
  // MessageHandle needs access to FinalizeRoot() through its stored Message*.
  template <typename T, bool kIsRoot>
  ...
  // Finalizes the open child and adds its encoded size to this message.
  void EndNestedMessage();

  // Implements Finalize() and FinalizeRoot(). Finalizes open children, then
  // writes the reserved length or appends a proto group closing byte.
  // If kIsRoot is true, omits the closing byte. The template removes this
  // choice at compile time without a per-message root flag.
  // Returns the encoded size. Repeated calls return it without further writes.
  template <bool kIsRoot>
  uint32_t FinalizeImpl();
