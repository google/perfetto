...
// Encoding used for nested messages.
//
...
// does not repeat the field id. The tracing v2 reader turns this back into
// length-delimited protobuf before forwarding the packet.
//
// The root chooses the encoding on Reset(); all its children use the same one.
...
class PERFETTO_EXPORT_COMPONENT Message {
 public:
  ...
  // Clears the state using length-delimited encoding.
  ...
  // Encoding selected by the root and used by all its children.
 ...
 private:
  ...
  // in kProtoGroup mode, which never reserves a length.
  uint8_t* size_field_;
  ...
  // True for nested proto-group messages; packet framing closes the root.
  bool needs_proto_group_end_;
