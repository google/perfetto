...
static inline bool PerfettoDsRegister(struct PerfettoDs* ds,
  ...
  PerfettoDsSetSupportsProtoGroupEncoding(ds_impl, true);
...
// Converts a PerfettoDsPacketEncoding to the PerfettoPbMsgEncoding with the
// same meaning. Aborts on an unknown value: this header cannot write it.
...
// Starts a packet on `tracer` and initializes `root` with the encoding that
// the writer needs.
