...
export class FilterStore {
  ...
  /**
   * Removes a specific filter from a specific group.
   * If the group becomes empty, it is deleted.
   */
  removeFilterFromGroup(
    groupId: string,
    filterPredicate: (filter: Filter) => boolean,
    ...
    this.history.push(new Map(this.groups));
    const updatedFilters = group.filters.filter((f) => !filterPredicate(f));

    if (updatedFilters.length === 0) {
      this.groups.delete(groupId);
    } else if (updatedFilters.length !== group.filters.length) {
      this.groups.set(groupId, {...group, filters: updatedFilters});
    }
