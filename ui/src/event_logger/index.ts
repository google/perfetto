...
export interface EventLogger {
   ...
   */
  logEvent(event: string, params: unknown): void;
}

/**
 * A no-op (null object) implementation of the event logger.
 * All methods have empty bodies and do nothing.
 */
...
/**
 * Global state store for event logger.
 * Uses NullEventTracker by default for safety, so no events are accidentally sent.
 */
interface EventLoggerState {
  eventLogger: EventLogger;
}

export const eventLoggerState = createStore<EventLoggerState>({
