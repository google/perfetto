...
export function trackContainFrameTag(trackUri: string): boolean {
  const trackMap = lynxPerfGlobals.state.trackUriToThreadMap.get(trackUri);
  if (!trackMap) {
    return false;
  }
  ...
  for (const value of frameDurationMap.values()) {
