  ...
  implements m.ClassComponent<AndroidGameInterventionListAttrs>
{
  view({attrs}: m.CVnode<AndroidGameInterventionListAttrs>) {
    ...
    for (const row of data) {
      if (
        row.standard_mode_supported !== null &&
        row.standard_mode_supported !== 0
      ) {
      ...
      if (row.perf_mode_supported !== null && row.perf_mode_supported !== 0) {
      ...
      if (
        row.battery_mode_supported !== null &&
        row.battery_mode_supported !== 0
      ) {
      ...
      let currentModeStr: string;
      if (row.current_mode === 1) {
        currentModeStr = 'Standard';
      } else if (row.current_mode === 2) {
        currentModeStr = 'Performance';
      } else if (row.current_mode === 3) {
        currentModeStr = 'Battery';
      } else {
        currentModeStr =
          row.current_mode !== null ? String(row.current_mode) : 'Unknown';
      }
