...
import {zodToGeminiSchema} from './zod_schema';
...
function toolsToDeclarations(request: NeutralRequest) {
...
const CAPABILITIES: ProtocolCapabilities = {
  ...
  streaming: true,
  vision: false, // Phase 1 is text-only.
};
...
export class GeminiProtocol implements Protocol {
  ...
  async *createStream(
    ...
    const model = request.params.modelName;
