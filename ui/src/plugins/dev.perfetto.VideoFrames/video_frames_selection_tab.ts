...
export class VideoFramesSelectionTab implements AreaSelectionTab {
  ...
  private async loadImage(idx: number) {
    ...
    // Delegate to the shared player, which handles both backends: v1 still images
    // (blob URL of the JPEG/WEBP bytes) and v2 hardware video (WebCodecs decode
    // from the nearest preceding key frame). The player owns the imageUrl/object
    // URL lifecycle, so we just mirror its result for rendering.
    await this.player.ensureFramesLoaded();
    await this.player.goToId(this.frames[idx].id);
    this.imageUrl = this.player.imageUrl;
    m.redraw();
  }
