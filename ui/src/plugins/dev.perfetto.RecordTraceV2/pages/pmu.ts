...
} from '../../../widgets/tabbed_split_panel';
import {v4} from 'uuid';
...
class Toggle implements m.ClassComponent<ToggleAttrs> {
  view({attrs}: m.CVnode<ToggleAttrs>) {
    return m(
      `.toggle${attrs.enabled ? '.enabled' : ''}${attrs.cssClass ?? ''}`,
      m(
        'label',
