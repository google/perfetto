...
export class PixelInputLifecycleExtension implements InputLifecycleExtension {
  ...
  getSqlJoinSpec(): SqlJoinSpec {
    return {
      ...
      tableAlias: 'pixel',
      joinOn:
        '((cast_int!(pixel.in_ts + pixel.resample_latency_offset) / 1000) * 1000) = ((cast_int!(a_evt.event_time) / 1000) * 1000)',
    };
  }
  ...
  async resolveInputId(
    ...
    const query = `
      ...
      JOIN pixel_touch_events p ON
        ((cast_int!(p.in_ts + p.resample_latency_offset) / 1000) * 1000) = ((cast_int!(e.event_time) / 1000) * 1000)
      WHERE ${whereClauses}
      LIMIT 1
    `;
