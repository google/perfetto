...
describe('parsePinIntents', () => {
  describe('Dictionary inputs', () => {
    ...
    it('parses multiple requests in an array', () => {
    ...
    it('parses JSON-encoded string dictionaries from URL startupCommands', () => {
      const input = [
        '{"cuj":"SHADE_EXPAND","process":"systemui","jankType":"sf_frames"}',
      ];
      expect(parsePinIntents(input)).toEqual([
        {
          kind: PinIntentKind.CujScopedJank,
  ...
  describe('Validation and error handling', () => {
    it('returns empty array for valid empty array or array with empty dictionary', () => {
      expect(parsePinIntents([])).toEqual([]);
      expect(parsePinIntents([{}])).toEqual([]);
    });

    it('fails fast with ZodError when non-array or invalid input is provided', () => {
      expect(() => parsePinIntents(undefined)).toThrow();
      expect(() => parsePinIntents(null)).toThrow();
      expect(() => parsePinIntents({})).toThrow();
      expect(() => parsePinIntents(123)).toThrow();
      expect(() => parsePinIntents(['invalid-json'])).toThrow();
    });
  });
});
