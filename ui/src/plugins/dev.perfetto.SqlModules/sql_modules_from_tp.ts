...
// Contains only what the TP table functions don't expose: tags, includes, and
// data availability checks. (Table importance IS exposed by TP now — see the
// `importance` column of __intrinsic_stdlib_tables — so it is read from there.)
const TABLE_META_SCHEMA = z.object({
  data_check_sql: z.string().nullish(),
});

// Importance levels as reported by the TP `importance` column; anything else
// (including the empty string for unannotated tables) maps to null.
const IMPORTANCE_LEVELS = ['core', 'high', 'mid', 'low'] as const;
type Importance = (typeof IMPORTANCE_LEVELS)[number];
function parseImportance(s: string): Importance | null {
...
export async function loadSqlModulesFromTp(
  ...
  const [modsResult, tablesResult, fnsResult, macrosResult] = await Promise.all(
    [
      ...
      engine.query(
        `SELECT module, name, type, description, exposed, importance, cols
  ...
  for (; tIter.valid(); tIter.next()) {
    ...
    getOrCreate(tablesByModule, modKey, () => []).push({
      ...
      type: tIter.type,
      importance: parseImportance(tIter.importance),
