...
export class Agent {
  ...
  async *sendMessage(
    ...
    // The model handle for the currently-selected conversational model. Looked
    // up per turn so switching models mid-conversation just routes the next
    // turn to the new model (history carries over).
    const handle = this.gateway.getDefaultModelHandle('conversational');
    if (handle === undefined) {
      yield {
        type: 'error',
        message:
          'No conversational model configured. Add a provider and select a ' +
          'model in the assistant settings.',
      };
      return;
    }
    ...
    for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
      ...
      for await (const evt of handle.createStream(
        {
  ...
  private buildToolDefs(loaded: Set<string>): NeutralToolDef[] {
