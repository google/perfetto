...
// assistant. Renders the transcript, the input box, the model picker (header),
// a cancel control for in-flight turns, and a context strip showing what the
// next prompt will carry. All conversation state lives in the ChatSession
// (owned by IntellettoPlugin, so it survives the panel unmounting); this
...
// Separator between provider id and model id in the picker's option values.
// NUL can't appear in either id (they come from JSON/settings), unlike space.
...
export class ChatPanel implements m.ClassComponent<ChatPanelAttrs> {
  ...
  private renderHeader(attrs: ChatPanelAttrs): m.Children {
    ...
    const selected = gateway.getSelectedModel();
    const value =
      selected !== undefined
        ? `${selected.provider.id}${MODEL_VALUE_SEP}${selected.model.id}`
        : '';

    return m(
      '.pf-intelletto__header',
      models.length === 0
        ? m('span.pf-intelletto__no-model', 'No models configured')
        : m(
            Select,
            {
              value,
              onchange: (e: Event) => {
                ...
                const [providerId, modelId] = v.split(MODEL_VALUE_SEP);
                gateway.setSelectedModel(providerId, modelId);
              },
            },
            models.map((rm) =>
              m(
                'option',
                {value: `${rm.provider.id}${MODEL_VALUE_SEP}${rm.model.id}`},
