...
export const anthropicProtocol: Protocol = {
  ...
  createProvider(cfg: ProviderConfig, model: string, systemPrompt: string) {
    return new AnthropicProvider(
      ...
      model,
      systemPrompt,
      cfg.baseUrl ?? DEFAULT_ANTHROPIC_BASE,
    );
  },
  async listModels(cfg: ProviderConfig): Promise<ReadonlyArray<ModelInfo>> {
    const raw = await listAnthropicModels(
      cfg.apiKey,
      cfg.baseUrl ?? DEFAULT_ANTHROPIC_BASE,
    );
    return raw.map((r) => ({id: r.id, displayName: r.display_name}));
  },
};
...
export class AnthropicProvider implements LlmProvider {
  ...
  constructor(
    ...
    model: string,
    systemPrompt: string,
    baseUrl: string = DEFAULT_ANTHROPIC_BASE,
  ) {
    this.apiKey = apiKey;
    this.model = model;
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      for (const block of response.content) {
        ...
        if (!tool) {
          ...
          onToolResult?.(block.name, errorMsg);
          continue;
        }
...
async function callApi(opts: {
  ...
  const {apiKey, baseUrl, model, system, messages, tools, signal} = opts;
