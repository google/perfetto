...
import type {NodeData} from './graph_model';
import {
  collectUpstream,
  findConnectedInputs,
  fnvHash,
...
import type {IrContext, SqlStatement} from './node_types';
import type {FromConfig} from './nodes/from';
...
export interface IrEntry {
  ...
  // Module include keys required by this entry (from `from` index).
...
// FROM index produce no entries — their real table names appear in SQL directly.
export function buildIR(
  ...
  collectUpstream(index, nodeId, visited, order);
  ...
  for (const n of order) {
    const primary = getPrimaryInput(index, n.id);
    if (primary) {
      refCount.set(primary.id, (refCount.get(primary.id) ?? 0) + 1);
    }
    // Multi-input index: count all non-primary (port >= 1) inputs.
    const manifest = getManifest(n.type);
    const ports = getManifestInputs(manifest, n);
    if (ports.length >= 2) {
      const ci = findConnectedInputs(index, n.id);
  ...
  function resolveDep(node: NodeData): string | undefined {
    ...
    // FROM index use real table names, not hashes — no dep needed.
  ...
  for (const n of order) {
    const manifest = getManifest(n.type);
    const primaryInput = getPrimaryInput(index, n.id);
    ...
    // FROM index are special: they register their table name directly
    ...
    if (manifest.emitIr) {
      ...
      const connected = findConnectedInputs(index, n.id);
      const ports = getManifestInputs(manifest, n);
