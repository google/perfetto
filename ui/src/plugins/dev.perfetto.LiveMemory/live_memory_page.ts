...
type ProcessMetric = 'rss' | 'anon' | 'file' | 'swap' | 'dmabuf';
...
const PROCESS_METRIC_OPTIONS: ReadonlyArray<{
  ...
  {key: 'rss', label: 'Total RSS', counter: 'mem.rss'},
  {key: 'anon', label: 'Anon', counter: 'mem.rss.anon'},
  {key: 'file', label: 'File', counter: 'mem.rss.file'},
  {key: 'swap', label: 'Swap', counter: 'mem.swap'},
  {key: 'dmabuf', label: 'DMA-BUF', counter: 'mem.dmabuf_rss'},
];
...
export class LiveMemoryPage implements m.ClassComponent<LiveMemoryPageAttrs> {
  ...
  private renderRecording(): m.Children {
    return m(
      ...
      m(
        ...
        m(SegmentedButtons, {
          options: [
            ...
            {label: 'Pressure & Faults', icon: 'speed'},
            {label: 'Swap', icon: 'swap_horiz'},
            {label: 'Processes', icon: 'apps'},
          ],
          selectedOption: (
            ['system', 'file_cache', 'pressure', 'swap', 'processes'] as const
      ...
      this.activeTab === 'pressure' && this.renderPressureTab(),
      this.activeTab === 'swap' && this.renderSwapTab(),
  ...
  private renderPageCacheTab(): m.Children {
    return [
      ...
      panel(
        'Mapped & Dirty Breakdown',
        'Mapped = in process page tables. Dirty = modified, pending write. ' +
          'Stacks to total file cache (estimated assuming independence).',
        this.fileCacheBreakdownData
          ? m(LineChart, {
              data: this.fileCacheBreakdownData,
              height: 250,
              xAxisLabel: 'Time (s)',
              yAxisLabel: 'Cache',
              showLegend: true,
      ...
      panel(
        'File Cache Activity',
        'Refaults = evicted pages needed again (thrashing). ' +
          'Stolen = pages reclaimed. Scanned = pages considered for reclaim.',
  ...
  private renderPressureTab(): m.Children {
    return [
      panel(
        'Memory Pressure (PSI)',
      ...
      panel(
        'Page Faults',
        'Minor = page not in TLB but in RAM. Major = page must be read from disk. High major faults indicate I/O-bound memory pressure.',
  ...
  private renderSwapTab(): m.Children {
    return [
      this.swapChartData &&
        panel(
          'Swap Usage',
  ...
  private renderProcessSection(): m.Children {
    ...
    const subtitle = cat
      ...
      : `Stacked ${metricInfo.label} per category. Click a category to drill into individual processes.`;
      ...
      m(
        ...
        m(SegmentedButtons, {
          ...
          onOptionSelected: (i: number) => {
            ...
            // Re-query charts with the new metric.
            this.categoryChartData = undefined;
            this.drilldownChartData = undefined;
            if (this.engine) {
              const t0 = this.traceT0 ?? 0;
              const counter = PROCESS_METRIC_OPTIONS[i].counter;
              this.queryCategoryTimeSeries(this.engine, t0, counter).then(
                (data) => {
  ...
  private activeProcessCounter(): string {
  ...
  private drillDownToCategory(seriesName: string) {
    ...
    if (this.engine) {
      const counter = PROCESS_METRIC_OPTIONS.find(
        (o) => o.key === this.processMetric,
      )!.counter;
      this.queryDrilldownTimeSeries(
        this.engine,
  ...
  private async cloneAndQuery(): Promise<void> {
    ...
    try {
      ...
      const [
        ...
        this.queryCategoryTimeSeries(engine, t0, this.activeProcessCounter()),
  ...
  private async querySystemTimeSeries(
    ...
    //   KernelStack + Zram + MemFree + OtherKernel = MemTotal
    // where Anon = Active(anon)+Inactive(anon),
    //       FileCache = Active(file)+Inactive(file) - Shmem.
    // Using LRU totals accounts for nearly all mapped pages, leaving a
    // small residual.
    ...
    for (const ts of timestamps) {
      ...
      const anon =
        (row.get('Active(anon)') ?? 0) + (row.get('Inactive(anon)') ?? 0);
      const fileLru =
        (row.get('Active(file)') ?? 0) + (row.get('Inactive(file)') ?? 0);
      const shmem = row.get('Shmem') ?? 0;
      // Shmem is counted inside the file LRU lists, so subtract it to get
      // the actual reclaimable file cache.
      const fileCache = Math.max(0, fileLru - shmem);
      const buffers = row.get('Buffers') ?? 0;
    ...
    const series: LineChartSeries[] = [
      ...
      {name: 'File cache', points: filePts, color: '#f39c12'},
  ...
  // ---------------------------------------------------------------------------
  // File cache breakdown: Mapped/Unmapped and Dirty/Clean.
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  // File cache activity: reclamation and thrashing rates from vmstat.
  // ---------------------------------------------------------------------------
  ...
  private async querySankeyData(
    ...
    // Level 2: File cache → Active/Inactive breakdown.
    ...
    if (activeFile > 0) {
      nodes.push({name: 'Active (hot)', color: '#e67e22', depth: 2});
      links.push({
        source: 'File cache',
        target: 'Active (hot)',
        value: activeFile,
    ...
    if (inactiveFile > 0) {
      nodes.push({name: 'Inactive (cold)', color: '#f5b041', depth: 2});
      links.push({
        source: 'File cache',
        target: 'Inactive (cold)',
        value: inactiveFile,
      });
    }
