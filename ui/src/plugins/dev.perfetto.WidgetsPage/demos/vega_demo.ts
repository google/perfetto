...
export class VegaDemo implements m.ClassComponent {
  view() {
    return renderWidgetContainer({
      label: 'VegaView',
      render: (opt) =>
        m(VegaView, {
          spec: getExampleSpec(opt.exampleSpec as SpecExample),
          data: getExampleData(opt.exampleData as DataExample),
        }),
      schema: {
        exampleSpec: enumOption(
          SpecExample.BarChart,
          Object.values(SpecExample),
        ),
        exampleData: enumOption(
          DataExample.English,
