...
export interface LynxElementAbbr {
  ...
  l: number; // left
  t: number; // top
  n: string; // name, aka tag
  id: string; // id selector
  cl: string[]; // classes
  in: Record<string, string>; // inline style
  at: Record<string, string>; // attributes
  c?: LynxElementAbbr[]; // children
}

export interface ElementDetailAttr {
  details: LynxElement[];
}
...
export interface IssuseElements {
  title: string;
  description: ReactNode;
  dataSource: LynxElement[];
  columns: Array<{
    title: ReactNode;
    dataIndex: string;
    key: string;
