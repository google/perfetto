...
function buildLatestProcessMemory(data: SnapshotData): ProcessMemoryRow[] {
  ...
  for (const [processName, counterMap] of data.processCountersByName) {
    ...
    if (rssKb === 0) continue;
    const rssByTs = counterMap.get('mem.rss');
    const rssTrendKb =
      rssByTs !== undefined
        ? [...rssByTs.entries()]
            .sort((a, b) => a[0] - b[0])
          ...
          m(
            ...
            m(
              ...
              m(
                ...
                m(SegmentedButton, {value: 'category'}, 'By Category'),
                m(SegmentedButton, {value: 'oom_score'}, 'By OOM Score'),
              ),
              m(
                SegmentedButtons,
                {
                ...
                PROCESS_METRIC_OPTIONS.map((o) =>
                  m(SegmentedButton, {value: o.key}, o.label),
                ),
              ),
            ),
          ),
...
class ProcessTable implements m.ClassComponent<ProcessTableAttrs> {
  ...
  view({attrs}: m.CVnode<ProcessTableAttrs>): m.Children {
    ...
    const columns: GridColumn[] = [
      ...
      {key: 'trend', header: m(GridCell, 'Trend')},
    ...
    return m('.pf-memscope-process-table', [
      session.isProfiling &&
        m(
