...
You can also interact with the Node Query Builder, a visual query builder:
...
- The query builder page must be open (#!/querybuilder) for graph tools to work

## Node Query Builder Graph Format

The graph JSON has this structure:
{
  "nodes": [["id", {node_data}], ...],  // Array of [id, node] tuples
  "connections": [{fromNode, fromPort, toNode, toPort}, ...],
  "labels": []
}

Every node has: id (string), type (string), x/y (number, canvas position),
and an optional nextId (string) for docking.

### Two ways to connect nodes

1. **Connections** (wired): Link output port 0 of one node to input port 0
   (or 1 for multi-input nodes like join) of another. Used when nodes are
   spatially separated on the canvas.

2. **Docking** (nextId): Set the parent node's nextId to the child node's id.
   The child visually stacks directly below the parent with no wire. This is
   the preferred way to chain single-input operations (e.g. from -> filter ->
   sort -> limit). Docked nodes don't need separate x/y positions or a
   connection entry - they inherit position from their parent.

Use docking for linear chains (from -> filter -> select -> sort).
Use connections for multi-input nodes (join, union_all, interval_intersect,
extend) where you need to wire left and right inputs separately.

### Node Types

**from** - Table source. Fields: table (string, e.g. "slice", "process").

**select** - Choose/rename columns. Fields:
  columns: {"col1": true, "col2": false, ...} (checked = included),
  expressions: [{expression: "ts/1e6", alias: "ts_ms"}, ...]

**filter** - Filter rows. Fields:
  conditions: [{column: "dur", op: ">", value: "1000000"}, ...],
  conjunction: "AND" | "OR".
  Operators: =, !=, >, >=, <, <=, LIKE, NOT LIKE, GLOB, IS NULL, IS NOT NULL.

**sort** - Order rows. Fields:
  conditions: [{column: "dur", order: "DESC"}, ...].

**limit** - Limit row count. Fields: limitCount: "100" (string).

**groupby** - GROUP BY with aggregates. Fields:
  groupColumns: ["name", "category"],
  aggregations: [{func: "COUNT"|"SUM"|"AVG"|"MIN"|"MAX", column: "dur", alias: "total_dur"}].

**distinct** - Remove duplicates. Fields:
  distinctColumns: {"col1": true, "col2": true, ...}.

**join** - Join two inputs (left=port 0, right=port 1). Fields:
  joinType: "INNER"|"LEFT"|"RIGHT"|"FULL",
  joinConditionType: "ON"|"USING",
  joinOn: "l.id = r.parent_id" (when ON),
  joinUsing: "utid" (when USING),
  leftColumns/rightColumns: {"col": true, ...},
  columnAliases: {"r.name": "right_name", ...}.

**union_all** - Union two inputs. Fields: distinct: boolean.

**interval_intersect** - Intersect time intervals from two inputs. Fields:
  partitionColumns: {"cpu": true, ...},
  filterNegativeDur: boolean.

**extend** - LEFT JOIN to add columns from right input. Fields:
  leftColumn: "id", rightColumn: "parent_id",
  columns: [{column: "name", alias: "parent_name"}, ...].

**extract_arg** - Extract args from arg_set_id. Fields:
  extractions: [{column: "arg_set_id", argName: "key_name", alias: "alias"}, ...].

**selection** - Use the current UI time selection as a source. Fields:
  ts: "123456789" (bigint as string), dur: "1000000".

### Example: Docked chain (slice -> filter dur > 1ms -> sort by dur)
{
  "nodes": [
    ["n1", {"id":"n1","type":"from","table":"slice","x":150,"y":100,
      "nextId":"n2"}],
    ["n2", {"id":"n2","type":"filter","x":0,"y":0,
      "conditions":[{"column":"dur","op":">","value":"1000000"}],
      "conjunction":"AND","filterExpression":"",
      "nextId":"n3"}],
    ["n3", {"id":"n3","type":"sort","x":0,"y":0,
      "conditions":[{"column":"dur","order":"DESC"}],
      "sortColumn":"dur","sortOrder":"DESC"}]
  ],
  "connections": [],
  "labels": []
}

### Example: Join (slice joined with thread on utid)
{
  "nodes": [
    ["n1", {"id":"n1","type":"from","table":"slice","x":100,"y":100}],
    ["n2", {"id":"n2","type":"from","table":"thread","x":100,"y":250}],
    ["n3", {"id":"n3","type":"join","x":350,"y":150,
      "joinType":"LEFT","joinConditionType":"USING","joinUsing":"utid",
      "joinOn":"","leftColumns":{},"rightColumns":{},"columnAliases":{}}]
  ],
  "connections": [
...
const SIDE_PANEL_URI = 'dev.perfetto.AiAssistant#chat';
...
export default class AiAssistantPlugin implements PerfettoPlugin {
  static readonly id = 'dev.perfetto.AiAssistant';
