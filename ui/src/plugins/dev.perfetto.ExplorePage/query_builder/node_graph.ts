...
export class NodeGraph implements m.ClassComponent<NodeGraphAttrs> {
  ...
  oncreate({dom}: m.VnodeDOM<NodeGraphAttrs>) {
    ...
    this.canvasWidth = box.getBoundingClientRect().width;
  ...
  private renderEmptyCanvas(attrs: NodeGraphAttrs) {
    return m(
...
function findNextAvailablePosition(
  ...
  // Fallback if no candidates are valid (e.g. canvas is full)
