...
const MAX_AUTO_REVEALED_FLAMECHARTS = 16;
...
export default class StackSamplesPlugin implements PerfettoPlugin {
  ...
  private async addTracksForSource(
    ...
    if (flamechartNodes.length > 0) {
      ...
      // threads reveal the child, unless there are too many of them. These
      // defaults run once, never during pan or zoom.
      trace.onTraceReady.addListener(() => {
        ...
        const toReveal = flamechartNodes.filter(
          ({utid}) => !utidsWithSlices.has(utid),
        );
        // Revealing expands every ancestor group, which would flood the
        // workspace on traces with many sampled threads.
        if (toReveal.length > MAX_AUTO_REVEALED_FLAMECHARTS) return;
        for (const {node} of toReveal) node.reveal();
      });
    }
  }
