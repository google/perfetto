  ...
  oninit: ({attrs, state}) => {
    ...
    const load = async () => {
      const findFirstWithMetrics = async (): Promise<KernelMetricData[]> => {
        ...
        return (
          (await fetchSelectedKernelMetricData(attrs.engine, firstId)) ?? []
        );
      };
      ...
      if (attrs.sliceId != null) {
        data =
          (await fetchSelectedKernelMetricData(attrs.engine, attrs.sliceId)) ??
          [];
        if (data.length === 0) {
