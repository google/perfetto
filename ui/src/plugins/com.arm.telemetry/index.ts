...
export default class implements PerfettoPlugin {
  ...
  private getComputableMetrics(
    ...
    const cpuSpecs = new Map<number, ArmTelemetryCpuSpec>(
      [...cpuids.entries()]
        .filter(([_, v]) => {
          return this._ctx!.cpuInfos.registeredCpuids().includes(v);
        })
        .map(([k, v]) => {
          return [k, this._ctx!.cpuInfos.getCpuDesc(v)];
        }),
    );

    // For each cpu find applicable metrics based on its available counters
    const allApplicableMetrics: ApplicableMetricDesc[] = [...cpuSpecs.entries()]
      .map(([cpu, spec]) => {
        const counters = cpuCountersMap.get(cpu)!;
        const metrics: ApplicableMetricDesc[] = Object.entries(spec.metrics)
          .filter(([_name, metric]) =>
            metric.events.every((e) => counters.includes(e)),
          )
          .map(([name, metric]) => {
            const result: ApplicableMetricDesc = {
              name,
              cpus: [cpu],
