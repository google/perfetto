...
import {ENUM_RENDERERS} from './enums';
...
export function gridSchema(
  ...
  renderers?: {readonly [col: string]: (value: unknown) => m.Children},
): SchemaRegistry {
  ...
  for (const col of columns) {
    root[col] = {
      title: col,
      // Resolution order: caller-supplied renderer, then the shared enum/flags
      // renderer (proc-state, sched-group, capability/FGS/hosting bitmasks…),
      // then the pid-link renderer, else the raw value.
      cellRenderer:
        renderers?.[col] ??
        ENUM_RENDERERS[col] ??
