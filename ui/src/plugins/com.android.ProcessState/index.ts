...
// Sort key for processes without a known UID so they sort after numbered UIDs.
const NULL_UID_SORT_ORDER = 2147483647;
...
export default class ProcessState implements PerfettoPlugin {
  ...
  private async createConcurrencyTracks(ctx: Trace): Promise<TrackNode> {
    ...
    for (
      const it = states.iter({state: STR, name: STR});
      it.valid();
      it.next()
    ) {
      const {state, name} = it;
      ...
      summary.addChildLast(new TrackNode({uri, name}));
    }
  ...
  private async createProcessTracks(ctx: Trace): Promise<TrackNode> {
    ...
    const processes = await ctx.engine.query(`
      ...
      GROUP BY upid
      ORDER BY ifnull(uid, ${NULL_UID_SORT_ORDER}), min(ts), pid
    `);
