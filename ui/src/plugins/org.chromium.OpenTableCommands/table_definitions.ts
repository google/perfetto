...
// Simplified table definition structure for manual table definitions
...
/**
 * Converts a SqlTableDefinition to a SqlTable for use with openTableExplorer
 */
function sqlTableDefToSqlTable(def: SqlTableDefinition): SqlTable {
  const columns: SqlColumn[] = def.columns.map((col) => {
    // Handle both simple column names and complex column definitions
    const columnName =
  ...
  const result: SqlTable & {imports?: string[]} = {
    name: def.name,
...
// Export the table definitions as SqlTable objects
export function getThreadTable(): SqlTable {
  return sqlTableDefToSqlTable(THREAD_TABLE);
}

export function getProcessTable(): SqlTable {
  return sqlTableDefToSqlTable(PROCESS_TABLE);
}

export function getSliceTable(): SqlTable {
  return sqlTableDefToSqlTable(SLICE_TABLE);
}

export function getAndroidLogsTable(): SqlTable {
  return sqlTableDefToSqlTable(ANDROID_LOGS_TABLE);
}

export function getSchedTable(): SqlTable {
  return sqlTableDefToSqlTable(SCHED_TABLE);
}

export function getThreadStateTable(): SqlTable {
  return sqlTableDefToSqlTable(THREAD_STATE_TABLE);
}
