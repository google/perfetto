...
// Look up a terminology by ID.
// Falls back to the first registered terminology if the ID is not found.
export function getTerminology(id: string): Terminology {
  const t = terminologies.get(id);
  if (t) return t;
