...
function kernelQueryBody(): string {
...
export function buildKernelQuery(sliceIdFilter: number): string {
  const whereFilter = `
...
function reduceKernelRows(iter: RowIter): Map<number, KernelGroup> {
  ...
  const aggregations = getCounterAggregations();
...
function buildMetricSectionData(
  ...
  const terminology = getTerminology(ctx.terminologyId);
  ...
  let sections: MetricSection[] = getSections()
    .map((section) => ({
  ...
  const durationId = getWellKnownMetricId('duration', availableMetrics);
  const cyclesId = getWellKnownMetricId('cycles', availableMetrics);
  const freqId = getWellKnownMetricId('frequency', availableMetrics);
...
export async function fetchSelectedKernelMetricData(
  ...
  const kernelQuery = buildKernelQuery(sliceId);
  const kernelResult = await engine.query(kernelQuery);
  const groups = reduceKernelRows(kernelResult.iter({}));
...
//
// On init, loads the metric data for the selected slice (or finds the
// first kernel with metrics), builds section collapse state, and renders
  ...
  oninit: ({attrs, state}) => {
    ...
    const load = async () => {
      const findFirstWithMetrics = async (): Promise<KernelMetricData[]> => {
        // Find the first compute slice that has counter data using a
        // single query instead of iterating through all launches.
        const sql = `
          SELECT s.id
          FROM gpu_slice s
          INNER JOIN gpu_track tr ON tr.id = s.track_id
      ...
      // Collapse sections with collapsedByDefault
      const registeredSections = getSections();
