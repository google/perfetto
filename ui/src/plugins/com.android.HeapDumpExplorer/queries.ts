...
import {shortClassName, DOMINATOR_TREE_PREAMBLE} from './components';

// ─── Helpers ──────────────────────────────────────────────────────────────────

async function requireDominatorTree(engine: Engine): Promise<void> {
  await engine.query(DOMINATOR_TREE_PREAMBLE);
}
...
async function batchBitmapBufferHashes(
  ...
  // Read pre-computed hashes from the trace processor.
  const ids = [...bufToBitmaps.keys()].join(',');
  const bufRes = await engine.query(`
    SELECT od.object_id AS buf_id, od.array_data_hash AS hash
    FROM heap_graph_object_data od
    WHERE od.object_id IN (${ids})
...
export async function getOverview(engine: Engine): Promise<OverviewData> {
  ...
  // Hash pixel buffers for content-based grouping.
  ...
  // Group by content hash — skip bitmaps without a hash.
    ...
    if (arrayDataId !== null && arrayElementType !== null) {
      ...
      const jsonRes = await engine.query(`
        SELECT __intrinsic_heap_graph_get_array_json(
          ${arrayDataId}, '${arrayElementType}', ${arrayElementCount ?? 0}
        ) AS data
      `);
...
export async function getHeapGraphTrackInfo(
  ...
  // Look up path hash in dominator tree first, then class tree (only present
  // if the HeapProfile plugin has rendered a flamegraph).
