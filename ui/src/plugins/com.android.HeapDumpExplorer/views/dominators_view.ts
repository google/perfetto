...
function dominatorsQuery(showReachable: boolean): string {
...
function DominatorsView(): m.Component<DominatorsViewAttrs> {
  ...
  function createDataSource(engine: Engine) {
    dataSource = new SQLDataSource({
      engine,
      sqlSchema: createSimpleSchema(dominatorsQuery(showReachable)),
  ...
  return {
    oninit(vnode) {
      createDataSource(vnode.attrs.engine);
    },
    view(vnode) {
