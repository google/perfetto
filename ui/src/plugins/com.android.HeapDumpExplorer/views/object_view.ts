...
function ObjectView(): m.Component<ObjectViewAttrs> {
  ...
  function renderSectionWithToggle(
    sectionKey: string,
    title: string,
    enrich: () => Promise<void>,
    content: m.Children,
    ...
    const isOn = reachableSections.has(sectionKey);
    return m(
      Section,
      {title, defaultOpen: defaultOpen !== false},
      m('div', {class: 'ah-heading-row ah-mb-3'}, [
        m(ReachableToggle, {
          checked: isOn,
          onchange: (v: boolean) => {
            if (v) {
              reachableSections.add(sectionKey);
              enrich()
                .then(() => m.redraw())
                .catch(console.error);
            } else {
              reachableSections.delete(sectionKey);
            }
            m.redraw();
          },
        }),
      ...
      return m('div', {class: 'ah-view-scroll ah-view-stack'}, [
        ...
        detail.isClassObj
          ? renderSectionWithToggle(
              ...
              'Static Fields',
              () =>
                queries.enrichFieldsWithReachable(
                  vnode.attrs.engine,
        ...
        detail.isArrayInstance
          ? renderSectionWithToggle(
              'array',
              `Array Elements (${detail.arrayLength})`,
              () =>
                queries.enrichArrayElemsWithReachable(
                  vnode.attrs.engine,
                  detail !== null && detail !== 'loading'
                    ? detail.arrayElems
                    : [],
        ...
        detail.reverseRefs.length > 0
          ? renderSectionWithToggle(
              ...
              m(
                ...
                {class: 'ah-subtable'},
                m(DataGrid, {
                  key: `refs-${String(reachableSections.has('refs'))}`,
                  ...
                  initialColumns: [
                    ...
                    ...(reachableSections.has('refs')
                      ? [
                          {id: 'reachable_size', field: 'reachable_size'},
                          {
                            id: 'reachable_native',
                            field: 'reachable_native',
                          },
                          {
                            id: 'reachable_count',
        ...
        detail.dominated.length > 0
          ? renderSectionWithToggle(
              ...
              m(
                ...
                {class: 'ah-subtable'},
                m(DataGrid, {
                  key: `dom-${String(reachableSections.has('dom'))}`,
                  schema: makeInstanceSchema(navigate),
                  rootSchema: 'query',
