...
interface ObjectsViewAttrs {
  ...
  readonly params: ObjectsParams;
}

function objectsQuery(cls: string, heap: string | null): string {
...
function ObjectsView(): m.Component<ObjectsViewAttrs> {
  ...
  function setup(engine: Engine, cls: string, heap: string | null) {
    const query = objectsQuery(cls, heap);
  ...
  return {
    oninit(vnode) {
      ...
      prevClassName = cls;
      prevHeap = heap;
      setup(vnode.attrs.engine, cls, heap);
    },
    onupdate(vnode) {
      const cls = vnode.attrs.params.className ?? '';
      const heap = vnode.attrs.params.heap ?? null;
    ...
    view(vnode) {
      ...
      return m('div', {class: 'ah-view-content'}, [
        m('h2', {class: 'ah-view-heading'}, counter.heading('Instances')),
