...
export async function loadBaseline(opts: LoadOptions): Promise<LoadedBaseline> {
  ...
  try {
    ...
    // Pre-warm the stdlib materializations the cross-trace diff tabs
    // need. The first query into these heavy `CREATE PERFETTO TABLE`
    // views pays an O(heap) build cost (~1.5–2 s on a 60 MB hprof);
    // subsequent queries are instant filters. Fire-and-forget here so
    // the user can navigate Overview / Flamegraph while the build runs
    // in the worker — by the time they click Classes or Dominators
    // it's already materialized.
    void prewarmDiffMaterializations(engine);
...
// Trigger the heavy stdlib `CREATE PERFETTO TABLE`s the diff views
// rely on. Errors are swallowed — pre-warm is opportunistic; the diff
// view will retry on user navigation if this fails.
async function prewarmDiffMaterializations(
  engine: WasmEngineProxy,
): Promise<void> {
  try {
    await engine.query(`
      INCLUDE PERFETTO MODULE android.memory.heap_graph.heap_graph_class_aggregation;
      SELECT 1 FROM android_heap_graph_class_aggregation LIMIT 1;
    `);
  } catch {
    // Engine may have been disposed before the prewarm finished — fine.
  }
}
