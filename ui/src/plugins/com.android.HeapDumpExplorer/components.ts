...
interface SectionAttrs {
  ...
  // Optional inline actions rendered to the right of the title (e.g. a
  // "View in Flamegraph" button on a path section). Click events on the
  // actions area are isolated from the section toggle via stopPropagation
  // — clicking an action shouldn't accidentally collapse the section.
          ...
          vnode.attrs.actions !== undefined
            ? m(
                'div',
                {
                  ...
                  // Don't toggle the section when interacting with actions.
