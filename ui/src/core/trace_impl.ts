...
class TraceStorage<T> implements Storage<T> {
  ...
  get(): T {
    ...
    } else if (this.desc.defaultValue !== undefined) {
      value = this.desc.defaultValue;
    } else {
      // Assume the zod schema is sufficiently flexible to handle 'undefined'
      // otherwise the plugin will crash on a clean start which will be
      // immediately obvious.
      value = this.desc.schema.parse(undefined);
    }
    const normalizedValue = deepFreeze(value);
