...
export class SettingsManagerImpl implements SettingsManager {
  ...
  constructor(store: Storage, parentRegistry?: Registry<SettingImpl<unknown>>) {
    this.store = store;
    this.registry = parentRegistry ? parentRegistry.createChild() : new Registry<SettingImpl<unknown>>((setting) => setting.id);
    this.load();
  }
  ...
  register<T>(setting: SettingDescriptor<T>): Setting<T> {
    ...
    settingImpl.adopt(this.registry.register(settingImpl as SettingImpl<unknown>));
