...
import WasmModuleGen from '../gen/proto_text_utils';

/**
 * This file is the TS-equivalent of src/proto_text_utils.
...
/**
 * Convert a binary-encoded TraceMetricV2Spec to text proto format.
 */
export async function traceMetricV2SpecToText(
  spec: Uint8Array | protos.ITraceMetricV2Spec,
): Promise<string> {
  return pbToText(
    spec,
    protos.TraceMetricV2Spec.encode,
    'trace_metric_v2_spec_to_text',
  );
}
...
async function initWasmOnce(): Promise<WasmModule> {
  if (moduleInstance === undefined) {
    ...
    const resp = await fetch(assetSrc('proto_text_utils.wasm'));
    ...
    const bufAddr =
      instance.ccall('proto_text_utils_buf', 'number', [], []) >>> 0;
    const bufSize =
      instance.ccall('proto_text_utils_buf_size', 'number', [], []) >>> 0;
    moduleInstance = {
      module: instance,
