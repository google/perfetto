...
/**
 * Convert a binary-encoded SynthesizeAudioArgs to text proto format.
 */
export async function synthArgsToText(
  args: Uint8Array | protos.ISynthesizeAudioArgs,
): Promise<string> {
  return pbToText(
    args,
    protos.SynthesizeAudioArgs.encode,
    'synth_args_pb_to_txt',
  );
}

/**
 * Convert a text proto (pbtxt) string to a proto-encoded SynthesizeAudioArgs.
 */
export async function synthArgsToPb(
  argsTxt: string,
): Promise<Result<Uint8Array>> {
  return textToPb(argsTxt, 'synth_args_txt_to_pb');
}
