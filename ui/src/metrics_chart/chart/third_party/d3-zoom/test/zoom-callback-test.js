...
it('zoom.on(\'zoom\') callback', () => {
  ...
  const event = new ZoomEvent('zoom', {sourceEvent: null, target: z,
...
it('zoom.on(\'start\') callback', () => {
  ...
  div.call(z.transform, zoomIdentity);
  const event = new ZoomEvent('start', {sourceEvent: null, target: z,
    transform: zoomIdentity});
  assert.deepStrictEqual(a, {event, d: 'hello', that: div.node()});
  a = {};
  z.on('start', null);
...
it('zoom.on(\'end\') callback', () => {
  ...
  div.call(z.transform, zoomIdentity);
  const event = new ZoomEvent('end', {sourceEvent: null, target: z,
    transform: zoomIdentity});
  assert.deepStrictEqual(a, {event, d: 'hello', that: div.node()});
  a = {};
  z.on('end', null);
