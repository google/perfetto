...
class SlicePlugin implements PerfettoPlugin {
  ...
  async onTraceLoad(ctx: Trace): Promise<void> {
    ...
    await this.addGlobalSliceTracks(ctx);
    await this.addProcessSliceTracks(ctx);
    await this.addThreadSliceTracks(ctx);
    await this.addUserSliceTracks(ctx);
  ...
  async addGlobalSliceTracks(ctx: Trace): Promise<void> {
    const {engine} = ctx;
    ...
    const rawGlobalTracks = await engine.query(`
    ...
    for (; it.valid(); it.next()) {
      ...
      if (maxDepth === null) {
        ...
        ctx.tracks.registerTrack({
          ...
          track: new SliceTrack({trace: ctx, uri}, maxDepth, trackIds),
  ...
  async addProcessSliceTracks(ctx: Trace): Promise<void> {
    const result = await ctx.engine.query(`
      select
        upid,
        t.name as trackName,
  ...
  async addThreadSliceTracks(ctx: Trace): Promise<void> {
    const result = await ctx.engine.query(`
  ...
  async addUserSliceTracks(ctx: Trace): Promise<void> {
    const {engine} = ctx;
