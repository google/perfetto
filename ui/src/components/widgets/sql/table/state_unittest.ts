...
test('sqlTableState: columnManipulation', () => {
...
test('sqlTableState: getBaseQuery', () => {
  const trace = createFakeTraceImpl({allowQueries: true});
  const state = new SqlTableState(trace, table);

  // Check the generated base SQL query (no filters, sorting, or pagination).
  expect(normalize(state.getBaseQuery())).toBe(
    'SELECT table_0.id AS id, table_0.name AS name, table_0.ts AS ts FROM table AS table_0',
  );
});
