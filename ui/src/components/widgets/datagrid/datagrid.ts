...
export class DataGrid implements m.ClassComponent<DataGridAttrs> {
  ...
  view({attrs}: m.Vnode<DataGridAttrs>) {
    ...
    return m(
      ...
      m(DataGridToolbar, {
        ...
        drillDownFields: this.pivot?.groupBy.map(({field}) => field),
  ...
  private async formatData(
    ...
    switch (format) {
      case 'tsv': {
        const columnNames = buildColumnNames(schema, rootSchema, columns);
  ...
  private formatFilter(
    ...
    // Get the cell formatter for this column, falling back to default
    const formatter = colInfo?.cellFormatter ?? defaultValueFormatter;

    // Create a dummy row for the formatter (some formatters may use row context)
    const dummyRow: Row = {};

    // Format a value, wrapping strings in muted quotes
    const formatValue = (v: SqlValue): m.Children => {
      const formatted = formatter(v, dummyRow);
      if (typeof v === 'string') {
        return m('span', [
