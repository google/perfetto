...
/**
 * Compute CDF from an array of values.
 *
 * @param values Array of numeric values
 * @param numPoints Number of points to compute
 * @returns Array of CDF points
 */
function computeCDFPoints(
  values: number[],
  ...
  if (min === max) {
    return [{value: min, probability: 1.0}];
  }
...
export class InMemoryCDFLoader implements CDFLoader {
  ...
  use(config: CDFLoaderConfig): CDFLoaderResult {
    ...
    for (const [groupName, groupValues] of grouped) {
      const points = computeCDFPoints(groupValues, config.points ?? 100);
      if (points.length > 0) {
        lines.push({
  ...
  use(config: CDFLoaderConfig): CDFLoaderResult {
    ...
    const result = this.querySlot.use({
      ...
      queryFn: async () => {
        ...
        // Query to get sorted values (with optional grouping)
        const orderByClause = this.groupCol
          ? `${this.groupCol}, ${this.valueCol}`
          : this.valueCol;
        ...
        const sql = `
          ...
          _stats AS (
            SELECT
              ...
              ${this.groupCol ? ', grp' : ''}
            FROM _data
            ${this.groupCol ? 'GROUP BY grp' : ''}
          )
          SELECT
            _data.value,
            ${this.groupCol ? '_data.grp,' : ''}
            ROW_NUMBER() OVER (${this.groupCol ? 'PARTITION BY _data.grp' : ''} ORDER BY _data.value) AS row_num,
            _stats.total_count
          FROM _data
          JOIN _stats ${this.groupCol ? 'ON _data.grp = _stats.grp' : ''}
        `;

        const queryResult = await this.engine.query(sql);

        // Build CDF from results
        ...
        for (; iter.valid(); iter.next()) {
          ...
          const rowNum = iter.row_num;
          const totalCount = iter.total_count;

          if (!isValidNumber(value)) continue;

          const probability = rowNum / totalCount;
          const groupName =
          ...
          if (
            ...
            probability - points[points.length - 1].probability >= 1 / numPoints
          ) {
            points.push({value, probability});
          }
        ...
        // Convert to CDFLine array
        const lines: CDFLine[] = [];
        for (const [groupName, points] of grouped) {
          lines.push({
            name: groupName === 'default' && !this.groupCol ? 'CDF' : groupName,
            points,
          });
        }
