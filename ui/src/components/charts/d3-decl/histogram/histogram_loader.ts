...
export interface HistogramLoaderConfig {
  ...
  /**
   * DataGrid-style filters. The loader will extract relevant numeric range filters
   * (>= and <=) and apply them to the data.
   */
...
export class InMemoryHistogramLoader implements HistogramLoader {
  ...
  use(config: HistogramLoaderConfig): HistogramLoaderResult {
    ...
    // Compute histogram - always recompute, no caching
  ...
  use(config: HistogramLoaderConfig): HistogramLoaderResult {
    ...
    const result = this.querySlot.use({
      ...
      queryFn: async () => {
        ...
        // Extract numeric range filters (>= and <=)
        ...
        if (config.filters && config.filters.length > 0) {
          ...
          for (const filter of config.filters) {
            if (filter.op === '>=' && typeof filter.value === 'number') {
              minVal = Math.max(minVal, filter.value);
            } else if (filter.op === '<=' && typeof filter.value === 'number') {
              maxVal = Math.min(maxVal, filter.value);
            }
          }

          if (minVal !== -Infinity) filterMin = minVal;
          if (maxVal !== Infinity) filterMax = maxVal;
        }

        const filterClause =
          filterMin !== undefined && filterMax !== undefined
            ? `WHERE ${col} >= ${filterMin} AND ${col} <= ${filterMax}`
            : filterMin !== undefined
              ? `WHERE ${col} >= ${filterMin}`
              : filterMax !== undefined
                ? `WHERE ${col} <= ${filterMax}`
