...
export class Histogram implements m.ClassComponent<HistogramAttrs> {
  ...
  view({attrs}: m.Vnode<HistogramAttrs>): m.Children {
    ...
    return m(
      '.pf-histogram',
      {
        class: classNames(fillParent && 'pf-histogram--fill-parent', className),
        style,
      },
      [
        m(
    ...
    return buckets
      .map((bucket, i) => {
        ...
        return m('rect.pf-histogram__bar', {
          ...
          height: barHeight,
          class: classNames(isHovered && 'pf-histogram__bar--hover'),
          onmouseenter: () => {
  ...
  private renderTooltip(
    ...
    return m(
      ...
      m('.pf-histogram__tooltip-content', [
        ...
        m('.pf-histogram__tooltip-row', formatYValue(bucket.count)),
      ]),
    );
  }
}
