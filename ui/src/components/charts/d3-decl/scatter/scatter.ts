...
export class ScatterPlot implements m.ClassComponent<ScatterPlotAttrs> {
  ...
  view({attrs}: m.Vnode<ScatterPlotAttrs>): m.Children {
    ...
    return m(
      ...
      {
        class: classNames(
          fillParent && 'pf-scatter-plot--fill-parent',
          className,
        ),
          ...
          [
            ...
            m(
              ...
              [
                ...
                m('rect.pf-scatter-plot__background', {
                  x: 0,
                  y: 0,
                  width: chartWidth,
                  height: chartHeight,
                ...
                ...data.points.map((point) => {
                  ...
                  const color = point.category
                    ...
                    : pointColor || 'var(--pf-scatter-plot-point-color)';
                  const isHovered = this.hoveredPoint === point;

                  return m('circle.pf-scatter-plot__point', {
                    cx,
                    cy,
                    r: isHovered ? pointSize * 1.5 : pointSize,
                    fill: color,
                    class: classNames(
                      isHovered && 'pf-scatter-plot__point--hover',
                    ),
                    onmouseenter: () => {
            ...
            showCorrelation &&
              ...
              m(
                ...
                [
                  m(
                    'text.pf-scatter-plot__correlation-label',
                    {
                      'x': -18,
                      'y': 6,
                      'text-anchor': 'end',
                  ...
                  m(
                    'text.pf-scatter-plot__correlation-value',
                    {
                      'x': -18,
                      'y': 22,
                      'text-anchor': 'end',
  ...
  private renderBrushOverlay(
    ...
    return m('rect.pf-scatter-plot__brush-selection', {
      x,
      y,
      width: w,
      height: h,
  ...
  private createColorScale(
    ...
    if (!categories || categories.length === 0) {
      return () => singleColor || 'var(--pf-scatter-plot-point-color)';
    }
    const scale = d3
      .scaleOrdinal<string>()
      .domain([...categories])
  ...
  private renderCorrelationLine(
    ...
    return m('line.pf-scatter-plot__correlation-line', {
      x1: xScale(x1),
      y1: yScale(y1),
      x2: xScale(x2),
      y2: yScale(y2),
  ...
  private renderLoading(
    ...
    return m(
      ...
      m('.pf-scatter-plot__loading', m(Spinner)),
    );
  }
  ...
  private renderEmpty(
    ...
    return m(
      ...
      m('.pf-scatter-plot__empty', 'No data to display'),
    );
  }
  ...
  private renderTooltip(
    ...
    return m(
      '.pf-scatter-plot__tooltip',
      m('.pf-scatter-plot__tooltip-content', [
        ...(point.label
          ? [m('.pf-scatter-plot__tooltip-row', point.label)]
          : []),
        ...(point.category
          ? [m('.pf-scatter-plot__tooltip-row', point.category)]
          : []),
        m(
          '.pf-scatter-plot__tooltip-row',
