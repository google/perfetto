...
/**
 * Simple bar chart component (single series, categorical X-axis).
 *
 * Displays categorical data as vertical bars, supporting:
 * - Multi-category brush selection (drag across categories)
 * - Click anywhere to clear filters
 * - Hover tooltips
 * - Sorting by category or value
 *
...
import {SimpleBarChartAttrs, BarDatum} from './bar_types';
 ...
 */
export class SimpleBarChart implements m.ClassComponent<SimpleBarChartAttrs> {
  ...
  view({attrs}: m.Vnode<SimpleBarChartAttrs>) {
    const {
      data,
      filters,
      column,
    ...
    const sortedBars = sort ? sortBars(data.bars, sort, false) : data.bars;
        ...
        m(
          ...
          m(
            ...
            [
              ...
              // Brush selection overlay (visual feedback during drag)
