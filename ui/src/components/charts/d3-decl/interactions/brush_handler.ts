...
export class BrushHandler1D {
  ...
  private handlePointerMove(e: PointerEvent): void {
    if (this.startValue === undefined) return;
  ...
  private handlePointerUp(e: PointerEvent): void {
    ...
    if (isDrag) {
      ...
      // Click on empty space - clear filters
      this.onClickEmpty();
    }
  ...
  private clientXToValue(e: PointerEvent): number {
    ...
    const chartX = (svgX / rect.width) * this.svg.viewBox.baseVal.width;
    const dataX = chartX - this.margin.left;

    // Clamp dataX to chart bounds [0, chartWidth]
    const clampedX = Math.max(0, Math.min(this.chartWidth, dataX));
...
export class BrushHandler2D {
  ...
  private clientToValue(e: PointerEvent): {x: number; y: number} {
    ...
    const chartX = (svgX / rect.width) * viewBox.width;
    const chartY = (svgY / rect.height) * viewBox.height;

    const dataX = chartX - this.margin.left;
    const dataY = chartY - this.margin.top;

    // Clamp coordinates to chart bounds
    const clampedX = Math.max(0, Math.min(this.chartWidth, dataX));
    const clampedY = Math.max(0, Math.min(this.chartHeight, dataY));
...
export class BrushHandlerCategorical {
  ...
  private clientXToCategory(e: PointerEvent): string | undefined {
    ...
    for (const category of this.categories) {
      ...
      if (dataX >= bandStart && dataX < bandEnd) {
        return category;
      }
    }
