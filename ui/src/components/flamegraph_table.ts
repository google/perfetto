...
export interface FlamegraphTableAttrs {
  // Tree-capable DataGrid source: FlamegraphTreeDataSource over resident
  // nodes, or a SQL source that fetches one level per expansion.
...
// Tree table over a flamegraph: one expandable row per node with self and
// cumulative values and share of the root total. DataGrid id-based tree mode
// keeps expansion O(visible rows) and the DOM virtualised.
export class FlamegraphTable implements m.ClassComponent<FlamegraphTableAttrs> {
  view({attrs}: m.CVnode<FlamegraphTableAttrs>): m.Children {
    ...
    return m(DataGrid, {
      ...
      fillHeight: true,
      // The flamegraph's own filter bar is the filtering surface; row-level
      // grid filters have no meaning on a tree walk.
...
// Tree-mode source over the flamegraph nodes the canvas already holds.
// Children are indexed once; useRows() walks only the expanded rows, sorting
// siblings by the requested column (total descending by default).
