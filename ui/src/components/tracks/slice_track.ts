...
export class SliceTrack<T extends RowSchema> implements TrackRenderer {
  ...
  render(trackCtx: TrackRenderContext): void {
    ...
    this.renderSelectedSlice(trackCtx);
  ...
  private renderSelectedSlice(trackCtx: TrackRenderContext): void {
    ...
    if (isInstant) {
      ...
      const rightPx = timescale.timeToPx(Time.add(data.ts, data.dur));
