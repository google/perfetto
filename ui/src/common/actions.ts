...
import { Aggregation, AggregationFunction, TableColumn, tableColumnEquals, toggleEnabled } from '../frontend/pivot_table_types';
import { DebugTrackV2Config } from '../tracks/debug/slice_track';

import { randomColor } from './colorizer';
import { computeIntervals, DropDirection, performReordering } from './dragndrop_logic';
...
function isFilteredTrack(track: Partial<AddTrackArgs>): boolean {
    return (
        globals.trackFilteringEnabled && globals.filteredTracks.some(filtered => isAddTrackArgs(filtered) && isSameTrack(filtered, track))
    );
}
...
function isFilteredTrackGroup(trackGroup: Partial<AddTrackGroupArgs>): boolean {
    return (
        globals.trackFilteringEnabled &&
        globals.filteredTracks.some(filtered => isAddTrackGroupArgs(filtered) && filtered.id === trackGroup.id)
    );
}
...
export const StateActions = {
    ...
    updateUiTrackIdByTraceTrackId(trackState: TrackState, uiTrackId: string, updater: (trackId: number, uiTrackId: string) => void) {
    ...
    fillUiTrackIdByTraceTrackId(state: StateDraft, trackState: TrackState, uiTrackId: string) {
        const setUiTrackId = (trackId: number, uiTrackId: string) => {
            if (state.uiTrackIdByTraceTrackId[trackId] !== undefined && state.uiTrackIdByTraceTrackId[trackId] !== uiTrackId) {
                throw new Error(
                    `Trying to map track id ${trackId} to UI track ${uiTrackId}, already mapped to ${state.uiTrackIdByTraceTrackId[trackId]}`
                );
            }
    ...
    cleanUiTrackIdByTraceTrackId(state: StateDraft, trackState: TrackState, uiTrackId: string) {
    ...
    setUtidToTrackSortKey(state: StateDraft, args: { threadOrderingMetadata: UtidToTrackSortKey }) {
    ...
    addDebugTrack(state: StateDraft, args: { engineId: string; name: string; config: DebugTrackV2Config }): void {
    ...
    sortThreadTracks(state: StateDraft, _: {}) {
        const getFullKey = (a: string) => {
            ...
            return [
                threadSortKey ? threadSortKey.sortKey : PrimaryTrackSortKey.ORDINARY_THREAD,
                threadSortKey && threadSortKey.tid !== undefined ? threadSortKey.tid : Number.MAX_VALUE,
    ...
    updateAggregateSorting(state: StateDraft, args: { id: string; column: string }) {
    ...
    moveTrack(state: StateDraft, args: { srcId: string; op: 'before' | 'after'; dstId: string }): void {
    ...
    toggleTrackGroupCollapsed(state: StateDraft, args: { trackGroupId: string }): void {
    ...
    setEngineReady(state: StateDraft, args: { engineId: string; ready: boolean; mode: EngineMode }): void {
    ...
    setEngineFailed(state: StateDraft, args: { mode: EngineMode; failure: string }): void {
    ...
    setPermalink(state: StateDraft, args: { requestId: string; hash: string }): void {
    ...
    setRecordConfig(state: StateDraft, args: { config: RecordConfig; configType?: LoadedConfig }): void {
    ...
    addAutomaticNote(state: StateDraft, args: { timestamp: TPTime; color: string; text: string }): void {
        const id = generateNextId(state);
        state.notes[id] = {
            noteType: 'DEFAULT',
            id,
    ...
    markCurrentArea(state: StateDraft, args: { color: string; persistent: boolean }): void {
        if (state.currentSelection === null || state.currentSelection.kind !== 'AREA') {
            return;
        }
    ...
    toggleMarkCurrentArea(state: StateDraft, args: { persistent: boolean }) {
        ...
        if (selection != null && selection.kind === 'AREA' && selection.noteId !== undefined) {
    ...
    changeNoteColor(state: StateDraft, args: { id: string; newColor: string }): void {
    ...
    removeNote(state: StateDraft, args: { id: string }): void {
        ...
        if (state.currentSelection.kind === 'NOTE' && state.currentSelection.id === args.id) {
            state.currentSelection = null;
        } else if (state.currentSelection.kind === 'AREA' && state.currentSelection.noteId === args.id) {
            state.currentSelection.noteId = undefined;
        }
    },

    selectSlice(state: StateDraft, args: { id: number; trackId: string; scroll?: boolean }): void {
        state.currentSelection = {
            kind: 'SLICE',
            id: args.id,
    ...
    selectCounter(state: StateDraft, args: { leftTs: TPTime; rightTs: TPTime; id: number; trackId: string }): void {
        state.currentSelection = {
            kind: 'COUNTER',
            leftTs: args.leftTs,
    ...
    selectHeapProfile(state: StateDraft, args: { id: number; upid: number; ts: TPTime; type: ProfileType }): void {
        state.currentSelection = {
            kind: 'HEAP_PROFILE',
            id: args.id,
            upid: args.upid,
    ...
    selectCpuProfileSample(state: StateDraft, args: { id: number; utid: number; ts: TPTime }): void {
        state.currentSelection = {
    ...
    expandFlamegraphState(state: StateDraft, args: { expandedCallsite?: CallsiteInfo }): void {
        if (state.currentFlamegraphState === null) return;
        state.currentFlamegraphState.expandedCallsite = args.expandedCallsite;
    },

    changeViewFlamegraphState(state: StateDraft, args: { viewingOption: FlamegraphStateViewingOption }): void {
        if (state.currentFlamegraphState === null) return;
        state.currentFlamegraphState.viewingOption = args.viewingOption;
    },

    changeFocusFlamegraphState(state: StateDraft, args: { focusRegex: string }): void {
        if (state.currentFlamegraphState === null) return;
        state.currentFlamegraphState.focusRegex = args.focusRegex;
    },

    selectChromeSlice(state: StateDraft, args: { id: number; trackId: string; table: string; scroll?: boolean }): void {
        state.currentSelection = {
            kind: 'CHROME_SLICE',
            id: args.id,
            trackId: args.trackId,
    ...
    selectThreadState(state: StateDraft, args: { id: number; trackId: string }): void {
        state.currentSelection = {
            kind: 'THREAD_STATE',
            id: args.id,
            trackId: args.trackId
    ...
    selectLog(state: StateDraft, args: { id: number; trackId: string; scroll?: boolean }): void {
        state.currentSelection = {
            kind: 'LOG',
            id: args.id,
            trackId: args.trackId
        ...
        for (const [addRemove, name] of diffs) {
            switch (addRemove) {
                case 'add':
                    ...
                    break;
                case 'remove':
                    state.ftraceFilter.excludedNames = state.ftraceFilter.excludedNames.filter((excluded: string) => excluded !== name);
                    break;
                default:
    ...
    setAvailableAdbDevices(state: StateDraft, args: { devices: AdbRecordingTarget[] }): void {
    ...
    reSelectArea(state: StateDraft, args: { areaId: string; noteId: string }): void {
        state.currentSelection = {
            kind: 'AREA',
            areaId: args.areaId,
            noteId: args.noteId
    ...
    toggleTrackSelection(state: StateDraft, args: { id: string; isTrackGroup: boolean }) {
        const selection = state.currentSelection;
    ...
    requestSelectedMetric(state: StateDraft, _: {}): void {
        ...
        state.metrics.requestedMetric = state.metrics.availableMetrics[state.metrics.selectedIndex];
    },
    ...
    setAvailableMetrics(state: StateDraft, args: { availableMetrics: string[] }): void {
    ...
    setMetricSelectedIndex(state: StateDraft, args: { index: number }): void {
        if (!state.metrics.availableMetrics || args.index >= state.metrics.availableMetrics.length) {
    ...
    togglePivotTable(state: StateDraft, args: { areaId: string | null }) {
        state.nonSerializableState.pivotTable.selectionArea =
            args.areaId === null ? undefined : { areaId: args.areaId, tracks: globals.state.areas[args.areaId].tracks };
        if (args.areaId !== state.nonSerializableState.pivotTable.selectionArea?.areaId) {
            state.nonSerializableState.pivotTable.queryResult = null;
        }
    },

    setPivotStateQueryResult(state: StateDraft, args: { queryResult: PivotTableResult | null }) {
    ...
    addPivotTableAggregation(state: StateDraft, args: { aggregation: Aggregation; after: number }) {
        state.nonSerializableState.pivotTable.selectedAggregations.splice(args.after, 0, args.aggregation);
    },

    removePivotTableAggregation(state: StateDraft, args: { index: number }) {
        state.nonSerializableState.pivotTable.selectedAggregations.splice(args.index, 1);
    },

    setPivotTableQueryRequested(state: StateDraft, args: { queryRequested: boolean }) {
        state.nonSerializableState.pivotTable.queryRequested = args.queryRequested;
    },

    setPivotTablePivotSelected(state: StateDraft, args: { column: TableColumn; selected: boolean }) {
        toggleEnabled(tableColumnEquals, state.nonSerializableState.pivotTable.selectedPivots, args.column, args.selected);
    },

    setPivotTableAggregationFunction(state: StateDraft, args: { index: number; function: AggregationFunction }) {
        state.nonSerializableState.pivotTable.selectedAggregations[args.index].aggregationFunction = args.function;
    },

    setPivotTableSortColumn(state: StateDraft, args: { aggregationIndex: number; order: SortDirection }) {
        state.nonSerializableState.pivotTable.selectedAggregations = state.nonSerializableState.pivotTable.selectedAggregations.map(
            (agg, index) => ({
                column: agg.column,
                aggregationFunction: agg.aggregationFunction,
                sortDirection: index === args.aggregationIndex ? args.order : undefined
            })
        );
    },
    ...
    removeVisualisedArg(state: StateDraft, args: { argName: string }) {
        state.visualisedArgs = state.visualisedArgs.filter(val => val !== args.argName);
    },

    setPivotTableArgumentNames(state: StateDraft, args: { argumentNames: string[] }) {
        state.nonSerializableState.pivotTable.argumentNames = args.argumentNames;
    },

    changePivotTablePivotOrder(state: StateDraft, args: { from: number; to: number; direction: DropDirection }) {
    ...
    changePivotTableAggregationOrder(state: StateDraft, args: { from: number; to: number; direction: DropDirection }) {
        const aggregations = state.nonSerializableState.pivotTable.selectedAggregations;
        state.nonSerializableState.pivotTable.selectedAggregations = performReordering(
            computeIntervals(aggregations.length, args.from, args.to, args.direction),
            aggregations
        );
    },
    ...
    removeLogTag(state: StateDraft, args: { tag: string }) {
        state.logFilteringCriteria.tags = state.logFilteringCriteria.tags.filter(t => t !== args.tag);
    },
    ...
    toggleCollapseByTextEntry(state: StateDraft, _: {}) {
        state.logFilteringCriteria.hideNonMatching = !state.logFilteringCriteria.hideNonMatching;
    }
};
...
export const Actions = new Proxy<DeferredActions<typeof StateActions>>({} as any, {
