...
const leftSidebarState = {
  collapsed: true,
  tab: 'bigtrace', // 'bigtrace' or 'metrics'
  toggle() {
...
const SchemaSidebar: m.Component = {
  view: () => {
    const isActive = (tab: string) => {
      return leftSidebarState.tab === tab ? '.isActive' : '';
    };

    return m('.side-nav.left', {
      class: leftSidebarState.collapsed ? 'collapsed' : '',
    }, [
      !leftSidebarState.collapsed && m('.sidebar-content-wrapper', [
        m('.TabContainer', [
          m('button.TabButton' + isActive('bigtrace'), {
            onclick: () => leftSidebarState.selectTab('bigtrace'),
          }, 'BigTrace'),
          m('button.TabButton' + isActive('metrics'), {
            onclick: () => leftSidebarState.selectTab('metrics'),
          }, 'Metrics'),
        ]),
        leftSidebarState.tab === 'metrics' && m(FilterableList, {
          title: 'Select Schema',
...
const BigtracePage: m.Component = {
  view: () => {
    const showQueryEditor = leftSidebarState.tab === 'bigtrace';
    return m(
      '.bigtrace-page-content',
      {
        style: {
          display: 'flex',
      ...
      showQueryEditor && m(Editor, {
        initialText: 'SELECT * FROM ...\n\n\n\n\n\n\n\n\n',
        onUpdate: (s: string) => console.log(s),
      }),
      m(
        '.bigtrace-page',
        {
...
const BigTraceShell: m.Component = (() => {
  ...
  return {
    oncreate: (vnode) => {
      leftSidebarState.selectTab('metrics');
  ...
  raf.mount(document.body, {
    view: () =>
      m(ThemeProvider, {theme: 'light'}, [
        m(OverlayContainer as any, {fillParent: true}, [
          m(BigTraceShell),
        ]),
      ]),
  });
