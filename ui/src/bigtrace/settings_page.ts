...
import { Setting as BigTraceSetting } from './settings_types';
      ...
      m(
        ...
        [
          ...
          m(
            ...
            {
              style: {
                pointerEvents: disabled !== undefined && disabled ? 'none' : 'auto',
              },
            },
            controls
          ),
...
export class SettingsPage implements m.ClassComponent {
  ...
  view() {
    ...
    const isEndpointMatch = !!(endpointSetting &&
      (endpointSetting.name.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
        endpointSetting.description.toLowerCase().includes(this.searchQuery.toLowerCase())));
    ...
    return m(
        SettingsShell,
        {
            title: 'Settings',
            className: 'page',
          stickyHeaderContent: m('.pf-settings-header-content', {
            style: {
              display: 'flex',
              alignItems: 'center',
              gap: '8px',
            ...
            endpointManager.isReloadRequired() && m(Button, {
