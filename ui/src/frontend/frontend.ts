...
import {maybeOpenTraceFromRoute as maybeDoOpenTraceFromRoute} from './trace_url_handler';
import {
...
import {AppImpl, AppInitArgs} from '../core/app_impl';
...
/** The frontend — main entry-point — of the UI. */
export class Frontend {
  ...
  protected routeChange(route: Route) {
    ...
    this.maybeOpenTraceFromRoute(route);
  }

  protected maybeOpenTraceFromRoute(route: Route): void {
    maybeDoOpenTraceFromRoute(route);
  }

  protected createContentSecurityPolicy(): Readonly<
  ...
  protected setupContentSecurityPolicy(): void {
    const policy = this.createContentSecurityPolicy();
  ...
  /** The main entry-point, starting the frontend. */
  start(): Promise<void> {
    ...
    const result = cssLoadPromise.then(() => this.onCssLoaded(app));

    if (app.testingMode) {
      document.body.classList.add('testing');
    }

    (window as {} as IdleDetectorWindow).waitForPerfettoIdle = (
      ms?: number,
    ) => {
  ...
  protected createApp(args: AppInitArgs): AppImpl {
    AppImpl.initialize(args);
    return AppImpl.instance;
  }

  protected installErrorHandlers(app: AppImpl): void {
  ...
  protected createRouter(): Router {
    const router = new Router(this.routeChange.bind(this));
    return router;
  }

  protected mountMainUI(
    app: AppImpl,
    themeSetting: Setting<'dark' | 'light'>,
  ): Disposable {
    ...
    return {
      [Symbol.dispose]: () => raf.mount(document.body, null),
    };
  }

  // Add support for opening traces from postMessage().
  protected installPostMessageHandler(): void {
    const delegate = this.handlePostMessage.bind(this);
    window.addEventListener(
      'message',
      (event) => postMessageHandler(event, delegate),
      {passive: true},
    );
  }

  protected handlePostMessage(_messageEvent: MessageEvent): boolean {
    return false;
  }

  protected detectPreloadedTrace(app: AppImpl): void {
    ...
    // otherwise.
    this.maybeChangeRpcPortFromFragment();
    checkHttpRpcConnection().then(() => {
      ...
      this.installPostMessageHandler();
  ...
  protected onCssLoaded(app: AppImpl): void {
    this.initializeDOM();
    ...
    const themeSetting = app.settings.register<'dark' | 'light'>({
      id: 'theme',
    ...
    this.mountMainUI(app, themeSetting);

    this.detectPreloadedTrace(app);
  ...
  protected maybeChangeRpcPortFromFragment() {
    const route = Router.parseUrl(window.location.href);
          ...
          buttons: [
            {
              ...
              primary: true,
              action: () =>
                Router.navigate('#!/flags/cspAllowAnyWebsocketPort'),
            },
          ],
        });
      } else {
