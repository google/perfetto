  ...
  private renderBulkTracksMenu(trace: TraceImpl, selection: AreaSelection) {
    ...
    return m(
      Stack,
      {
        spacing: 'small',
        orientation: 'horizontal',
        className: 'pf-timeline-toolbar__bulk',
      },
      [
        m(Icon, {icon: 'check', className: 'pf-timeline-toolbar__bulk-icon'}),
        m('span', `${selection.tracks.length}`),
        m(
          PopupMenu,
          {
            trigger: m(Button, {
              // icon: this.getIconForCount(selection.tracks.length),
              icon: 'move_item',
              title: `${selection.tracks.length} tracks`,
              rounded: true,
              compact: COMPACT_BUTTONS,
              // intent: Intent.Primary,
              // variant: ButtonVariant.Filled,
        ...
        m(
          PopupMenu,
          {
            trigger: m(Button, {
              ...
              title: `${selection.tracks.length} tracks`,
              rounded: true,
              disabled: !Boolean(settingsMenuItems),
              compact: COMPACT_BUTTONS,
              // intent: Intent.Primary,
              // variant: ButtonVariant.Filled,
            }),
          ...
          m(MenuTitle, {
            label: `Change settings for ${selection.tracks.length} tracks`,
          }),
          settingsMenuItems,
        ),
        m(Button, {
          icon: 'check_box',
          onclick: () => trace.selection.clearSelection(),
          compact: COMPACT_BUTTONS,
        }),
      ],
    );
    ...
    return m(
      PopupMenu,
      {
        trigger: m(Button, {
          ...
          icon: 'workspaces',
          compact: COMPACT_BUTTONS,
          className: 'pf-timeline-toolbar__workspace-button',
        }),
      },
      [
