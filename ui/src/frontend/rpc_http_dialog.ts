...
export async function CheckHttpRpcConnection(): Promise<void> {
  ...
  if (tpStatus.loadedTraceName) {
    ...
    // doesn't handle it
    const embedderResult = toPreloadedDialogResult(await embedderContext?.shouldUsePreloadedTrace?.(tpStatus.loadedTraceName));
    const result = embedderResult ?? await showDialogToUsePreloadedTrace(tpStatus);
    switch (result) {
...
function toPreloadedDialogResult(option: PreloadedTraceOption | undefined): PreloadedDialogResult | undefined {
  if (!option) {
    return undefined;
  }
  switch (option) {
    case 'use_trace': return PreloadedDialogResult.UseRpcWithPreloadedTrace;
    case 'reset_rpc': return PreloadedDialogResult.UseRpc;
    case 'use_wasm': return PreloadedDialogResult.UseWasm;
    default: throw new Error(`Unrecognized pre-loaded trace option: ${option}`);
  };
}
