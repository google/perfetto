...
//                            +------------------------------------------------+
//
// Client / server architecture (for ui/build while ui/run-dev-server is live):
// ---------------------------------------------------------------------------
// The script operates in one of three roles at startup, chosen by looking at
// out/ui/watch.lock:
//
//   1) Full build (default ui/build invocation). Acquires watch.lock, runs
//      all tasks synchronously, releases the lock on exit.
//
//   2) Watch-mode server (ui/run-dev-server -> build.js --watch --serve).
//      Acquires watch.lock with {pid, watch: true, host, port}, spawns tsc
//      --watch and rollup --watch, serves the dev-server, and tees ALL
//      subprocess stdout/stderr plus this script's own console.log into
//      out/ui/ui_build.log, timestamped line-by-line, ANSI-stripped.
//
//   3) Client (another ui/build while a watch-mode server from (2) is live).
//      Instead of trying to build itself, it:
//        a) Bumps SCM_REVISION in ui/src/gen/perfetto_version.ts to
//           "perfetto_incremental_build:N" (monotonic counter). tsc-watch
//           and rollup-watch in the server see this change through their
//           own file watchers and cascade-rebuild the bundle. No direct
//           IPC; the filesystem is the wire.
//        b) Polls out/ui/dist_version/frontend_bundle.js until it contains
//           the same "perfetto_incremental_build:N" marker -- this fence
//           proves every source change made before the client ran is now
//           in the bundle (tsc/rollup process edits in order).
//        c) While polling, tails ui_build.log from offset 0 and mirrors it
//           to its own stdout, so the agent driving `ui/build` sees the
//           tsc / rollup output generated by its edits.
//        d) Early-exits 1 if the log shows tsc "Found N errors" or a
//           rollup "[!] " line; exits 0 on fence match; times out at 120s.
//        e) On exit, truncates ui_build.log so the next client starts fresh
//           (the server opens the log with 'a' append mode, so this is
//           race-safe: the next server write goes to offset 0).
//
// This lets an AI agent iterate on ui/*.ts quickly: keep a watch-mode server
// running, edit, run ui/build to get a synchronous success/failure + the
// tsc/rollup output, repeat.
...
// When the script runs as a watch-mode server, subprocess stdout/stderr (and
// our own console logs) are tee'd to this stream so an incremental-build
// client invocation can drain and display them. undefined otherwise.
...
`,
  ...
  cfg.buildLogFile = pjoin(cfg.outDir, 'ui_build.log');

  // Coordination via watch.lock: if a watch-mode server is already
  // running, become an incremental-build client. If another full build is
  // running, bail out. With --no-build we don't write to outUiDir, so we
  // can coexist with anything.
  if (!args.no_build) {
    const watchInfo = prepareBuildLock(!!args.watch);
    if (watchInfo) {
      return runClientMode(watchInfo);
    }
  }
  ...
  const ver = childProcess.spawnSync(
      'python3', [VERSION_SCRIPT, '--stdout'],
      {cwd: cfg.outDir, stdio: ['ignore', 'pipe', 'inherit']});
  cfg.version = ver.stdout.toString().trim();
  ...
  // In watch-mode server, start tee'ing all subprocess output (and our own
  // console logs) to ui_build.log so the client can stream it back.
  if (cfg.watch && !args.no_build) {
    setupWatchLog();
  }
...
function startServer() {
  ...
  server.on('listening', () => {
    ...
    // Expose the port into a known file for AI agents.
    fs.writeFileSync(pjoin(cfg.outDir, 'dev_server.port'), `${port}\n`);
  });
}
...
function exec(cmd, args, opts) {
  ...
  // When the log tee is active (watch-mode server), pipe stdout+stderr so we
  // can mirror them into the log file. Otherwise just inherit.
  const teeing = logStream !== undefined;
  const stdio = teeing
      ? ['ignore', 'pipe', 'pipe']
      : ['ignore', 'inherit', 'inherit'];
  const spwOpts = {cwd: cfg.outDir, stdio};
  ...
  if (opts.async) {
    ...
    if (teeing) attachLogTee(proc, path.basename(cmd));
...
// ------------------------------------------
// Watch-server / client via files in out/
// This is to allow ui/build to be run concurrently while a dev-server is
// running, receive a copy of the logs and run until the marker in version.ts
// is seen (or an error occurs).
// ------------------------------------------

// Returns {pid, host, port} if a watch-mode server with a live PID exists,
// else undefined. Cleans up a stale watch.lock if the PID is dead.
function readWatchLockFile() {
  if (!fs.existsSync(cfg.lockFile)) return;
  let info;
  try {
    info = JSON.parse(fs.readFileSync(cfg.lockFile, 'utf8'));
  } catch (_) {
    fs.unlinkSync(cfg.lockFile);
    return;
  }
  try {
    process.kill(parseInt(info.pid), 0);  // Liveness probe.
  } catch (_) {
    console.log(`Removing stale ${cfg.lockFile} for PID ${info.pid}`);
    fs.unlinkSync(cfg.lockFile);
    return;
  }
  if (info.pid === process.pid) return;  // That's us.
  return info;
}

// Writes watch.lock claiming this process as the build owner and registers
// a cleanup on exit. If a live build instance is already running:
//   - if it's a watch-mode server, returns its info so the caller can enter
//     client mode;
//   - otherwise errors out (two concurrent full builds would clobber).
// Returns undefined when the lock was acquired by this process.
function prepareBuildLock(watch) {
  const existing = readWatchLockFile();
  if (existing) {
    if (existing.watch && !watch) return existing;
    console.error(
        `Error: a build.js instance is already running ` +
        `(${cfg.lockFile} PID=${existing.pid}).`);
    console.error('Hint: use --no-build (-n) to skip the build.');
    process.exit(1);
  }
  const info = {pid: process.pid, watch};
  writeAtomic(cfg.lockFile, JSON.stringify(info, null, 2) + '\n');
...
function releaseBuildLock() {
  if (!fs.existsSync(cfg.lockFile)) return;
  try {
    const info = JSON.parse(fs.readFileSync(cfg.lockFile, 'utf8'));
    if (info.pid !== process.pid) return;  // Someone else's.
  } catch (_) {}
  try { fs.unlinkSync(cfg.lockFile); } catch (_) {}
}

// Atomic write via tmp + rename so watchers never see a partial file.
function writeAtomic(dstPath, contents) {
...
// ------------------------------------------
// Log tee (watch-mode server side)
// ------------------------------------------

function setupWatchLog() {
  // Truncate on startup so each server session begins with an empty log.
  // Then reopen in append mode: every write goes to the current EOF, which
  // means if a client truncates the log mid-session, subsequent server writes
  // correctly start from offset 0 (no sparse holes).
  fs.writeFileSync(cfg.buildLogFile, '');
  logStream = fs.createWriteStream(cfg.buildLogFile, {flags: 'a'});

  // Tee console.log / console.error so our own progress lines show up in the
  // log too.
...
// Prefixes each non-empty line with [HH:MM:SS] (wall clock) and an optional
// [tag], then writes to the log stream. This guarantees every line in
// ui_build.log is timestamped and readable, regardless of whether the source
// had its own (ANSI-coloured) timestamp.
function writeToLog(text, tag) {
  if (!logStream) return;
  const prefix = tag ? `${tag}: ` : '';
  const ansiStripped = text.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, '');
  logStream.write(prefix + ansiStripped.trim() + '\n');
}

function attachLogTee(proc, tag) {
  proc.stdout.on('data', (chunk) => {
    process.stdout.write(chunk);
    writeToLog(chunk.toString('utf8'), tag);
  });
...
// ------------------------------------------
// Client mode: run when a watch-server is live
// ------------------------------------------

async function runClientMode(watchInfo) {
  console.log(
      `Watch-mode server detected (PID ${watchInfo.pid}). ` +
      `Triggering incremental build.`);
  const bundlePath = pjoin(cfg.outDir, 'ui/dist_version/frontend_bundle.js');

  // Rewrites SCM_REVISION to "perfetto_incremental_build:N" and returns N.
  // If the file still carries the original sha1 (first bump after server start),
  // the sha1 is replaced and N starts at 1.
  // This is used as a fence to detect when a new incremental build, triggered by
  // a client invocation of build while a build --watch is running, completes.
  const fp = pjoin(ROOT_DIR, 'ui/src/gen/perfetto_version.ts');
  const src = fs.readFileSync(fp, 'utf8');
  const m = src.match(/perfetto_incremental_build:(\d+)/);
  const counter = m ? parseInt(m[1]) + 1 : 1;
  const marker = `perfetto_incremental_build:${counter}`;

  const updated = src.replace(
      /export const SCM_REVISION = "[^"]*";/,
      `export const SCM_REVISION = "${marker}";`);
  if (updated === src) {
    throw new Error(`Could not locate SCM_REVISION in ${fp}`);
  }
  writeAtomic(fp, updated);

  const fenceRegex =
      /SCM_REVISION\s*=\s*"perfetto_incremental_build:(\d+)"/;
  const tErrPattern = /Found [1-9]\d* error/;
  const rErrPattern = /^\[!\] /m;

  let logOffset = 0;
  let errorSeen;
  const drainLog = () => {
    if (!fs.existsSync(cfg.buildLogFile)) return '';
    const size = fs.statSync(cfg.buildLogFile).size;
    if (size <= logOffset) return '';
    const fd = fs.openSync(cfg.buildLogFile, 'r');
    ...
    process.stdout.write(chunk);
    if (!errorSeen) {
      if (tErrPattern.test(chunk)) errorSeen = 'tsc reported errors';
      else if (rErrPattern.test(chunk)) errorSeen = 'rollup reported an error';
    }
    return chunk;
  };

  // Drains any remaining log output, truncates the log so the next client
  // invocation starts empty, and returns the given exit code.
  const finish = (code, msg) => {
    drainLog();
    try { fs.truncateSync(cfg.buildLogFile, 0); } catch (_) {}
  ...
  while (true) {
    drainLog();
    // Check the bundle for the fence.
    try {
      const bundle = fs.readFileSync(bundlePath, 'utf8');
      const bm = bundle.match(fenceRegex);
      if (bm && parseInt(bm[1]) >= counter) {
        const secs = ((performance.now() - tStart) / 1000).toFixed(2);
        return finish(0, `\nBuild ready (fence ${counter} in ${secs}s)`);
      }
    } catch (_) { /* bundle may be mid-write */ }
    // Early exit on tsc/rollup error.
    if (errorSeen) {
      return finish(1, `\nBuild failed: ${errorSeen}. See above for details.`);
    }
    if (performance.now() - tStart > TIMEOUT_MS) {
      return finish(
          1, `\nTimed out after ${TIMEOUT_MS / 1000}s waiting for ${marker}.`);
    }
