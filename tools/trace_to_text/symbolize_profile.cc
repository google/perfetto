...
void TraceSymbolTable::WriteResult(std::ostream* output, uint32_t seq_id) {
  ...
  for (const auto& p : symbols_for_frame_) {
    ...
    protos::ProfiledFrameSymbols* sym = packet.mutable_profiled_frame_symbols();
...
std::string TraceSymbolTable::ResolveString(uint64_t iid) {
